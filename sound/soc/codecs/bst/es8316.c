...
/*
 * es8316 -- es8316 ALSA SoC audio driver
 *
...
static const DECLARE_TLV_DB_SCALE(dac_vol_tlv, -9600, 34, 1);
				 ...
				 int clk_id, unsigned int freq, int dir)
{
	...
	ES8316_PRINT(component->dev, "[%s]-[%d] freq:%ld", __func__, __LINE__, freq);
			      ...
			      struct snd_soc_dai *dai)
{
	...
	if (playback) {
		...
		snd_soc_component_write(component, ES8316_HPMIX_SWITCH_REG14,
					0x88);
		snd_soc_component_write(component, ES8316_HPMIX_PDN_REG15,
					0x00);
		snd_soc_component_write(component, ES8316_HPMIX_VOL_REG16,
					0xBB);
		snd_soc_component_write(component, ES8316_CPHP_PDN2_REG1A,
					0x10);
		snd_soc_component_write(component, ES8316_CPHP_LDOCTL_REG1B,
					0x30);
		snd_soc_component_write(component, ES8316_CPHP_PDN1_REG19,
					0x02);
		snd_soc_component_write(component, ES8316_DAC_PDN_REG2F, 0x00);
		snd_soc_component_write(component, ES8316_CPHP_OUTEN_REG17,
					0x66);
	...
	if (--es8316->pwr_count == 0) {
		if (!es8316->hp_inserted)
			snd_soc_component_write(component, ES8316_SYS_PDN_REG0D,
						0x3F);
		snd_soc_component_write(component, ES8316_CLKMGR_CLKSW_REG01,
					0xF3);
	}
#endif
				...
				struct snd_soc_dai *dai)
{
	...
	case SNDRV_PCM_FORMAT_S16_LE:
		ES8316_PRINT(component->dev, "[%s]-[%d] format 16_le\n",
			     __func__, __LINE__);
		val = ES8316_DACWL_16;
		break;
...
static int es8316_init_regs(struct snd_soc_component *component)
{
	...
	snd_soc_component_write(component, ES8316_CLKMGR_ADCDIV2_REG05, 0x80);
	snd_soc_component_write(component, ES8316_CLKMGR_DACDIV1_REG06, 0x11);
	snd_soc_component_write(component, ES8316_CLKMGR_DACDIV2_REG07, 0x80);
...
static int es8316_probe(struct snd_soc_component *component)
{
	...
	if (!ret) {
		...
		if (!ret) {
			...
			snd_soc_component_write(component, ES8316_DAC_PDN_REG2F,
						0x11);
			snd_soc_component_write(component,
			...
			if (!es8316->hp_inserted)
				snd_soc_component_write(component,
							ES8316_SYS_PDN_REG0D,
							0x3F);
			snd_soc_component_write(component, ES8316_SYS_LP1_REG0E,
						0xFF);
			snd_soc_component_write(component, ES8316_SYS_LP2_REG0F,
						0xFF);
			snd_soc_component_write(component,
						ES8316_CLKMGR_CLKSW_REG01,
						0xF3);
	...
	/**
	 * Setting Differential Mux
	 * 00=Lin1-Rin1 (MIC DF)
	 * 01=Lin2-Rin2 (Board DF)
	 * 10=Lin1 DF2SE (MIC)
	 * 11=Lin2 DF2SE (Board)
	 */
	if (of_property_read_u32(component->dev->of_node, "es8316,analog-input-mux", &input_mux) == 0) {
		snd_soc_component_update_bits(component,
