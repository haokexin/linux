...
#define C1200BASE_LBLSP0_DIV				0x20000000
#define C1200BASE_LBLSP1_DIV				0x20020000
void __iomem *c1200_lblsp1_div_config;
#define CLK_DIV_MIN 1UL
#define CLK_DIV_MAX 0xFFFFUL
#define I2S_WCLK (884736000UL)
//#define I2S_WCLK (36864000UL)
#define TDM_WCLK (884736000UL)
//#define TDM_WCLK (36864000UL)
#define MCLK_DEFAULT_RATE (12288000UL)
#define PCM3168A_MAX_SYSCLK		(36864000UL)
...
struct pcm3168a_priv {
	...
	struct snd_soc_dai_driver dai_drv[1];
};

static int bst_pcm3168a_set_mclk_div(unsigned int freq);
...
#define PCM3168A_NUM_SCKI_RATIOS_ADC	ARRAY_SIZE(pcm3168a_scki_ratios)
...
static void pcm3168a_update_fixup_pcm_stream(struct snd_soc_dai *dai)
{
    ...
    for_each_pcm_streams(i) {
		...
		if (i == PCM3168A_DAI_DAC)
			channel_max = 8;
		else	
			channel_max = 6;

		if (pcm3168a->io_params[i].fmt == PCM3168A_FMT_RIGHT_J) {
		 	...
		 	*/
			if (pcm3168a->io_params[i].tdm_slots != 2)
				channel_max = 2;
		}
...
static int pcm3168a_set_dai_fmt(struct snd_soc_dai *dai, unsigned int format)
{
	...
	pcm3168a->io_params[PCM3168A_DAI_DAC].master_mode = master_mode;
	pcm3168a->io_params[PCM3168A_DAI_DAC].fmt = fmt;
//	regmap_update_bits(pcm3168a->regmap, reg, mask, fmt << shift);
	...
	pcm3168a->io_params[PCM3168A_DAI_ADC].master_mode = master_mode;
	pcm3168a->io_params[PCM3168A_DAI_ADC].fmt = fmt;
//	regmap_update_bits(pcm3168a->regmap, reg, mask, fmt << shift);
				 ...
				 int slot_width)
{
	struct snd_soc_component *component = dai->component;
	struct pcm3168a_priv *pcm3168a = snd_soc_component_get_drvdata(component);
	struct pcm3168a_io_params *io_params;
	int i;
	...
	for_each_pcm_streams(i) {
		...
		io_params = &pcm3168a->io_params[i];
			     ...
			     struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		io_params = &pcm3168a->io_params[PCM3168A_DAI_DAC];
		reg = PCM3168A_DAC_PWR_MST_FMT;
		mask = PCM3168A_DAC_MSDA_MASK;
		shift = PCM3168A_DAC_MSDA_SHIFT;
	} else {
		io_params = &pcm3168a->io_params[PCM3168A_DAI_ADC];
		reg = PCM3168A_ADC_MST_FMT;
	...
	switch (slot_width) {
		case 16:
//			dev_err(component->dev, "16-bit slots are supported only for slave mode using right justified，not support for tdm\n");
//			return -EINVAL;
			break;
		case 24:
			break;
	...
	regmap_update_bits(pcm3168a->regmap, reg, mask, val << shift);  //set master or slave
	...
	regmap_update_bits(pcm3168a->regmap, reg, mask, fmt << shift);  //set fmt : i2s tdm
...
static int pcm3168a_init_regs(struct pcm3168a_priv *pcm3168a)
{

	int ret;
	ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_PWR_MST_FMT, 0x86);  //24-bit I2S mode TDM format
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_OP_FLT, 0x00);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_INV, 0x00);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_MUTE, 0x00);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_ATT_DEMP_ZF, 0x00);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_VOL_MASTER, 0xff);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_VOL_CHAN_START, 0xff);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_VOL_CHAN_START+1, 0xff);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_VOL_CHAN_START+2, 0xff);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_VOL_CHAN_START+3, 0xff);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_VOL_CHAN_START+4, 0xff);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_VOL_CHAN_START+5, 0xff);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_VOL_CHAN_START+6, 0xff);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_VOL_CHAN_START+7, 0xff);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_SMODE, 0x00);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_MST_FMT, 0x06);  //24-bit I2S mode TDM format
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_PWR_HPFB, 0x00);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_SEAD, 0x00);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_INV, 0x00);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_MUTE, 0x00);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_ATT_OVF, 0x00);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_VOL_MASTER, 0xd7);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_VOL_CHAN_START, 0xd7);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_VOL_CHAN_START+1, 0xd7);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_VOL_CHAN_START+2, 0xd7);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_VOL_CHAN_START+3, 0xd7);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_VOL_CHAN_START+4, 0xd7);
	if (ret)
		return ret;

	ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_VOL_CHAN_START+5, 0xd7);
	if (ret)
		return ret;
			      ...
			      struct snd_soc_dai *dai)
{
	...
	if (pcm3168a->gpio_rst) {
		 ...
		 */
		gpiod_set_value(pcm3168a->gpio_rst, 0);
		msleep(DIV_ROUND_UP(3846 * 1000, pcm3168a->sysclk));
		gpiod_set_value(pcm3168a->gpio_rst, 1);
		msleep(DIV_ROUND_UP(3846 * 1000, pcm3168a->sysclk));
	} else {
	...
	pcm3168a->sysclk = MCLK_DEFAULT_RATE;
	bst_pcm3168a_set_mclk_div(pcm3168a->sysclk);
...
/* 	ret = pcm3168a_reset(pcm3168a);
	...
	ret = pcm3168a_init_regs(pcm3168a);
	if(ret)
	{
		dev_err(dev, "pcm3168a_init_regs error: %d\n", ret);
		return -EINVAL;
	}
...
static void pcm3168a_shutdown(struct snd_pcm_substream *substream,
				struct snd_soc_dai *dai)
{
	...
	//int ret = 0;
	struct device *dev = component->dev;
	dev_info(dev, "pcm3168a_shutdown\n");

//	regmap_write(pcm3168a->regmap, PCM3168A_RST_SMODE, 0);
/*
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		ret = regmap_write(pcm3168a->regmap, PCM3168A_DAC_OP_FLT, 0xf0);
		if (ret)
			dev_err(dev, "pcm3168a dac power save error: %d\n", ret);
	} else {
		ret = regmap_write(pcm3168a->regmap, PCM3168A_ADC_PWR_HPFB, 0x70);
		if (ret)
			dev_err(dev, "pcm3168a adc power save error: %d\n", ret);
...
static struct snd_soc_dai_driver pcm3168a_dais = {
		...
		.capture = {
			...
			.channels_min = 1,
			.channels_max = 6,
			.rates = SNDRV_PCM_RATE_8000_48000,  //pcm3168a tdm 单线最大支持 48K
...
static int bst_pcm3168a_set_sclk_div(unsigned int freq)
{
	u32 tmp, clk_div;

	if (freq) {
		tmp = ioread32(c1200_lblsp1_div_config + 0x0c);
	
		clk_div = TDM_WCLK / freq;
		if (clk_div < CLK_DIV_MIN || clk_div > CLK_DIV_MAX)
			clk_div = 0x240;   //TDM_WCLK(36.8Mhz)/24 = 1.536MHZ

		iowrite32((tmp & (~(0xffff << 0))) | ((clk_div & 0xffff) << 0),
			  c1200_lblsp1_div_config + 0x0c);
	}
...
static int bst_pcm3168a_hw_params(struct snd_soc_component *component,
			    struct snd_pcm_substream *substream,
			    struct snd_pcm_hw_params *params)
{
	struct pcm3168a_priv *pcm3168a = snd_soc_component_get_drvdata(component);
	unsigned int rate, ratio, max_ratio;
	int i, slot_width;
	unsigned int freq;
	struct device *dev = component->dev;
	int ret;
	...
	//	freq = 2048000;   //mclk_scale : 256
		freq = 4096000;   //mclk_scale : 512

	} else if (rate == 16000 ) {

	//	freq = 4096000;    //mclk_scale : 256
		freq = 8192000;    //mclk_scale : 512

	} else if (rate == 32000 ) {   //这两个 mclk ， wclk 36864000 都分不出来

	//	freq = 8192000;    //mclk_scale : 256
		freq = 16384000;    //mclk_scale : 512

	} else if (rate == 48000 ) {   //pcm3168a tdm 最大支持到 48K

	//	freq = 12288000;    //mclk_scale : 256
		freq = 24576000;    //mclk_scale : 512

	} else if (rate == 44100 ) {   //当前不支持，后面再评估能否支持

		dev_err(dev, "invalid rate\n");
		return -EINVAL;
	...
	dev_info(dev, "sysclk:%lu, rate:%d, ratio:%d\n", pcm3168a->sysclk, rate, ratio);
	...
	ret = bst_pcm3168a_set_mclk_div(pcm3168a->sysclk);  // 分频设置 i2s_mclk
	if (ret) {
		dev_err(component->dev,"Can't set I2S mclk rate: %d\n", ret);
		return ret;
	}

	bitclk = rate * 32 * 8;
	ret = bst_pcm3168a_set_sclk_div(bitclk);
	if (ret) {
		dev_err(component->dev,"Can't set TDM sclk rate: %d\n", ret);
		return ret;
	}
...
static const struct snd_soc_component_driver pcm3168a_driver = {
	...
	.hw_params = bst_pcm3168a_hw_params,
};
...
int pcm3168a_probe(struct device *dev, struct regmap *regmap)
{
	...
	c1200_lblsp1_div_config = ioremap(C1200BASE_LBLSP1_DIV, 0x1000);

	tmp = ioread32(c1200_lblsp1_div_config + 0x168);  //初始化设置 i2s_mclk 为 12.28M
	pr_info("jun:%s, 0x168_mclk: %d \n", __func__, tmp);
	iowrite32((tmp & (~(0xffff << 0))) | ((0x48 & 0xffff) << 0),
			  c1200_lblsp1_div_config + 0x168);
	tmp = ioread32(c1200_lblsp1_div_config + 0x168);
	pr_info("jun:%s, 0x168_mclk: %d \n", __func__, tmp);
	...
	if (IS_ERR(pcm3168a->gpio_rst)) {
	 ...
	 ret = gpiod_direction_output(pcm3168a->gpio_rst, 0);
	 gpiod_set_value(pcm3168a->gpio_rst, 0);
#endif
