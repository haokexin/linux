...
{ \
	...
	const u##sample_bits(*k) = (void *)runtime->dma_area; \
...
\
    ...
    else if(dev->is_mono_tx == 1) \
    { \
            for (i = 0; i < dev->fifo_th; i++) { \
                    iowrite32(k[tx_ptr], dev->i2s_base + LRBR_LTHR(0)); \
                    iowrite32(0, dev->i2s_base + RRBR_RTHR(0)); \
                    period_pos++; \
                    if (++tx_ptr >= runtime->buffer_size) \
                            tx_ptr = 0; \
            } \
...
{ \
	...
	u##sample_bits(*k) = (void *)runtime->dma_area; \
...
\
    ...
    else if(dev->is_mono_rx == 1) \
    { \
            for (i = 0; i < dev->fifo_th; i++) { \
                    k[rx_ptr]= ioread32(dev->i2s_base + LRBR_LTHR(0)); \
                                ioread32(dev->i2s_base + RRBR_RTHR(0)); \
                    period_pos++; \
                    if (++rx_ptr >= runtime->buffer_size) \
                            rx_ptr = 0; \
            } \
