				   ...
				   struct snd_pcm_substream *substream)
{
	...
	dma_data = snd_soc_dai_get_dma_data(asoc_rtd_to_cpu(rtd, 0), substream);
	if (!dma_data) {
        dev_err(rtd->dev, "%s: dma_data is NULL\n", __func__);
        return -EINVAL;
    }
	...
	hw.period_bytes_max = 3*8*hw.period_bytes_max;
	...
	/*pr_info("\n %s hw.period_bytes_min = %zu, hw.period_bytes_max = %zu, hw.buffer_bytes_max%zu\n",
	 __func__, hw.period_bytes_min, hw.period_bytes_max, hw.buffer_bytes_max);*/
