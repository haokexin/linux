...
struct scmi_clock_info {
	...
	const char * parent_index[8];
	unsigned char parent_num;
	enum clock_type type;
	unsigned int mux_index;
	unsigned int fix_factor_mult;
	unsigned int fix_factor_div;
	u64  fix_rate;
	bool rate_discrete;
	union {
		struct {
...
struct scmi_clk_proto_ops {
	...
	int (*is_enable)(const struct scmi_protocol_handle *ph, u32 clk_id);

	int (*get_parent)(const struct scmi_protocol_handle *ph, u32 clk_id);


	int (*has_child)(const struct scmi_protocol_handle *ph, u32 clk_id);

	int (*set_parent)(const struct scmi_protocol_handle *ph, u32 clk_id,u8 index);

	int (*dividor_set)(const struct scmi_protocol_handle *ph, u32 clk_id,unsigned long val);
...
/**
 * struct scmi_channel_proto_ops - represents the various operations provided
 *	by SCMI Reset Protocol
 *
 * @read: get the count of reset domains provided by SCMI
 * @write: gets the name of a reset domain
 * @check: gets the reset latency for the specified reset domain
 */
struct scmi_channel_proto_ops {
	int (*read)(const struct scmi_protocol_handle *ph, u32 reg,u32 *val);
	int (*write)(const struct scmi_protocol_handle *ph, u32 reg,u32 val);
	ANDROID_KABI_RESERVE(1);
};
...
struct scmi_pinctrl_ops {
       int (*get_groups_count)(const struct scmi_protocol_handle *ph);
       int (*get_group_name)(const struct scmi_protocol_handle *ph, u32 selector,
                                                 const char **name);
       int (*get_group_pins)(const struct scmi_protocol_handle *ph, u32 selector,
                                                 const unsigned **pins, unsigned *nr_pins);
       int (*get_functions_count)(const struct scmi_protocol_handle *ph);
       int (*get_function_name)(const struct scmi_protocol_handle *ph, u32 selector,
                                                        const char **name);
       int (*get_function_groups)(const struct scmi_protocol_handle *ph, u32 selector,
                                                          u32 *nr_groups, const void **groups);
       int (*set_mux)(const struct scmi_protocol_handle *ph, u32 selector, u32 group);
       int (*get_pins)(const struct scmi_protocol_handle *ph, u32 *nr_pins,
                                       const void **pins);
       int (*get_config)(const struct scmi_protocol_handle *ph, u32 pin,
                                         u32 *config);
       int (*set_config)(const struct scmi_protocol_handle *ph, u32 pin,
                                         u32 config);
       int (*get_config_group)(const struct scmi_protocol_handle *ph, u32 group,
                                         u32 *config);
       int (*set_config_group)(const struct scmi_protocol_handle *ph, u32 group,
                                         u32 config);
};
