...
/**
 * union dma_heap_phy_addrs_info - info used by get_phy_addrs()
 *
 * Used for implementation of polymorphism on different heaps
 * for the same get_phy_addrs() API.
 * System heap uses sg table, while CMA heap uses page and pagecount.
 */
union dma_heap_phy_addrs_info {
	/* system heap */
	struct {
		struct sg_table *table_out;
	};
	/* cma heap */
	struct {
		void *pages;
 ...
 * @get_phy_addrs:  get the physical address of the allocated memory
 * @import_global_fd: import global fd
 * @config_secure_mem: configure secure memory
...
struct dma_heap_ops {
	...
	int (*get_phy_addrs)(struct dma_heap *heap,
			struct dma_buf *dmabuf,
			union dma_heap_phy_addrs_info *info);
	int (*get_global_fd)(struct dma_heap *heap,
			struct dma_buf *dmabuf,
			unsigned long *fd_ptr);
	struct dma_buf *(*import_global_fd)(struct dma_heap *heap,
		unsigned long global_fd,
		unsigned long fd_flags);
	int (*config_secure_mem)(struct dma_heap *heap, 
