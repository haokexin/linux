...
int cryptodev_akcipher_init(struct cipher_data *out, const char *alg_name,
				uint8_t *keyp, size_t keylen, __u32 kop)
{
	int ret;

	out->async.ak = crypto_alloc_akcipher(alg_name, 0, 0);
	if (unlikely(IS_ERR(out->async.ak))) {
		ddebug(1, "Failed to load akcipher %s", alg_name);
		return -EINVAL;
	}

	if(kop == COP_SET_KEY)
		ret = crypto_akcipher_set_priv_key(out->async.ak, keyp, keylen);
	else
		ret = crypto_akcipher_set_pub_key(out->async.ak, keyp, keylen);
	
	out->akcipher = 1;
	init_completion(&out->async.result.completion);

	out->async.akrequest = akcipher_request_alloc(out->async.ak, GFP_KERNEL);
	if (unlikely(!out->async.akrequest)) {
		derr(1, "error allocating async akcrypto request");
		ret = -ENOMEM;
		goto error;
	}

	akcipher_request_set_callback(out->async.akrequest,
	...
	return 0;
error:
	if (out->async.akrequest)
		akcipher_request_free(out->async.akrequest);
	if (out->async.ak)
		crypto_free_akcipher(out->async.ak);

	return ret;
}

int cryptodev_skcipher_init(struct cipher_data *out, const char *alg_name,
				uint8_t *keyp, size_t keylen, int stream, int aead)
{
	...
	if (aead == 0 ) {
		...
		if (unlikely(IS_ERR(out->async.s))) {
			ddebug(1, "Failed to load skcipher %s", alg_name);
				return -EINVAL;
		}
...
{
	if (cdata->init) {
		if (cdata->aead == 0 && cdata->akcipher == 0) {
			skcipher_request_free(cdata->async.request);
			crypto_free_skcipher(cdata->async.s);
		} else if (cdata->aead == 0 && cdata->akcipher == 1)
		{
			if (cdata->async.akrequest)
				akcipher_request_free(cdata->async.akrequest);
			if (cdata->async.ak)
				crypto_free_akcipher(cdata->async.ak);
		}
		else {
...
ssize_t cryptodev_cipher_sign(struct cipher_data *cdata,
		...
		size_t len)
{
	...
	if (cdata->aead == 0 && cdata->akcipher == 1)
	{
		akcipher_request_set_crypt(cdata->async.akrequest,(struct scatterlist *)src, dst, len, 2*len);
		ret = crypto_akcipher_sign(cdata->async.akrequest);
...
ssize_t cryptodev_cipher_verify(struct cipher_data *cdata,
		const struct scatterlist *src, struct scatterlist *dst,
		size_t len)
{
	int ret;
	...
	if (cdata->aead == 0 && cdata->akcipher == 1)
	{
		akcipher_request_set_crypt(cdata->async.akrequest,(struct scatterlist *)src, dst, len, 1);
		ret = crypto_akcipher_verify(cdata->async.akrequest);
...
ssize_t cryptodev_cipher_macgen(struct cipher_data *cdata,
		const struct scatterlist *src, struct scatterlist *dst,
		size_t len)
{
	int ret;
	...
	if (cdata->aead == 0 && cdata->akcipher == 0)
	{
		skcipher_request_set_crypt(cdata->async.request,(struct scatterlist *)src, dst, len, cdata->async.iv);
		ret = crypto_skcipher_macgen(cdata->async.request);
	}else {
		return -EPROTONOSUPPORT;
	}
...
ssize_t cryptodev_cipher_macver(struct cipher_data *cdata,
		const struct scatterlist *src, struct scatterlist *dst,
		size_t len)
{
	int ret;
	...
	{
		skcipher_request_set_crypt(cdata->async.request, (struct scatterlist *)src, dst, len, cdata->async.iv);
		ret = crypto_skcipher_macver(cdata->async.request);
	}else {
		return -EPROTONOSUPPORT;
	}
		...
		size_t len)
{
	...
	{
		akcipher_request_set_crypt(cdata->async.akrequest,(struct scatterlist *)src, dst, len, len);
		ret = crypto_akcipher_encrypt(cdata->async.akrequest);
		...
		size_t len)
{
	...
	{
		akcipher_request_set_crypt(cdata->async.akrequest,(struct scatterlist *)src, dst, len, len);
		ret = crypto_akcipher_decrypt(cdata->async.akrequest);
	}else {
		aead_request_set_crypt(cdata->async.arequest,
			(struct scatterlist *)src, dst,
			len, cdata->async.iv);
