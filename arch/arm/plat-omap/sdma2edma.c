				  ...
				  int dma_trigger, int src_or_dst_synch)
{
	...
	if ((enum sync_dimension)sync_mode > ABSYNC) {
		printk(KERN_ERR "SDMA2EDMA: Line:%d : Param \'sync_mode\'\
				   our of range\n", __LINE__);
		return;
	}
			      ...
			      int dst_ei, int dst_fi)
{
	if ((enum address_mode)dest_amode > FIFO) {
		printk(KERN_ERR "SDMA2EDMA: Line:%d : Param \'dest_amode\'\
				our of range\n", __LINE__);
		return;
	}
			     ...
			     int src_ei, int src_fi)
{
	if ((enum address_mode)src_amode > FIFO) {
		printk(KERN_ERR "SDMA2EDMA: Line:%d : Param \'src_amode\'\
				our of range\n", __LINE__);
		return;
	}
...
{
	printk(KERN_INFO "omap_set_dma_src_burst_mode: un-supported SDMA\
			  wrapper\n");
}
...
{
	printk(KERN_INFO "omap_set_dma_dest_burst_mode: un-supported SDMA\
			  wrapper\n");
}
