...
static int __init edma_probe(struct platform_device *pdev)
{
	...
	if (tc_errs_handled) {
		status = request_irq(TI816X_IRQ_TPTC0, dma_tc0err_handler, 0,
					"edma_tc0", &pdev->dev);
		if (status < 0) {
			dev_dbg(&pdev->dev, "request_irq %d failed --> %d\n",
				TI816X_IRQ_TPTC0, status);
			return status;
		}
		status = request_irq(TI816X_IRQ_TPTC1, dma_tc1err_handler, 0,
					"edma_tc1", &pdev->dev);
		if (status < 0) {
			dev_dbg(&pdev->dev, "request_irq %d --> %d\n",
				TI816X_IRQ_TPTC1, status);
			return status;
		}
		status = request_irq(TI816X_IRQ_TPTC2, dma_tc2err_handler, 0,
					"edma_tc2", &pdev->dev);
		if (status < 0) {
			dev_dbg(&pdev->dev, "request_irq %d failed --> %d\n",
				TI816X_IRQ_TPTC2, status);
			return status;
		}
		status = request_irq(TI816X_IRQ_TPTC3, dma_tc3err_handler, 0,
					"edma_tc3", &pdev->dev);
		if (status < 0) {
			dev_dbg(&pdev->dev, "request_irq %d --> %d\n",
				TI816X_IRQ_TPTC3, status);
			return status;
		}
	}
