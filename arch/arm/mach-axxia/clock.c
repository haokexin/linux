...
#define clk_register_clkdev(_clk, _conid, _devfmt, ...) \
	do { \
		...
		cl = clkdev_create(_clk, _conid, _devfmt, ## __VA_ARGS__); \
	} while (0)
...
axxia_init_clocks(int is_sim)
{
	...
	/* PL011 UARTs */
	clk_register_clkdev(clk[clk_per], NULL, "2010080000.serial");
	clk_register_clkdev(clk[clk_per], NULL, "2010081000.serial");
	clk_register_clkdev(clk[clk_per], NULL, "2010082000.serial");
	clk_register_clkdev(clk[clk_per], NULL, "2010083000.serial");
