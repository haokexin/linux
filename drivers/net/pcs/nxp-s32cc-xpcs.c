...
static void xpcs_prepare_link_state(struct s32cc_xpcs *xpcs,
				    const struct phylink_link_state *state)
{
	bool an_enabled = linkmode_test_bit(ETHTOOL_LINK_MODE_Autoneg_BIT,
					    state->advertising);
...
static int xpcs_set_1g_mode(struct s32cc_xpcs *xpcs,
			    const struct phylink_link_state *state)
{
	unsigned int val;
	int ret = 0;
	...
	/* These step-by-step instructions on switching the PHY to 1G speed
	 * are provided in the SerDes Subsystem Reference Manual,
	 * chapter 5.3 "Switching the PHY to 1G speed"
	 */
...
static int xpcs_set_2g5_mode(struct s32cc_xpcs *xpcs,
			     const struct phylink_link_state *state)
{
	unsigned int val;
	int ret = 0;
	...
	/* These step-by-step instructions on switching the PHY to 2.5G speed
	 * are provided in the SerDes Subsystem Reference Manual,
	 * chapter 5.4 "Switching the PHY to 2.5G speed"
	 */
