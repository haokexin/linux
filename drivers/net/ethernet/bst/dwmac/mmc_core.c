...
void dwmac_mmc_ctrl(void __iomem *mmcaddr, unsigned int mode)
{
	...
	pr_debug("bstgmac: MMC ctrl register (offset 0x%x): 0x%08x\n",
		 MMC_CNTRL, value);
}
 ...
 */
void dwmac_mmc_read(void __iomem *mmcaddr, struct bstgmac_counters *mmc)
{
	u32 value;

	value = readl(mmcaddr + MMC_CNTRL);
	value |= MMC_CNTRL_RESET_ON_READ;
	writel(value, mmcaddr + MMC_CNTRL);
...
const struct bstgmac_mmc_ops dwmac_mmc_ops = {
	.ctrl = dwmac_mmc_ctrl,
 ...
 * have to be incremented.
 */
static void dwxgmac_mmc_read(void __iomem *mmcaddr, struct bstgmac_counters *mmc)
{
...
const struct bstgmac_mmc_ops dwxgmac_mmc_ops = {
