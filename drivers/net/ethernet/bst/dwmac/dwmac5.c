...
static void dwmac5_log_error(struct net_device *ndev, u32 value, bool corr,
		...
		unsigned long field_offset, struct bstgmac_safety_stats *stats)
{
...
int dwmac5_safety_feat_dump(struct bstgmac_safety_stats *stats,
...
dwmac5_rxp_get_next_entry(struct bstgmac_tc_entry *entries, unsigned int count,
			  u32 curr_prio)
{
	struct bstgmac_tc_entry *entry;
	u32 min_prio = ~0x0;
...
int dwmac5_rxp_config(void __iomem *ioaddr, struct bstgmac_tc_entry *entries,
		      unsigned int count)
{
	...
	//pr_emerg("[%s]%d DisEnable",__func__,__LINE__);
	/* Disable RX Parser */
...
re_enable:
	...
	//pr_emerg("[%s]%d ReEnable",__func__,__LINE__);
	return ret;
}
...
void dwmac5_fpe_configure(void __iomem *ioaddr, u32 tx_mask, u32 num_rxq,
...
void dwmac5_fpe_get_config(void __iomem *ioaddr, struct ethtool_fpe *eth_fpe)
{
	u32 regval;

	regval = readl(ioaddr + MAC_FPE_CTRL_STS);
	eth_fpe->fpe = (regval & 0x1);

	regval = readl(ioaddr + MTL_FPE_CTRL_STS);
