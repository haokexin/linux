			...
			struct tc_cbs_qopt_offload *qopt)
{
	...
	if (mode_to_use == MTL_QUEUE_DCB && qopt->enable) {
		...
		ret = bstgmac_dma_qmode(priv, priv->ioaddr, queue, MTL_QUEUE_DCB);
		if (ret)
			return ret;
			   ...
			   struct tc_taprio_qopt_offload *qopt)
{
	...
	if (fpe) {
		priv->fpe_tx_queue_mask = 0x1;
		priv->fpe_min_frag_size = 0x0;
		if (priv->fpe_hs) {
...
static int tc_action_gate(struct bstgmac_priv *priv,
			   struct flow_action_entry *act)
{
	u32 size, wid = priv->dma_cap.estwid, dep = priv->tc_entries_max;
	struct plat_stmmacenet_data *plat = priv->plat;
	struct timespec64 time, current_time, qopt_time;
	ktime_t current_time_ns;
	int i, ret = 0;
	...
	if (act->gate.num_entries >= dep)
		return -ERANGE;
	if (!act->gate.basetime)
		return -ERANGE;
	if (!act->gate.cycletime)
		return -ERANGE;
	...
	for (i = 0; i < size; i++) {
		s64 delta_ns = act->gate.entries[i].interval;
		u32 gates = act->gate.entries[i].gate_state; //0/1

		if (delta_ns > GENMASK(wid, 0))
			return -ERANGE;

		priv->plat->est->gcl[i] = delta_ns | (gates << (wid+i));
	}
	...
	priv->ptp_clock_ops.gettime64(&priv->ptp_clock_ops, &current_time);
	current_time_ns = timespec64_to_ktime(current_time);
	time = bstgmac_calc_tas_basetime(act->gate.basetime, current_time_ns,
					 act->gate.cycletime);
	priv->plat->est->btr[0] = (u32)time.tv_nsec;
	priv->plat->est->btr[1] = (u32)time.tv_sec;
	qopt_time = ktime_to_timespec64(act->gate.basetime);
	priv->plat->est->btr_reserve[0] = (u32)qopt_time.tv_nsec;
	priv->plat->est->btr_reserve[1] = (u32)qopt_time.tv_sec;

	ctr = act->gate.cycletime;
	priv->plat->est->ctr[0] = do_div(ctr, NSEC_PER_SEC);
	priv->plat->est->ctr[1] = (u32)ctr;
	//priv->plat->est->ter = 10000000;
	ret = bstmac_sgf_configure(priv, priv->ioaddr, priv->plat->est,
				    priv->plat->clk_ptp_rate);
	mutex_unlock(&priv->plat->est->lock);
	if (ret) {
		netdev_err(priv->dev, "failed to configure SGF\n");
		goto disable;
	}

	netdev_info(priv->dev, "configured SGF\n");
...
disable:
	...
	mutex_lock(&priv->plat->est->lock);
	priv->plat->est->enable = false;
	bstmac_sgf_configure(priv, priv->ioaddr, priv->plat->est,
			      priv->plat->clk_ptp_rate);
	mutex_unlock(&priv->plat->est->lock);

	netdev_info(priv->dev, "disabled SGF\n");
				 ...
				 struct netlink_ext_ack *extack)
{
	...
	flow_action_for_each(i, act, action) {
		...
		case FLOW_ACTION_DROP:
			entry->action |= BSTMAC_FLOW_ACTION_DROP;
			return 0;
		case FLOW_ACTION_GATE:
			entry->action |= BSTMAC_FLOW_ACTION_GATE;
			return 0;
		default:
			break;
		}
...
static int tc_add_ip_flow(struct bstgmac_priv  *priv,
			  struct flow_cls_offload *cls,
			  struct bstgmac_flow_entry *entry)
{
	struct flow_rule *rule = flow_cls_offload_flow_rule(cls);
	struct flow_dissector *dissector = rule->match.dissector;
	bool inv = entry->action & BSTMAC_FLOW_ACTION_DROP;
	u32 hw_match;
	u16 addr_type = 0;
	int ret;
	struct flow_action_entry *act;
	...
	if (entry->action & BSTMAC_FLOW_ACTION_GATE) {
		action = &rule->action;
		act = &action->entries[0];

		return tc_action_gate(priv, act);
	}
	...
	if (addr_type == FLOW_DISSECTOR_KEY_IPV4_ADDRS) {
		struct flow_match_ipv4_addrs ip4_match;
		/* Nothing to do here */
		if (!dissector_uses_key(dissector, FLOW_DISSECTOR_KEY_IPV4_ADDRS))
			return -EINVAL;

		flow_rule_match_ipv4_addrs(rule, &ip4_match);
		hw_match = ntohl(ip4_match.key->src) & ntohl(ip4_match.mask->src);
		if (hw_match) {
			ret = bstgmac_config_l3_filter(priv, priv->hw, entry->idx,
						       true, false, true, inv, rule);
			if (ret)
				return ret;
		}

		hw_match = ntohl(ip4_match.key->dst) & ntohl(ip4_match.mask->dst);
		if (hw_match) {
			ret = bstgmac_config_l3_filter(priv, priv->hw, entry->idx,
						       true, false, false, inv, rule);
			if (ret)
				return ret;
		}
	}
	...
	if (addr_type == FLOW_DISSECTOR_KEY_IPV6_ADDRS) {
		struct flow_match_ipv6_addrs ip6_match;
		/* Nothing to do here */
		if (!dissector_uses_key(dissector, FLOW_DISSECTOR_KEY_IPV6_ADDRS))
			return -EINVAL;

		flow_rule_match_ipv6_addrs(rule, &ip6_match);
		if (!ipv6_addr_any(&ip6_match.key->src)) {
			ret = bstgmac_config_l3_filter(priv, priv->hw, entry->idx,
						       true, true, true, inv, rule);

			if (ret)
				return ret;
		} else {
			ret = bstgmac_config_l3_filter(priv, priv->hw, entry->idx,
						       true, true, false, inv, rule);
...
static int tc_setup_etf(struct bstgmac_priv *priv,
			struct tc_etf_qopt_offload *qopt)
{
	...
	if (qopt->queue >= priv->plat->tx_queues_to_use)
		return -EINVAL;
	if (!(priv->tx_queue[qopt->queue].tbs & BSTMAC_TBS_AVAIL))
		return -EINVAL;

	if (qopt->enable)
		priv->tx_queue[qopt->queue].tbs |= BSTMAC_TBS_EN;
	else
		priv->tx_queue[qopt->queue].tbs &= ~BSTMAC_TBS_EN;
