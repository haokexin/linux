				     ...
				     u32 prio, u32 queue)
{
	...
	tmp_q = (queue < 4) ? queue : (queue - 4);

	value = readl(ioaddr + base_register);

	value &= ~GMAC_RXQCTRL_PSRQX_MASK(tmp_q);
	value |= (prio << GMAC_RXQCTRL_PSRQX_SHIFT(tmp_q)) &
						GMAC_RXQCTRL_PSRQX_MASK(tmp_q);
				     ...
				     u32 prio, u32 queue)
{
	...
	value &= ~GMAC_TXQCTRL_PSTQX_MASK(tmp_q);
	value |= (prio << GMAC_TXQCTRL_PSTQX_SHIFT(tmp_q)) &
						GMAC_TXQCTRL_PSTQX_MASK(tmp_q);
				    ...
				    u8 packet, u32 queue)
{
	...
	if (packet == PACKET_AVCPQ) {
		...
		reg_val = readl(ioaddr + GMAC_RXQ_CTRL0);
		reg_val &= ~(0x3 << (queue << 1));
		reg_val |= (0x1 << (queue << 1));
		writel(reg_val, ioaddr + GMAC_RXQ_CTRL0);
				 ...
				 const unsigned char *addr, unsigned int reg_n)
{
	...
	bstgmac_dwmac4_set_mac_addr(ioaddr, addr, GMAC_ADDR_HIGH(reg_n),
				    GMAC_ADDR_LOW(reg_n));
}
				 ...
				 unsigned char *addr, unsigned int reg_n)
{
	...
	bstgmac_dwmac4_get_mac_addr(ioaddr, addr, GMAC_ADDR_HIGH(reg_n),
				    GMAC_ADDR_LOW(reg_n));
}
				      ...
				      __be16 proto, u16 vid)
{
	...
	if (hw->promisc) {
		/* fix the issue that eth vlan interface can't create in promisc mode */
		return 0;
...
static void dwmac4_mac_txq_flush(struct mac_device_info *hw, u32 txq_cnt, bool enable)
{
	void __iomem *ioaddr = hw->pcsr;
	...
	for (queue = 0; queue < txq_cnt; queue++) {
		value = readl(ioaddr + GMAC_MTL_TXQ_OPMODE(queue));
		if (enable)
			value |= GMAC_MTL_TXQ_OPMODE_FTQ;
		else
			value &= ~(GMAC_MTL_TXQ_OPMODE_FTQ);

		writel(value, ioaddr + GMAC_MTL_TXQ_OPMODE(queue));
	}
	...
	while (1) {
		...
		for (queue = 0; queue < txq_cnt; queue++) {
			value = readl(ioaddr + GMAC_MTL_TXQ_OPMODE(queue));
			if (!(value & GMAC_MTL_TXQ_OPMODE_FTQ))
				clear_flag++;
			else
				clear_flag = 0;
		}
			      ...
			      struct net_device *dev)
{
	...
	//pr_emerg("[%s]%d:Fix Filter RecvAl",__func__,__LINE__);
	//value |= 1 << 31;
	//pr_emerg("[%s]%d:Set GMAC_PACKET_FILTER :%x",__func__,__LINE__,value);
...
static void dwmac4_phystatus(void __iomem *ioaddr, struct bstgmac_extra_stats *x)
{
	u32 status;
...
static int bst_dwmac_lpi_irq_status(struct mac_device_info *hw,
...
static void dwmac4_debug(void __iomem *ioaddr, struct bstgmac_extra_stats *x,
				   ...
				   struct flow_rule *rule)
{
	...
	if (en) {
		if (ipv6) {
			...
			if (sa) {
				writel(ntohl(ipv6_match.key->src.in6_u.u6_addr32[3]),
				       ioaddr + GMAC_L3_ADDR0(filter_no));
				writel(ntohl(ipv6_match.key->src.in6_u.u6_addr32[2]),
				       ioaddr + GMAC_L3_ADDR1(filter_no));
				writel(ntohl(ipv6_match.key->src.in6_u.u6_addr32[1]),
				       ioaddr + GMAC_L3_ADDR2(filter_no));
				writel(ntohl(ipv6_match.key->src.in6_u.u6_addr32[0]),
				       ioaddr + GMAC_L3_ADDR3(filter_no));
			} else {
				writel(ntohl(ipv6_match.key->dst.in6_u.u6_addr32[3]),
				       ioaddr + GMAC_L3_ADDR0(filter_no));
				writel(ntohl(ipv6_match.key->dst.in6_u.u6_addr32[2]),
				       ioaddr + GMAC_L3_ADDR1(filter_no));
				writel(ntohl(ipv6_match.key->dst.in6_u.u6_addr32[1]),
				       ioaddr + GMAC_L3_ADDR2(filter_no));
				writel(ntohl(ipv6_match.key->dst.in6_u.u6_addr32[0]),
				       ioaddr + GMAC_L3_ADDR3(filter_no));
			}
		} else {
			...
			if (sa) {
				...
				writel(src_match, ioaddr + GMAC_L3_ADDR0(filter_no));
			} else {
				dst_match =
					ntohl(ipv4_match.key->dst) & ntohl(ipv4_match.mask->dst);
				writel(dst_match, ioaddr + GMAC_L3_ADDR1(filter_no));
			}
		}
	} else {
		writel(0, ioaddr + GMAC_L3L4_CTRL(filter_no));
		writel(0, ioaddr + GMAC_L3_ADDR0(filter_no));
		writel(0, ioaddr + GMAC_L3_ADDR1(filter_no));
		writel(0, ioaddr + GMAC_L3_ADDR2(filter_no));
		writel(0, ioaddr + GMAC_L3_ADDR3(filter_no));
	}
...
const struct bstgmac_ops dwmac510_ops = {
	.core_init = dwmac4_core_init,
	.set_mac = bstgmac_dwmac4_set_mac,
	...
	.host_lpi_irq_status = bst_dwmac_lpi_irq_status,
	...
	.mac_txq_flush = dwmac4_mac_txq_flush,
