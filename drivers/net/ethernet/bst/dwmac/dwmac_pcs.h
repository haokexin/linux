 ...
 * Copyright (C) 2016 STMicroelectronics Ltd
 */

#ifndef __BSTGMAC_PCS_H__
#define __BSTGMAC_PCS_H__
				 ...
				 struct bstgmac_extra_stats *x)
{
	...
	if (intr_status & PCS_ANE_IRQ) {
		...
		if (val & GMAC_AN_STATUS_ANC)
			pr_info("bstgmac_pcs: ANE process completed\n");
	}

	if (intr_status & PCS_LINK_IRQ) {
		x->irq_pcs_link_n++;
		if (val & GMAC_AN_STATUS_LS)
			pr_info("bstgmac_pcs: Link Up\n");
		else
			pr_info("bstgmac_pcs: Link Down\n");
	}
}
