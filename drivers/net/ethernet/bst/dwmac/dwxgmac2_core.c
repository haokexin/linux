			       ...
			       struct net_device *dev)
{
	...
	if (priv->fpe_hs) {
		if (XGMAC_HWFEAT_FPESEL & readl(ioaddr + XGMAC_HW_FEATURE3))
			value |= XGMAC_FPEIE;
	}
...
static int dwxgmac2_lpi_irq_status(struct mac_device_info *hw,
				    struct bstgmac_extra_stats *x)
{
	void __iomem *ioaddr = hw->pcsr;
	int ret = 0;

	/* Clear LPI interrupt by reading XGMAC_LPI_CTRL_STATUS */
	u32 status = readl(ioaddr + XGMAC_LPI_CTRL_STATUS);

	if (status & XGMAC_LPI_CTRL_STATUS_TLPIEN) {
		ret |= CORE_IRQ_TX_PATH_IN_LPI_MODE;
		x->irq_tx_path_in_lpi_mode_n++;
	}

	if (status & XGMAC_LPI_CTRL_STATUS_TLPIEX) {
		ret |= CORE_IRQ_TX_PATH_EXIT_LPI_MODE;
		x->irq_tx_path_exit_lpi_mode_n++;
	}

	if (status & XGMAC_LPI_CTRL_STATUS_RLPIEN) {
		ret |= CORE_IRQ_RX_PATH_IN_LPI_MODE;
		x->irq_rx_path_in_lpi_mode_n++;
	}

	if (status & XGMAC_LPI_CTRL_STATUS_RLPIEX) {
		ret |= CORE_IRQ_RX_PATH_EXIT_LPI_MODE;
		x->irq_rx_path_exit_lpi_mode_n++;
	}
...
static int dwxgmac2_wdata_ucerr_irq_status(struct mac_device_info *hw,
				    struct bstgmac_extra_stats *x)
{
	void __iomem *ioaddr = hw->pcsr;
	int ret = 0;
	void __iomem *sys_ctrl = NULL;
	u32 reg_value = 0;

	u32 status = readl(ioaddr + XGMAC_DMA_AS_SFTY_ECC_INTR_STS);
	...
	/*clear interrupt*/
    sys_ctrl = ioremap(0x30000000, 0x1000);
	if (!sys_ctrl)
		return (-EPROBE_DEFER);

    reg_value = readl(sys_ctrl + 0x60);
	reg_value &= ~(0x1 << 4); //clear interrupt
	writel(reg_value, sys_ctrl + 0x60);
	iounmap(sys_ctrl);

	return ret;
}

static int dwxgmac2_paddr_parity_irq_status(struct mac_device_info *hw,
				    struct bstgmac_extra_stats *x)
{
	//void __iomem *ioaddr = hw->pcsr;
	int ret = 0;
	void __iomem *lb_cpu_perip_csr = NULL;
	u32 reg_value = 0;

	lb_cpu_perip_csr = ioremap(0x32060000, 0x1000);
	if (!lb_cpu_perip_csr)
		return (-EPROBE_DEFER);

    reg_value = readl(lb_cpu_perip_csr + 0x104);
	reg_value &= ~(0x1 << 1); //clear interrupt
	writel(reg_value, lb_cpu_perip_csr + 0x104);
	iounmap(lb_cpu_perip_csr);
...
static void dwxgmac3_write_single_vlan(struct net_device *dev, u16 vid)
{
...
static int dwxgmac3_write_vlan_filter(struct net_device *dev,
...
static int dwxgmac3_add_hw_vlan_rx_fltr(struct net_device *dev,
				      struct mac_device_info *hw,
				      __be16 proto, u16 vid)
{
	...
	if (hw->num_vlan == 1) {
		...
		if (hw->vlan_filter[0] & XGMAC_VLAN_TAG_VID) {
			netdev_err(dev, "Only single VLAN ID supported\n");
			return -EPERM;
		}

		hw->vlan_filter[0] = vid;
		dwxgmac3_write_single_vlan(dev, vid);
	...
	ret = dwxgmac3_write_vlan_filter(dev, hw, index, val);
...
static int dwxgmac3_del_hw_vlan_rx_fltr(struct net_device *dev,
				      struct mac_device_info *hw,
				      __be16 proto, u16 vid)
{
	int i, ret = 0;
	...
	/* Single Rx VLAN Filter */
	if (hw->num_vlan == 1) {
		if ((hw->vlan_filter[0] & XGMAC_VLAN_TAG_VID) == vid) {
			hw->vlan_filter[0] = 0;
			dwxgmac3_write_single_vlan(dev, 0);
		}
		return 0;
	}
	...
	for (i = 0; i < hw->num_vlan; i++) {
		if ((hw->vlan_filter[i] & XGMAC_VLAN_TAG_DATA_VID) == vid) {
			ret = dwxgmac3_write_vlan_filter(dev, hw, i, 0);
...
static void dwxgmac3_vlan_promisc_enable(struct net_device *dev,
				       struct mac_device_info *hw)
{
	void __iomem *ioaddr = hw->pcsr;
	u32 value;
	...
	for (i = 0; i < hw->num_vlan; i++) {
		if (hw->vlan_filter[i] & XGMAC_VLAN_TAG_DATA_VEN) {
			...
			dwxgmac3_write_vlan_filter(dev, hw, i, val);
		}
	}
...
static void dwxgmac3_restore_hw_vlan_rx_fltr(struct net_device *dev,
					   struct mac_device_info *hw)
{
	void __iomem *ioaddr = hw->pcsr;
	u32 value;
	...
	/* Single Rx VLAN Filter */
	if (hw->num_vlan == 1) {
		dwxgmac3_write_single_vlan(dev, hw->vlan_filter[0]);
		return;
	}
...
{
	void __iomem *ioaddr = (void __iomem *)dev->base_addr;
	u32 value = readl(ioaddr + XGMAC_PACKET_FILTER), rxq_ctrl;
	int mcbitslog2 = hw->mcast_bits_log2;
	u32 mc_filter[8];
	int i;

	value &= ~(XGMAC_FILTER_RA |XGMAC_FILTER_PR | XGMAC_FILTER_HMC | XGMAC_FILTER_PM 
		| XGMAC_FILTER_PCF);
	...
	if (dev->flags & IFF_PROMISC) {
		...
		if (hw->vlan_fail_q_en) {
			rxq_ctrl = readl(ioaddr + XGMAC_RXQ_CTRL4);
			rxq_ctrl &= ~XGMAC_RXQCTRL_VFFQ_MASK;
			rxq_ctrl |= XGMAC_RXQCTRL_VFFQE |
				 (hw->vlan_fail_q << XGMAC_RXQCTRL_VFFQ_SHIFT);
			writel(rxq_ctrl, ioaddr + XGMAC_RXQ_CTRL4);
	...
	if (dev->flags & IFF_PROMISC && !hw->vlan_fail_q_en) {
		if (!hw->promisc) {
			hw->promisc = 1;
			dwxgmac3_vlan_promisc_enable(dev, hw);
		}
	} else {
		if (hw->promisc) {
			hw->promisc = 0;
			dwxgmac3_restore_hw_vlan_rx_fltr(dev, hw);
		}
	}
}
...
static int dwxgmac2_rss_configure(struct mac_device_info *hw,
				  struct bstgmac_rss *cfg, u32 num_rxq)
{
	void __iomem *ioaddr = hw->pcsr;
	u32 value, *key;
	int i, ret;
...
#define STAT_OFF(field)		offsetof(struct bstgmac_safety_stats, field)
...
static const struct dwxgmac3_error_desc dwxgmac3_mtl_errors[32]= {
	...
	{ true, "GCES", "MTL EST Memory Error" },
	{ true, "GAMS", "MTL EST Memory Address Mismatch Error" },
	{ true, "GUES", "MTL EST Memory Error" },
	{ false, "UNKNOWN", "Unknown Error" }, /* 11 */
	{ true, "RPCES", "MTL RX Parser Memory Error" },
	{ true, "RPAMS", "MTL RX Parser Memory Address Mismatch Error" },
	{ true, "RPUES", "MTL RX Parser Memory Error" },
	{ false, "UNKNOWN", "Unknown Error" }, /* 15 */
	{ true, "SCES", "MTL SGF GCL Memory Error" }, /* 16 */
	{ true, "SAMS", "MTL SGF GCL Memory Address Mismatch Error" }, /* 17 */
	{ true, "SUES", "MTL SGF GCL Memory Error" }, /* 18 */
	{ false, "UNKNOWN", "Unknown Error" }, /* 19 */
	{ false, "UNKNOWN", "Unknown Error" }, /* 20 */
					   ...
					   struct bstgmac_safety_stats *stats)
{
	...
	corr = ((mtl & XGMAC_MEUIS) || (dma & XGMAC_MSUIS)) ? false : true;
	if (err) {
		dwxgmac3_handle_mtl_err(ndev, ioaddr, corr, stats);
		ret |= !corr;
	}
	err = dma & (XGMAC_DEUIS | XGMAC_DECIS | XGMAC_MCSIS);
	corr = (dma & XGMAC_DECIS) ? true : false;
	if (err) {
...
static int dwxgmac3_safety_feat_dump(struct bstgmac_safety_stats *stats,
...
static struct bstgmac_tc_entry *
dwxgmac3_rxp_get_next_entry(struct bstgmac_tc_entry *entries,
...
static int dwxgmac3_rxp_config(void __iomem *ioaddr,
			       struct bstgmac_tc_entry *entries,
			       unsigned int count)
{
	struct bstgmac_tc_entry *entry, *frag;
	int i, ret, nve = 0;
	u32 curr_prio = 0;
	u32 old_val, val;
	static u8 gid = 0;
				    ...
				    u32 sub_second_inc, u32 systime_flags)
{
	...
	val |= XGMAC_PPSCMDx(index, XGMAC_PPSCMD_SINGLE_START);
	...
	/* flex pps config */
	val = readl(ioaddr + XGMAC_INT_EN);
	val |= XGMAC_TSIE;
	writel(val, ioaddr + XGMAC_INT_EN);

	val = readl(ioaddr + XGMAC_TIMESTAMP_CTRL);
	val |= XGMAC_TSTRIG;
				     ...
				     struct flow_rule *rule)
{
	...
	if (ipv6) {
		...
		if (sa) {
			value |= XGMAC_L3SAM0;
			if (inv & BSTMAC_FLOW_ACTION_DROP)
				value |= XGMAC_L3SAIM0;
		} else {
			value |= XGMAC_L3DAM0;
			if (inv & BSTMAC_FLOW_ACTION_DROP)
		...
		if (ipv6) {
			...
			if (sa) {
				dwxgmac2_filter_write(hw, filter_no, XGMAC_L3_ADDR0,
					ntohl(ipv6_match.key->src.in6_u.u6_addr32[3]));
				dwxgmac2_filter_write(hw, filter_no, XGMAC_L3_ADDR1,
					ntohl(ipv6_match.key->src.in6_u.u6_addr32[2]));
				dwxgmac2_filter_write(hw, filter_no, XGMAC_L3_ADDR2,
					ntohl(ipv6_match.key->src.in6_u.u6_addr32[1]));
				dwxgmac2_filter_write(hw, filter_no, XGMAC_L3_ADDR3,
					ntohl(ipv6_match.key->src.in6_u.u6_addr32[0]));
			} else {
				dwxgmac2_filter_write(hw, filter_no, XGMAC_L3_ADDR0,
					ntohl(ipv6_match.key->dst.in6_u.u6_addr32[3]));
				dwxgmac2_filter_write(hw, filter_no, XGMAC_L3_ADDR1,
					ntohl(ipv6_match.key->dst.in6_u.u6_addr32[2]));
				dwxgmac2_filter_write(hw, filter_no, XGMAC_L3_ADDR2,
					ntohl(ipv6_match.key->dst.in6_u.u6_addr32[1]));
				dwxgmac2_filter_write(hw, filter_no, XGMAC_L3_ADDR3,
					ntohl(ipv6_match.key->dst.in6_u.u6_addr32[0]));
			}
		} else {
			flow_rule_match_ipv4_addrs(rule, &ipv4_match);
			if (sa) {
				src_match =
					ntohl(ipv4_match.key->src) & ntohl(ipv4_match.mask->src);
				dwxgmac2_filter_write(hw, filter_no, XGMAC_L3_ADDR0, src_match);
			} else {
				dst_match =
					ntohl(ipv4_match.key->dst) & ntohl(ipv4_match.mask->dst);
				dwxgmac2_filter_write(hw, filter_no, XGMAC_L3_ADDR1, dst_match);
			}
		}
	} else {
		dwxgmac2_filter_write(hw, filter_no, XGMAC_L3L4_CTRL, 0);
		dwxgmac2_filter_write(hw, filter_no, XGMAC_L3_ADDR0, 0);
		dwxgmac2_filter_write(hw, filter_no, XGMAC_L3_ADDR1, 0);
		dwxgmac2_filter_write(hw, filter_no, XGMAC_L3_ADDR2, 0);
		dwxgmac2_filter_write(hw, filter_no, XGMAC_L3_ADDR3, 0);
	}
	return 0;
}
...
#endif
static int dwxgmac3_gcl_write(void __iomem *ioaddr, u32 reg, u32 val, bool gcl)
{
	u32 ctrl;

	writel(val, ioaddr + XGMAC_MTL_EST_GCL_DATA);

	ctrl = readl(ioaddr + XGMAC_MTL_EST_GCL_CONTROL);
	ctrl &= (~(7 << XGMAC_ADDR_SHIFT));
...
static int dwxgmac3_gcl_switch_to_est(void __iomem *ioaddr)
{
	u32 ctrl;
...
static int dwxgmac3_gcl_switch_to_sgf(void __iomem *ioaddr)
{
	u32 ctrl;
...
static int dwxgmac3_sgf_configure(void __iomem *ioaddr, struct stmmac_est *cfg,
				  unsigned int ptp_rate)
{
	int i, ret = 0x0;
	u32 ctrl;
	
	ret = dwxgmac3_gcl_switch_to_sgf(ioaddr);
	ret |= dwxgmac3_gcl_write(ioaddr, XGMAC_BTR_LOW, cfg->btr[0], false);
	ret |= dwxgmac3_gcl_write(ioaddr, XGMAC_BTR_HIGH, cfg->btr[1], false);
	ret |= dwxgmac3_gcl_write(ioaddr, XGMAC_TER, cfg->ter, false);
	ret |= dwxgmac3_gcl_write(ioaddr, XGMAC_LLR, cfg->gcl_size, false);
	ret |= dwxgmac3_gcl_write(ioaddr, XGMAC_CTR_LOW, cfg->ctr[0], false);
	ret |= dwxgmac3_gcl_write(ioaddr, XGMAC_CTR_HIGH, cfg->ctr[1], false);
	if (ret)
		return ret;

	for (i = 0; i < cfg->gcl_size; i++) {
		ret = dwxgmac3_gcl_write(ioaddr, i, cfg->gcl[i], true);
		if (ret)
			return ret;
	}

	ctrl = readl(ioaddr + XGMAC_MTL_SGF_CONTROL);
	ctrl &= ~XGMAC_PTOV;
	ctrl |= ((1000000000 / ptp_rate) * 9) << XGMAC_PTOV_SHIFT;
	if (cfg->enable)
		ctrl |= XGMAC_ESGF | XGMAC_SSWL;
	else
		ctrl &= ~XGMAC_ESGF;

	writel(ctrl, ioaddr + XGMAC_MTL_SGF_CONTROL);
	return 0;
}
				  ...
				  unsigned int ptp_rate)
{
	...
	ret = dwxgmac3_gcl_switch_to_est(ioaddr);
	ret |= dwxgmac3_gcl_write(ioaddr, XGMAC_BTR_LOW, cfg->btr[0], false);
	ret |= dwxgmac3_gcl_write(ioaddr, XGMAC_BTR_HIGH, cfg->btr[1], false);
	ret |= dwxgmac3_gcl_write(ioaddr, XGMAC_TER, cfg->ter, false);
	ret |= dwxgmac3_gcl_write(ioaddr, XGMAC_LLR, cfg->gcl_size, false);
...
static void dwxgmac3_fpe_configure(void __iomem *ioaddr, u32 tx_mask, u32 num_rxq,
			  u32 min_fsize, bool enable)
{
	u32 value;
...
static void dwxgmac3_fpe_get_config(void __iomem *ioaddr, struct ethtool_fpe *eth_fpe)
{
	u32 regval;

	regval = readl(ioaddr + XGMAC_FPE_CTRL_STS);
	eth_fpe->fpe = (regval & 0x1);

	regval = readl(ioaddr + XGMAC_MTL_FPE_CTRL_STS);
	eth_fpe->tx_queue_mask = ((regval & PEC_MASK) >> 8);
	eth_fpe->min_frag_size = (regval & AFSZ_MASK);
}
...
void dwxgmac3_fpe_send_mpacket(void __iomem *ioaddr, enum stmmac_mpacket_type type)
{
	u32 value;
				    ...
				    u8 packet, u32 queue)
{
	...
	static const struct bstgmac_rx_routing route_possibilities[] = {
	...
	if (packet == PACKET_AVCPQ) {
		...
		//enable ptp over eth packet
		val = readl(ioaddr + XGMAC_TIMESTAMP_CTRL);
		val |= (XGMAC_TSIPENA) | (XGMAC_TSVER2ENA);
		writel(val, ioaddr + XGMAC_TIMESTAMP_CTRL);
...
static void dwxgmac3_mac_txq_flush(struct mac_device_info *hw, u32 txq_cnt, bool enable)
{
	void __iomem *ioaddr = hw->pcsr;
	u32 queue = 0, value = 0, cnt = 0, clear_flag = 0;

	/* MAC Transmit Queue */
	for (queue = 0; queue < txq_cnt; queue++) {
...
const struct bstgmac_ops dwxgmac210_ops = {
	...
	.host_lpi_irq_status = dwxgmac2_lpi_irq_status,
	.wdata_ucerr_irq_status = dwxgmac2_wdata_ucerr_irq_status,
	.paddr_parity_irq_status = dwxgmac2_paddr_parity_irq_status,
	...
	.add_hw_vlan_rx_fltr = dwxgmac3_add_hw_vlan_rx_fltr,
	.del_hw_vlan_rx_fltr = dwxgmac3_del_hw_vlan_rx_fltr,
	...
	.fpe_get_config = dwxgmac3_fpe_get_config,
	.fpe_send_mpacket = dwxgmac3_fpe_send_mpacket,
	.fpe_irq_status = dwxgmac3_fpe_irq_status,
	.mac_txq_flush = dwxgmac3_mac_txq_flush,
	.sgf_configure = dwxgmac3_sgf_configure,
};

static u32 dwxgmac3_get_num_vlan(void __iomem *ioaddr)
{
	u32 val, num_vlan;
...
int dwxgmac2_setup(struct bstgmac_priv *priv)
{
	...
	mac->num_vlan = dwxgmac3_get_num_vlan(priv->ioaddr);
