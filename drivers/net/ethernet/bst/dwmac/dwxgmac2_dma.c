...
#if 1
//printk("[csl]:: %s,%d, readl(ioaddr + XGMAC_MTL_DPP_CONTROL) | (0x1 << 2): 0x%llx\n",__func__,__LINE__,readl(ioaddr + XGMAC_MTL_DPP_CONTROL) | (0x1 << 2));
			mdelay(100);
			writel(readl(ioaddr + XGMAC_MTL_DPP_CONTROL) | (0x1 << 2), ioaddr + XGMAC_MTL_DPP_CONTROL); //Data Parity protection（[baremental]: not config）
#endif
#if 1
			mdelay(10);
			writel(0x0, ioaddr + XGMAC_MTL_ECC_CONTROL); //enable ecc check
			mdelay(10);
			writel(0x1111, ioaddr + XGMAC_MTL_ECC_INT_ENABLE); //tx/rx/rx parser memory Correctable Error Interrupt Enable
			mdelay(10);
#endif
			      ...
			      struct stmmac_dma_cfg *dma_cfg, int atds)
{
	...
	writel(value, ioaddr + XGMAC_AXI_TX_AR_ACE_CTRL);

	value = readl(ioaddr + XGMAC_AXI_RX_AW_ACE_CTRL);
	value &= (~(XGMAC_RDC_MASK | XGMAC_RHC_MASK | XGMAC_RPC_MASK | XGMAC_RDWC_MASK));
	value |= (3 << XGMAC_RDC_SHIFT) | (3 << XGMAC_RHC_SHIFT) | (3 << XGMAC_RPC_SHIFT) | (3 << XGMAC_RDWC_SHIFT);
	writel(value, ioaddr + XGMAC_AXI_RX_AW_ACE_CTRL);

	value = readl(ioaddr + XGMAC_AXI_TXRX_AWAR_ACE_CTRL);
...
static void dwxgmac3_enable_rxo_int(void __iomem *ioaddr, bool en, u32 channel)
{
	u32 value;
				 ...
				 u32 channel, int fifosz, u8 qmode)
{
	...
	writel(value/*0x1f000b*/, ioaddr + XGMAC_MTL_RXQ_OPMODE(channel));

	/* Enable MTL RX overflow */
	//dwxgmac3_enable_rxo_int(ioaddr, 1, channel);
}
...
static void dwxgmac2_enable_dma_irq_bits(void __iomem *ioaddr, u32 chan, u32 bits)
{
    u32 value;
...
static void dwxgmac2_disable_dma_irq_bits(void __iomem *ioaddr, u32 chan, u32 bits)
{
    u32 value;
...
int dwxgmac2_dma_ri_interrupt(void __iomem *ioaddr,
			     struct bstgmac_extra_stats *x, u32 chan)
{
	...
	if (unlikely(intr_status & XGMAC_RBU)) {
		 ...
		 *  XGMAC_RBU
		 */
		writel(XGMAC_RBU | XGMAC_AIS, ioaddr + XGMAC_DMA_CH_STATUS(chan));
	}
	...
	if (likely(intr_status & XGMAC_RI)) {
		 ...
		 *  XGMAC_RI
		 */
		writel(XGMAC_RI | XGMAC_NIS, ioaddr + XGMAC_DMA_CH_STATUS(chan));
	}
	dma_wmb();
	return ret;
}

int dwxgmac2_dma_ti_interrupt(void __iomem *ioaddr,
			     struct bstgmac_extra_stats *x, u32 chan)
{
    int ret = 0;
	u32 intr_status = readl(ioaddr + XGMAC_DMA_CH_STATUS(chan));
	...
	if (unlikely(intr_status & XGMAC_TBU)) {
		 ...
		 * DMA_CHAN_STATUS_TBU
		 */
		writel(XGMAC_TBU | XGMAC_AIS, ioaddr + XGMAC_DMA_CH_STATUS(chan));
	}
#endif
	...
	if (likely(intr_status & XGMAC_TI)) {
		 ...
		 * DMA_CHAN_STATUS_TI
		 */
		writel(XGMAC_TI | XGMAC_NIS, ioaddr + XGMAC_DMA_CH_STATUS(chan));
	}
	if (unlikely(intr_status & XGMAC_TPS)) {
			x->tx_process_stopped_irq++;
			ret |= tx_hard_error;
			writel(XGMAC_TPS | XGMAC_AIS, ioaddr + XGMAC_DMA_CH_STATUS(chan));
	}
	if (unlikely(intr_status & XGMAC_FBE)) {
			x->fatal_bus_error_irq++;
			ret |= tx_hard_error;
			writel(XGMAC_FBE | XGMAC_AIS, ioaddr + XGMAC_DMA_CH_STATUS(chan));
	}
	return ret;
}
...
static void dwxgmac3_mac_rxq_flush(void __iomem *ioaddr, u32 rx_channel_cnt, bool enable)
{
	u32 value = 0, chan = 0;

	/* MAC Receive Queue */
	for (chan = 0; chan < rx_channel_cnt; chan++) {
		value = readl(ioaddr + XGMAC_DMA_CH_RX_CONTROL(chan));
		if (enable)
			value |= XGMAC_RPF;
		else
...
const struct bstgmac_dma_ops dwxgmac210_dma_ops = {
	...
	.enable_dma_irq_bits = dwxgmac2_enable_dma_irq_bits,
	.disable_dma_irq_bits = dwxgmac2_disable_dma_irq_bits,
	.start_tx = dwxgmac2_dma_start_tx,
	.stop_tx = dwxgmac2_dma_stop_tx,
	.start_rx = dwxgmac2_dma_start_rx,
	.stop_rx = dwxgmac2_dma_stop_rx,
	.dma_interrupt = dwxgmac2_dma_interrupt,
	.dma_ri_interrupt = dwxgmac2_dma_ri_interrupt/*dwxgmac2_dma_interrupt*/,
	.dma_ti_interrupt = dwxgmac2_dma_ti_interrupt/*dwxgmac2_dma_interrupt*/,
	...
	.mac_rxq_flush = dwxgmac3_mac_rxq_flush,
	.enable_rxo_int = dwxgmac3_enable_rxo_int,
};
