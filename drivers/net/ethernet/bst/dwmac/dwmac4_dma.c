...
static void dwmac4_mac_rxq_flush(void __iomem *ioaddr, u32 rx_channel_cnt, bool enable)
{
	u32 value = 0, chan = 0;
			    ...
			    struct stmmac_dma_cfg *dma_cfg, int atds)
{
	...
	value = (value & ~(3 << DMA_INT_MODE_SHIFT)) | ((dma_cfg->dma_int_mode & 0x3) << DMA_INT_MODE_SHIFT);
				       ...
				       u32 channel, int fifosz, u8 qmode)
{
	...
	//pr_emerg("[%s]%d:mtl_rx_op:%08x",__func__,__LINE__,mtl_rx_op);
	...
	mtl_rx_op |= 1 << 4;
	//pr_emerg("[%s]%d:Fix forwad all error pkts.",__func__,__LINE__);
	//pr_emerg("[%s]%d:mtl_rx_op:%08x fifosz:%d qmode:%d",__func__,__LINE__,mtl_rx_op,fifosz,qmode);
				  ...
				  struct dma_features *dma_cap)
{
	...
	//pr_emerg("[%s]%d. Only For test,Close Ecc&Parity function temporary.because maybe config the SOC NOC&DDRC parity Func.",__func__,__LINE__);
...
static void dwmac4_qmode(void __iomem *ioaddr, u32 channel, u8 qmode)
{
	...
	if (qmode != MTL_QUEUE_AVB) {
		mtl_tx_op |= MTL_OP_MODE_TXQEN;
		/* disable AV algorithm */
		value = readl(ioaddr + MTL_ETSX_CTRL_BASE_ADDR(channel));
		value &= ~MTL_ETS_CTRL_AVALG;
		value &= ~MTL_ETS_CTRL_CC;
		writel(value, ioaddr + MTL_ETSX_CTRL_BASE_ADDR(channel));
	} else
		mtl_tx_op |= MTL_OP_MODE_TXQEN_AV;
...
const struct bstgmac_dma_ops dwmac4_dma_ops = {
	...
	.mac_rxq_flush = dwmac4_mac_rxq_flush,
};

const struct bstgmac_dma_ops dwmac410_dma_ops = {
	.reset = dwmac4_dma_reset,
	.init = dwmac4_dma_init,
	...
	.disable_dma_irq = dwmac4_disable_dma_irq,
	.enable_dma_irq_bits = dwmac4_enable_dma_irq_bits,
	.disable_dma_irq_bits = dwmac4_disable_dma_irq_bits,
	.start_tx = dwmac4_dma_start_tx,
	.stop_tx = dwmac4_dma_stop_tx,
	.start_rx = dwmac4_dma_start_rx,
	.stop_rx = dwmac4_dma_stop_rx,
	.dma_interrupt = dwmac4_dma_interrupt,
	.dma_ri_interrupt = bstgmac_dma_ri_interrupt,
	.dma_ti_interrupt = bstgmac_dma_ti_interrupt,
	.get_hw_feature = dwmac4_get_hw_feature,
	.rx_watchdog = dwmac4_rx_watchdog,
	.set_rx_ring_len = dwmac4_set_rx_ring_len,
	.set_tx_ring_len = dwmac4_set_tx_ring_len,
