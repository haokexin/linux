...
static int jumbo_frm(void *p, struct sk_buff *skb, int csum)
{
	...
	bstgmac_prepare_tx_desc(priv, desc, 1, bmax, csum, BSTGMAC_CHAIN_MODE,
				0, false, skb->len);
	...
	while (len != 0) {
		...
		if (len > bmax) {
			...
			tx_q->tx_skbuff_dma[entry].buf = des2;
			tx_q->tx_skbuff_dma[entry].len = bmax;
			bstgmac_prepare_tx_desc(priv, desc, 0, bmax, csum,
						BSTGMAC_CHAIN_MODE, 1, false,
						skb->len);
			len -= bmax;
			i++;
		} else {
...
{
	struct bstgmac_rx_queue *rx_q = (struct bstgmac_rx_queue *)priv_ptr;
...
const struct bstgmac_mode_ops chain_mode_ops = {
	.init = init_dma_chain,
