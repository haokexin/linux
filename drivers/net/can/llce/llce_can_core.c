...
static struct device *get_can_core_dev(struct llce_can_core *can_core)
{
				...
				struct device *dev)
{
	...
	if (IS_ERR(can_core->config))
		return dev_err_probe(dev, PTR_ERR(can_core->config),
				     "Failed to request config channel\n");
...
static void llce_can_core_remove(struct platform_device *pdev)
{
...
static struct platform_driver llce_can_core_driver = {
	...
	.remove_new = llce_can_core_remove,
	.driver = {
		.name = "llce_can_core",
