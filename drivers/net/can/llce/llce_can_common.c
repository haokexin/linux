...
static void *get_netdev_name(struct device *dev, const char *basename)
{
	...
	ret = check_add_overflow(strlen(basename), 3, &name_len);
	if (ret) {
		pr_err("basename is too long.\n");
		return ERR_PTR(ret);
	}
	...
	ret = snprintf(dev_name, name_len, "%s%lu", basename, id);
	if (ret < 0) {
		pr_err("Failed to obtain a name based on: '%s' and '%lu'\n",
		       basename, id);
...
{
	netif_napi_add_weight(dev->can.dev, &dev->napi, llce_rx_poll,
			      LLCE_CAN_MAX_RX_MB);
}
...
static void strscpylower(char *dest, const char *source, size_t len)
{
	size_t i;

	strscpy(dest, source, len);
...
static void get_strings(struct net_device *dev, u32 stringset, u8 *buf)
{
	...
	for (i = 0; i < ARRAY_SIZE(llce_errors); i++) {
		...
		strscpylower(strings[j], err_name, ETH_GSTRING_LEN);
		j++;
	}
}
