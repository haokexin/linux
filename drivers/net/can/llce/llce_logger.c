				    ...
				    struct device *dev)
{
	...
	if (IS_ERR(can_dev->config))
		return dev_err_probe(dev, PTR_ERR(can_dev->config),
...
static int llce_logger_probe(struct platform_device *pdev)
{
	...
	struct can_priv *can;
	struct logger_config_msg config_msg = {
			.cmd = LOGGER_CMD_FW_SUPPORT,
			.fw_logger_support = false,
	};
	int ret;
...
static void llce_logger_remove(struct platform_device *pdev)
{
