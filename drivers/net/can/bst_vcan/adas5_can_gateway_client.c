...
static adas5_can_gateway_client_data_t *s_ins;
...
static int32_t receive_message(void)
{
	...
	switch0_can_gateway_client_ext_t *switch0_can_gateway_ext = &s_ins->switch0_can_gateway_ext;
	...
	if (ret == QUERY_INFO_DST_STS_OFFLINE) {
		if (switch0_can_gateway_ext->avail_changed_cb)
			switch0_can_gateway_ext->avail_changed_cb(false, switch0_can_gateway_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_ONLINE) {
		if (switch0_can_gateway_ext->avail_changed_cb)
			switch0_can_gateway_ext->avail_changed_cb(true, switch0_can_gateway_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_CHANGED) {
		ret = ipc_trans_layer_get_endmap(data->pid, &map);
		if (ret == 0) {
			chip_map = switch0_can_gateway_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & switch0_can_gateway_ext->cid_mask) != 0 ? true : false;
			if (status != switch0_can_gateway_ext->status) {
				switch0_can_gateway_ext->status = status;
				if (switch0_can_gateway_ext->avail_changed_cb)
					switch0_can_gateway_ext->avail_changed_cb(status, switch0_can_gateway_ext->avail_ext);
			}
...
static int32_t dispatch_message(void)
{
	...
	while (packet_obj) {
		...
		// dispatch message
		ret = -1;
		if (des->header.pid == s_ins->switch0_can_gateway_ext.cid) {
			if (des->header.typ == MSGBX_MSG_TYPE_BROADCAST)
				ret = s_ins->client.switch0_can_gateway_client.dispatch_broadcast(des);
			else if (des->header.typ == MSGBX_MSG_TYPE_REPLY)
				ret = s_ins->client.switch0_can_gateway_client.dispatch_reply(des);
		}
		if (ret < 0)
...
static int32_t start(void)
{
	...
	if (ret != 0) {
#elif defined IPC_RTE_RTOS
	data->route_task = TaskCreate((task_func_t)router_func, "adas5_can_gateway_client_thread", 0, NULL,
		98, (uint8_t *)stack_buffer, &tcb_buffer, true);
	if (data->route_task == 0) {
#elif defined IPC_RTE_KERNEL
	data->route_task = kthread_run(router_func, NULL, "adas5_can_gateway_client_thread");
...
#endif
// adas5_can_gateway_client_init
adas5_can_gateway_client_t *adas5_can_gateway_client_init(adas5_can_gateway_client_data_t *ins)
{
	int32_t ret = 0;
	...
	// init clients
	ret = switch0_can_gateway_client_init(data, &ins->client.switch0_can_gateway_client, &ins->switch0_can_gateway_ext);
	if (ret < 0)
		return NULL;

	// create client handle.
	ret = ipc_trans_layer_proxy_create_handle(data->pid, data->fid, data->sid, ins->switch0_can_gateway_ext.cid, ins->switch0_can_gateway_ext.ccid, &data->handle);
	if (ret < 0)
	{
...
int32_t adas5_can_gateway_client_destroy(void)
{
	int32_t ret = 0;
	com_client_data_t *data = (com_client_data_t *)s_ins;
	...
	switch0_can_gateway_client_destroy();

	IPC_MUTEX_DESTROY(&data->send_mtx);
	destroy_registry_list(data->common_registry, IPC_TOKEN_NUM);
	ipc_memset(s_ins, 0, sizeof(adas5_can_gateway_client_data_t));
	s_ins = NULL;
	return ret;
}
