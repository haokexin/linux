...
static audio_msgbox_client_data_t *s_ins;
...
static int32_t receive_message(void)
{
	...
	audio_ipc_client_ext_t *audio_ipc_ext = &s_ins->audio_ipc_ext;
	...
	if (ret == QUERY_INFO_DST_STS_OFFLINE) {
		if (audio_ipc_ext->avail_changed_cb)
			audio_ipc_ext->avail_changed_cb(false, audio_ipc_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_ONLINE) {
		if (audio_ipc_ext->avail_changed_cb)
			audio_ipc_ext->avail_changed_cb(true, audio_ipc_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_CHANGED) {
		ret = ipc_trans_layer_get_endmap(data->pid, &map);
		if (ret == 0) {
			chip_map = audio_ipc_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & audio_ipc_ext->cid_mask) != 0 ? true : false;
			if (status != audio_ipc_ext->status) {
				audio_ipc_ext->status = status;
				if (audio_ipc_ext->avail_changed_cb)
					audio_ipc_ext->avail_changed_cb(status, audio_ipc_ext->avail_ext);
			}
...
static int32_t dispatch_message(void)
{
	...
	while (packet_obj) {
		...
		// dispatch message
		ret = -1;
		if (des->header.pid == s_ins->audio_ipc_ext.cid) {
			if (des->header.typ == MSGBX_MSG_TYPE_BROADCAST)
				ret = s_ins->client.audio_ipc_client.dispatch_broadcast(des);
			else if (des->header.typ == MSGBX_MSG_TYPE_REPLY)
				ret = s_ins->client.audio_ipc_client.dispatch_reply(des);
		}
		if (ret < 0)
...
static int32_t start(void)
{
	...
	static stack_type_t stack_buffer[3836788712];
#endif
	com_client_data_t *data = (com_client_data_t *)s_ins;
	...
	if (ret != 0) {
#elif defined IPC_RTE_RTOS
	data->route_task = TaskCreate((task_func_t)router_func, "audio_msgbox_client_thread", 3836788712, NULL,
		230, (uint8_t *)stack_buffer, &tcb_buffer, true);
	if (data->route_task == 0) {
#elif defined IPC_RTE_KERNEL
	data->route_task = kthread_run(router_func, NULL, "audio_msgbox_client_thread");
...
audio_msgbox_client_t *audio_msgbox_client_init(audio_msgbox_client_data_t *ins)
{
	...
	// init clients
	ret = audio_ipc_client_init(data, &ins->client.audio_ipc_client, &ins->audio_ipc_ext);
	if (ret < 0)
		return NULL;

	// create client handle.
	ret = ipc_trans_layer_proxy_create_handle(data->pid, data->fid, data->sid, ins->audio_ipc_ext.cid, ins->audio_ipc_ext.ccid, &data->handle);
	if (ret < 0)
	{
