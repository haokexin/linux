...
#define test_XrpDspCmd_t struct _test_XrpDspCmd_t
							...
							const test_MyUnion_t *in)
{
	...
	ret = ipc_ser_put_align(ser, (uint8_t *)in, sizeof(test_MyUnion_t), 4);
	return ret >= 0 ? 0 : -1;
}
							...
							test_MyUnion_t *out)
{
	...
	ptr = (test_MyUnion_t *)alloc_data(buf, sizeof(test_MyUnion_t), 4);
	if (!ptr)
		return -1;
	*out = *ptr;
	return 0;
...
/**
 * Serialize test_XrpDspCmd_t.
 *
 * @param ser Pointer to serdes_t, which stores the serialized data.
 * @param in The input data to be serialized.
 * @return 0 if success, negative if fail.
 */
static inline int32_t serialize_test_XrpDspCmd(
							serdes_t *ser,
							const test_XrpDspCmd_t *in)
{
	int32_t ret = 0;
 ...
 * @return 0 if success, negative if fail.
 */
static inline int32_t deserialize_test_XrpDspCmd(
							des_buf_t *buf,
							test_XrpDspCmd_t *out)
{
	int32_t ret = 0;
