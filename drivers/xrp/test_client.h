 ...
 * The data MAY CHANGED after leaving the callback function.
 */
typedef void (*test_slt_method_callback_t)(
 ...
 * The data MAY CHANGED after leaving the callback function.
 */
typedef void (*test_xrp_shmem_addr_method_callback_t)(
 ...
 */
struct _test_client_ext_t {
	uint8_t cid;
	uint8_t res[7];
	callback_registration_t heartbeat_registry;
};
#define test_client_ext_t struct _test_client_ext_t

/**
 * Initializes the client.
 *
 * @param data The data for com_client_data_t
 * @param client The data for test_client_t
 * @param ext The data for test_client_ext_t
 * @return 0 if success, negative if fail.
 */
int32_t test_client_init(com_client_data_t *data,
