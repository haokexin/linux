...
static sample_client_data_t *s_ins;
...
static int32_t dispatch_message(void)
{
	...
	while (true) {
		...
		if (ipc_trans_layer_proxy_get_broadcast_msg(data->pid, data->handle, des) >= 0) {
			has_message = true;
			if (des->header.pid == s_ins->test_ext.cid)
				ret = s_ins->client.test_client.dispatch_broadcast(des);
			else
		...
		if (ipc_trans_layer_proxy_get_reply_msg(data->pid, data->handle, des) >= 0) {
			has_message = true;
			if (des->header.pid == s_ins->test_ext.cid)
				ret = s_ins->client.test_client.dispatch_reply(des);
			else
...
//	int32_t ret = 0;
	...
	if (ret != thrd_success) {
#elif defined IPC_RTE_KERNEL
	data->route_task = kthread_run(router_func, NULL, "sample_client_thread");
...
sample_client_t *sample_client_init(sample_client_data_t *ins)
{
	...
	// init clients
	ret = test_client_init(data, &ins->client.test_client, &ins->test_ext);
	if (ret < 0)
		return NULL;

	// create client handle.
	ret = ipc_trans_layer_proxy_create_handle(data->pid, data->fid, data->sid, ins->test_ext.cid, &data->handle);
	if (ret < 0)
		return NULL;
...
int32_t sample_client_destroy(void)
{
	...
	ipc_memset(s_ins, 0, sizeof(sample_client_data_t));
	s_ins = NULL;
	return ret;
}
