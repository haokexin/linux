...
struct scmi_msg_attr_domain_reset {
	__le32 domain_id;
};
...
static int
scmi_reset_domain_attributes_get(const struct scmi_protocol_handle *ph,unsigned int domain_id, struct reset_dom_info *dom_info)
{
	int ret=-1;
	struct scmi_xfer *t;
	struct scmi_msg_resp_reset_domain_attributes *rattr;
	struct scmi_msg_attr_domain_reset * tattr;
	u32 attributes;
	char * addr = NULL;
	...
	ret = ph->xops->xfer_get_init(ph, RESET_DOMAIN_ATTRIBUTES,sizeof(*tattr), sizeof(*rattr), &t);
	if (ret)
		return ret;


	tattr = t->tx.buf;
	tattr->domain_id = cpu_to_le32(domain_id);
	...
	ret = ph->xops->do_xfer(ph, t);
	if (!ret) {
			attributes = le32_to_cpu(rattr->attributes);
			dom_info->async_reset = SUPPORTS_ASYNC_RESET(attributes);
			dom_info->reset_notify = SUPPORTS_NOTIFY_RESET(attributes);
			dom_info->latency_us = le32_to_cpu(rattr->latency);
			if (dom_info->latency_us == U32_MAX)
				dom_info->latency_us = 0;
			strlcpy(dom_info->name, rattr->names, SCMI_MAX_STR_SIZE);	
	}
...
static const char *scmi_reset_name_get(const struct scmi_protocol_handle *ph,u32 domain)
{
	struct scmi_reset_info *pi = ph->get_priv(ph);
...
static int scmi_reset_protocol_init(const struct scmi_protocol_handle *ph)
{
	...
	for(domain_id=0;domain_id<pinfo->num_domains;domain_id++){
		bom = pinfo->dom_info+domain_id;
		scmi_reset_domain_attributes_get(ph, domain_id,bom);
	}
