...
struct scmi_info {
	...
	//struct completion       timer_complete;
				 ...
				 u32 msg_hdr, void *priv)
{
	...
	// writel(0x0,info->base_timer+0x8);
	// writel(0x200,info->base_timer);
	// writel(0x3,info->base_timer+0x8);
		   ...
		   struct scmi_xfer *xfer)
{
	...
	struct device *dev = info->dev;
	struct scmi_chan_info *cinfo;
	struct scmi_msgbox *scmi_msgbox_info = NULL;
	...
	//reinit_completion(&info->timer_complete);


	scmi_msgbox_info = cinfo->transport_info;
				  ...
				  const struct scmi_protocol *proto)
{
	...
	dev_err(handle->dev, "Initialized protocol: 0x%X\n", pi->proto->id);
...
static irqreturn_t dw_apb_clockevent_irq(int irq, void *data){
	u32 ctrl;
	struct scmi_info *info = (struct scmi_info *)data;

	ctrl = readl(info->base_timer+0x08);
	ctrl |= APBTMR_CONTROL_INT;
	writel(ctrl,info->base_timer+0x08);


	complete(&info->timer_complete);
...
static int scmi_probe(struct platform_device *pdev)
{
	...
	//init_completion(&info->timer_complete);
	//spin_lock_init(&info->timer_lock);

	//info->base_timer = of_iomap(np, 0);
	// irq = irq_of_parse_and_map(np, 0);
	// if (irq == 0){
	// 	printk("scmi driver irq = 0\n");
	// 	return -EINVAL;
	// }


	// err = request_irq(irq, dw_apb_clockevent_irq, IRQF_TIMER | IRQF_IRQPOLL | IRQF_NOBALANCING,"scmi-timer", info);
	// if (err) {
...
#endif
#ifdef CONFIG_SCMI_COMPLETE_COUNT
	{ .compatible = "arm,scmi-msgbox", .data = &scmi_msgbox_desc },
#endif
...
static int dw_scmi_plat_suspend(struct device *dev){

	return 0;
}


static int  dw_scmi_plat_resume(struct device *dev)
{
	struct scmi_info *info = dev_get_drvdata(dev);
	struct scmi_chan_info *cinfo;
	int ret;
	...
	ret = info->desc->ops->chan_resume(cinfo);
...
static const struct dev_pm_ops dw_scmi_pm_ops = {
	SET_LATE_SYSTEM_SLEEP_PM_OPS(dw_scmi_plat_suspend, dw_scmi_plat_resume)
};
