...
static security_client_data_t *s_ins;
...
static int32_t receive_message(void)
{
	...
	service_client_ext_t *service_ext = &s_ins->service_ext;
	...
	if (ret == QUERY_INFO_DST_STS_OFFLINE) {
		if (service_ext->avail_changed_cb)
			service_ext->avail_changed_cb(false, service_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_ONLINE) {
		if (service_ext->avail_changed_cb)
			service_ext->avail_changed_cb(true, service_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_CHANGED) {
		ret = ipc_trans_layer_get_endmap(data->pid, &map);
		if (ret == 0) {
			chip_map = service_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & service_ext->cid_mask) != 0 ? true : false;
			if (status != service_ext->status) {
				service_ext->status = status;
				if (service_ext->avail_changed_cb)
					service_ext->avail_changed_cb(status, service_ext->avail_ext);
			}
...
static int32_t dispatch_message(void)
{
	...
	while (packet_obj) {
		...
		// dispatch message
		ret = -1;
		if (des->header.pid == s_ins->service_ext.cid) {
			if (des->header.typ == MSGBX_MSG_TYPE_BROADCAST)
				ret = s_ins->client.service_client.dispatch_broadcast(des);
			else if (des->header.typ == MSGBX_MSG_TYPE_REPLY)
				ret = s_ins->client.service_client.dispatch_reply(des);
		}
		if (ret < 0)
...
static int32_t start(void)
{
	...
	if (ret != 0) {
#elif defined IPC_RTE_RTOS
	data->route_task = TaskCreate((task_func_t)router_func, "security_client_thread", 0, NULL,
		0, (uint8_t *)stack_buffer, &tcb_buffer, true);
	if (data->route_task == 0) {
#elif defined IPC_RTE_KERNEL
	data->route_task = kthread_run(router_func, NULL, "security_client_thread");
...
security_client_t *security_client_init(security_client_data_t *ins)
{
	int32_t ret = 0;
	...
	// init clients
	ret = service_client_init(data, &ins->client.service_client, &ins->service_ext);
	if (ret < 0)
		return NULL;

	// create client handle.
	ret = ipc_trans_layer_proxy_create_handle(data->pid, data->fid, data->sid, ins->service_ext.cid, ins->service_ext.ccid, &data->handle);
	if (ret < 0)
	{
...
// security_client_destroy
int32_t security_client_destroy(void)
{
	int32_t ret = 0;
	com_client_data_t *data = (com_client_data_t *)s_ins;
