 ...
 * change on the way
 */
static void clk_change_rate_only(struct clk_core *core)
{
	struct clk_core *child;
	struct hlist_node *tmp;
	unsigned long old_rate;
	...
	hlist_for_each_entry_safe(child, tmp, &core->children, child_node) {
		...
		clk_change_rate_only(child);
	}

	/* handle the new child who might not be in core->children yet */
	if (core->new_child){
		
		clk_change_rate_only(core->new_child);
	}
...
static int clk_core_set_rate_only_nolock(struct clk_core *core,
				    unsigned long req_rate)
{
	struct clk_core *top, *fail_clk;
	unsigned long rate;
...
/**
 * bst_clk_set_rate_only - specify a new rate for clk
 * @clk: the clk whose rate is being changed
 * @rate: the new rate for clk
 *
 * In the simplest case clk_set_rate will only adjust the rate of clk.
 ...
 * Returns 0 on success, -EERROR otherwise.
 */
int bst_clk_set_rate_only(struct clk *clk, unsigned long rate)
{
	int ret;
	...
	ret = clk_core_set_rate_only_nolock(clk->core, rate);
...
EXPORT_SYMBOL_GPL(bst_clk_set_rate_only);



/**
 * bst_clk_set_rate - specify a new rate for clk
 * @clk: the clk whose rate is being changed
 * @rate: the new rate for clk
 *
 * In the simplest case clk_set_rate will only adjust the rate of clk.
...
int bst_clk_set_rate_divider(struct clk *clk, unsigned long rate)
{
	...
	// if(parent){
	// 	parent_rate = clk->core->ops->rate_get(clk->core->hw, rate, parent_rate);
	// }

	

	clk->core->ops->set_rate(clk->core->hw, rate, parent_rate);

	clk_prepare_unlock();

	return ret;
}
EXPORT_SYMBOL_GPL(bst_clk_set_rate_divider);
