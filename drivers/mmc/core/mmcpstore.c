...
static struct mmcpstore_context {
	...
	struct pstore_device_info info;
...
static ssize_t mmcpstore_panic_write(const char *buf, size_t sects, loff_t off)
{
	struct mmcpstore_context *cxt = &oops_cxt;
	struct mmc_card *card = cxt->card;
	unsigned int sects = size / SECTOR_SIZE;
	unsigned int sect_off = off / SECTOR_SIZE;
	int ret;
	...
	ret = mmcpstore_panic_write_req(buf, sects, cxt->start_sect + sect_off);
	if (ret)
		return ret;
...
void mmcpstore_card_set(struct mmc_card *card, const char *disk_name)
{
	...
	struct pstore_device_info *info = &cxt->info;
	struct block_device *bdev;
	...
	info->zone.panic_write = mmcpstore_panic_write;
	ret = register_pstore_device(info);
	if (ret) {
...
void unregister_mmcpstore(void)
{
	...
	unregister_pstore_device(&cxt->info);
	kfree(cxt->mrq->data);
	kfree(cxt->mrq->stop);
	kfree(cxt->mrq->cmd);
	kfree(cxt->mrq);
