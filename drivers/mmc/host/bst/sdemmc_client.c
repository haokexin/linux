...
static sdemmc_client_ext_t *s_ext;
...
static inline int32_t call_fusacoreip_method_callback(des_buf_t *des)
{
	...
	des_buf_t *buf = NULL;
	com_client_data_t *data = s_data;
	sdemmc_fusacoreip_method_callback_t cb = NULL;
	...
	if (ret >= 0)
		ret = deserialize_sdemmc_ErrorEnum(buf, &err);
	...
	cb = (sdemmc_fusacoreip_method_callback_t)(reg->cb);
	if (cb)
				...
				)
{
	...
	*out->psm_id = (sdemmc_UInt32Array4_t *)psm_id;
	*out->err = err;
...
static inline int32_t call_fusaenable_method_callback(des_buf_t *des)
{
	...
	des_buf_t *buf = NULL;
	com_client_data_t *data = s_data;
	sdemmc_fusaenable_method_callback_t cb = NULL;
	...
	if (ret < 0)
		return -ERR_APP_SERDES;
	if (err == SDEMMC_NO_ERROR) {
		if (ret >= 0)
			ret = deserialize_8(buf, (uint8_t *)&block_id_out);
		if (ret >= 0)
			ret = deserialize_sdemmc_UInt32Array4(buf, &psm_id);
		if (ret < 0)
			return -ERR_APP_SERDES;
	}

	// call callback function
	cb = (sdemmc_fusaenable_method_callback_t)(reg->cb);
	if (cb)
...
// initialize client
int32_t sdemmc_client_init(com_client_data_t *data, sdemmc_client_t *client,
