...
static healthmonitor_client_ext_t *s_ext;

#ifndef IPC_RTE_BAREMETAL

struct _fusacoreip_method_out_t {
	DECL_SEM(sem)
	healthmonitor_ErrorEnum_t *err;
};
...
struct _fusaenable_method_out_t {
	...
	healthmonitor_UInt32Array4_t **psm_id;
	healthmonitor_ErrorEnum_t *err;
};
...
static void fusacoreip_method_sync_callback(
				const healthmonitor_ErrorEnum_t err,
				void *ext,
...
static int32_t call_fusacoreip_method_async(const uint32_t dtc,
				healthmonitor_fusacoreip_method_callback_t cb,
				void *ext,
				des_buf_t *ext_buf)
{
	int32_t ret = 0;
...
static inline int32_t call_fusacoreip_method_callback(des_buf_t *des)
{
	...
	des_buf_t *buf = NULL;
	com_client_data_t *data = s_data;
	healthmonitor_fusacoreip_method_callback_t cb = NULL;
	...
	if (ret >= 0)
		ret = deserialize_healthmonitor_ErrorEnum(buf, &err);
	...
	cb = (healthmonitor_fusacoreip_method_callback_t)(reg->cb);
	if (cb)
...
static void fusaenable_method_sync_callback(
				...
				const healthmonitor_UInt32Array4_t *psm_id,
				const healthmonitor_ErrorEnum_t err,
				void *ext,
				const ext_info_t *info
				)
{
	...
	*out->psm_id = (healthmonitor_UInt32Array4_t *)psm_id;
	*out->err = err;
...
static int32_t call_fusaenable_method_async(const uint8_t block_id,
				healthmonitor_fusaenable_method_callback_t cb,
				void *ext,
				des_buf_t *ext_buf)
{
	int32_t ret = 0;
...
static inline int32_t call_fusaenable_method_callback(des_buf_t *des)
{
	...
	des_buf_t *buf = NULL;
	com_client_data_t *data = s_data;
	healthmonitor_fusaenable_method_callback_t cb = NULL;
	...
	if (ret < 0)
		return -ERR_APP_SERDES;
	if (err == HEALTHMONITOR_NO_ERROR) {
		if (ret >= 0)
			ret = deserialize_8(buf, (uint8_t *)&block_id_out);
		if (ret >= 0)
			ret = deserialize_healthmonitor_UInt32Array4(buf, &psm_id);
		if (ret < 0)
			return -ERR_APP_SERDES;
	}

	// call callback function
	cb = (healthmonitor_fusaenable_method_callback_t)(reg->cb);
	if (cb)
...
// initialize client
int32_t healthmonitor_client_init(com_client_data_t *data, healthmonitor_client_t *client,
			healthmonitor_client_ext_t *ext)
{
	if (!data || !client || !ext)
		return -1;
...
// destroy client
void healthmonitor_client_destroy(void)
{
