...
static int32_t receive_message(void)
{
	...
	st_public1_client_ext_t *st_public1_ext = &s_ins->st_public1_ext;
	...
	if (ret == QUERY_INFO_DST_STS_OFFLINE) {
		if (st_public1_ext->avail_changed_cb)
			st_public1_ext->avail_changed_cb(false, st_public1_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_ONLINE) {
		if (st_public1_ext->avail_changed_cb)
			st_public1_ext->avail_changed_cb(true, st_public1_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_CHANGED) {
		ret = ipc_trans_layer_get_endmap(data->pid, &map);
		if (ret == 0) {
			chip_map = st_public1_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & st_public1_ext->cid_mask) != 0 ? true : false;
			if (status != st_public1_ext->status) {
				st_public1_ext->status = status;
				if (st_public1_ext->avail_changed_cb)
					st_public1_ext->avail_changed_cb(status, st_public1_ext->avail_ext);
			}
...
static int32_t start(void)
{
	...
	if (ret != 0) {
#elif defined IPC_RTE_RTOS
	data->route_task = TaskCreate((task_func_t)router_func, "thermalclient_thread", 0, NULL,
...
thermalclient_t *thermalclient_init(thermalclient_data_t *ins)
{
	...
	ret = ipc_trans_layer_proxy_create_handle(data->pid, data->fid, data->sid, ins->st_public1_ext.cid, ins->st_public1_ext.ccid, &data->handle);
	if (ret < 0)
	{
