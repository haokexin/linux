...
static int32_t receive_message(void)
{
	...
	st_public_client_ext_t *st_public_ext = &s_ins->st_public_ext;
	...
	if (ret == QUERY_INFO_DST_STS_OFFLINE) {
		if (st_public_ext->avail_changed_cb)
			st_public_ext->avail_changed_cb(false, st_public_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_ONLINE) {
		if (st_public_ext->avail_changed_cb)
			st_public_ext->avail_changed_cb(true, st_public_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_CHANGED) {
		ret = ipc_trans_layer_get_endmap(data->pid, &map);
		if (ret == 0) {
			chip_map = st_public_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & st_public_ext->cid_mask) != 0 ? true : false;
			if (status != st_public_ext->status) {
				st_public_ext->status = status;
				if (st_public_ext->avail_changed_cb)
					st_public_ext->avail_changed_cb(status, st_public_ext->avail_ext);
			}
...
static int32_t dispatch_message(void)
{
	...
	while (packet_obj) {
		...
		// dispatch message
		ret = -1;
		if (des->header.pid == s_ins->st_public_ext.cid) {
			if (des->header.typ == MSGBX_MSG_TYPE_BROADCAST)
				ret = s_ins->client.st_public_client.dispatch_broadcast(des);
			else if (des->header.typ == MSGBX_MSG_TYPE_REPLY)
				ret = s_ins->client.st_public_client.dispatch_reply(des);
		}
		if (ret < 0)
...
static int32_t start(void)
{
	...
	if (ret != 0) {
#elif defined IPC_RTE_RTOS
	data->route_task = TaskCreate((task_func_t)router_func, "SocClient_thread", 0, NULL,
		0, (uint8_t *)stack_buffer, &tcb_buffer, true);
	if (data->route_task == 0) {
#elif defined IPC_RTE_KERNEL
	data->route_task = kthread_run(router_func, NULL, "SocClient_thread");
...
SocClient_t *SocClient_init(SocClient_data_t *ins)
{
	...
	// init clients
	ret = st_public_client_init(data, &ins->client.st_public_client, &ins->st_public_ext);
	if (ret < 0)
		return NULL;

	// create client handle.
	ret = ipc_trans_layer_proxy_create_handle(data->pid, data->fid, data->sid, ins->st_public_ext.cid, ins->st_public_ext.ccid, &data->handle);
	if (ret < 0)
	{
