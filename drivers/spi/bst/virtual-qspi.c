...
int qspi_client_method(SocClient_t * client,struct  nor_ipcmsg_  * msg_data)
{
	...
	if (!client) {
        printk("%s line:%d fls is not initial!\n", __func__, __LINE__);
        return -1;
    }

    if (!msg_data) {
        printk("%s line:%d msg_data is null !\n", __func__, __LINE__);
        return -1;
    }
    ...
    ret = client->st_public_client.qspi_method_sync(
                                fw_cmd_msg,
                                &st_err,
                                200,
                                NULL);
    if (ret < 0) {
        printk("%s line:%d send msg async method failed. ret is %d\n", __func__, __LINE__, ret);
        return -1;
    }
...
static int ram_erase(struct mtd_info *mtd, struct erase_info *instr)
{
	...
	if (check_offs_len(mtd, instr->addr, instr->len)){
		printk("%s %d erase must align 4K\n",__func__,__LINE__);
		return -EINVAL;
	}
	...
	for(i=0;i<instr->len/ERASE_BLOCK_SIZE;i++)
	{
		...
		if(ret < 0){
			printk("%s %d erase error!!!!!!!!!!!!",__func__,__LINE__);
			return -EINVAL;
		}
	}	
	...
	for(i=0;i<len/MAX_BLOCK_SIZE+1;i++)
	{
		...
		if(ret < 0){
			...
			if(i == len/MAX_BLOCK_SIZE)
				memcpy(&buf[i*MAX_BLOCK_SIZE],v_qspi_info->map_addr,len%MAX_BLOCK_SIZE);
			else
				memcpy(&buf[i*MAX_BLOCK_SIZE],v_qspi_info->map_addr,MAX_BLOCK_SIZE);
		}
	}
		...
		size_t *retlen, const u_char *buf)
{
	...
	for(i=0;i<len/MAX_BLOCK_SIZE+1;i++)
	{
		...
		if(i == len/MAX_BLOCK_SIZE){
			...
			memcpy(sbuf,&buf[i*MAX_BLOCK_SIZE],len%MAX_BLOCK_SIZE);
...
int mtdram_init_device(struct mtd_info *mtd, void *info,
...
static int  virtual_qspi_driver_probe(struct platform_device *pdev)
{
	...
	v_qspi_info = (struct virtual_qspi_ * )kmalloc(sizeof(struct virtual_qspi_),GFP_KERNEL);
	if(v_qspi_info == NULL)
		return -EINVAL;
    ...
    v_qspi_info->map_addr  = dma_alloc_coherent(&pdev->dev, 0x10000,&v_qspi_info->partition_dma_addr, GFP_KERNEL);
	...
	err = mtdram_init_device(v_qspi_info->mtd_info, v_qspi_info, MTDRAM_TOTAL_SIZE, "virtual device");
	if (err) {
