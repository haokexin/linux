...
static void dw_spi_hw_init(struct device *dev, struct dw_spi *dws)
{
	...
	if (dw_spi_ip_is(dws, PSSI)) {
		...
		// Set buard after resume when current_freq is setted.
		if (dws->current_freq != 0)
			dw_spi_set_clk(dws, dws->max_freq / dws->current_freq);
...
static int dw_slave_abort(struct spi_controller *controller)
{
	if (controller->slave) {
		struct dw_spi *dws =
...
int dw_spi_add_host_slave(struct device *dev, struct dw_spi *dws)
{
	...
	spi_controller_set_devdata(slave, dws);
	...
	slave->mode_bits = SPI_CPOL | SPI_CPHA | SPI_LOOP;
	if (dws->caps & DW_SPI_CAP_DFS32)
		slave->bits_per_word_mask = SPI_BPW_RANGE_MASK(4, 32);
	else
		slave->bits_per_word_mask = SPI_BPW_RANGE_MASK(4, 16);
	slave->bus_num = dws->bus_num;
	slave->num_chipselect = dws->num_cs;
	slave->setup = dw_spi_setup;
	slave->cleanup = dw_spi_cleanup;
	slave->transfer_one = dw_spi_transfer_one;
	slave->handle_err = dw_spi_handle_err;
	slave->max_speed_hz = dws->max_freq;
	slave->dev.of_node = dev->of_node;
	slave->slave_abort = dw_slave_abort;

	/* Basic HW init, get fifo len */
	...
	if (dws->dma_ops && dws->dma_ops->dma_init) {
		...
		if (ret) {
			...
			slave->can_dma = dws->dma_ops->can_dma;
		}
	}

	ret = spi_register_controller(slave);
...
err_dma_exit:
	if (dws->dma_ops && dws->dma_ops->dma_exit)
		dws->dma_ops->dma_exit(dws);
	dw_spi_enable_chip(dws, 0);
	free_irq(dws->irq, slave);
err_free_master:
	spi_controller_put(slave);
	return ret;
}
EXPORT_SYMBOL_NS_GPL(dw_spi_add_host_slave, SPI_DW_CORE);
