...
static void dw_spi_dma_maxburst_init(struct dw_spi *dws)
{
	...
	// dw_writel(dws, DW_SPI_DMARDLR, dws->rxburst - 1);  // Community Code.
...
static enum dma_slave_buswidth dw_spi_dma_convert_width(u8 n_bytes)
{
	...
	printk("[SPI] dma slave bus width undefined!\n");
...
static int dw_spi_dma_config_tx(struct dw_spi *dws)
{
	...
	txconf.src_addr_width = dw_spi_dma_convert_width(dws->n_bytes);
...
static int dw_spi_dma_config_rx(struct dw_spi *dws)
{
	...
	rxconf.dst_addr_width = dw_spi_dma_convert_width(dws->n_bytes);
