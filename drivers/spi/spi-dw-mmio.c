...
//		dws->max_freq = 200000000;
	...
	is_slave = false;
	is_slave = device_property_read_bool(&pdev->dev, "spi-slave");
	...
	if (is_slave) {
		ret = dw_spi_add_host_slave(&pdev->dev, dws);
		dev_info(&pdev->dev, "spi is slave num_cs:%u ret:%d", num_cs, ret);
	} else {
...
static int dw_spi_prepare_clk(struct dw_spi_mmio *dwsmmio, bool prepare)
{
	if (prepare) {
		int ret;
		...
		ret = clk_prepare_enable(dwsmmio->clk);
		SPI_DEBUG_PRINT("spi clk enable! ret : %d.\n", ret);

		ret = clk_prepare_enable(dwsmmio->pclk);
		SPI_DEBUG_PRINT("spi pclk enable! ret : %d.\n", ret);
...
static int dw_spi_suspend(struct device *dev)
{
	...
	return dw_spi_prepare_clk(dwsmmio, false);
}
...
static int dw_spi_resume(struct device *dev)
{
	...
	ret = dw_spi_prepare_clk(dwsmmio, true);
	if (ret) {
		dev_err(dev, "enable pclk failed!\n");
		return ret;
	}
