...
static int usbhid_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
    ...
    if(0x222a == hid->vendor) {
        unsigned int len = strlen(hid->name);
        snprintf(hid->name + len, sizeof(hid->name) - len, "bst%u",dev->portnum);
    }
	...
	if (usb_string(dev, dev->descriptor.iSerialNumber, hid->uniq, 64) <= 0)
		snprintf(hid->uniq, sizeof(hid->uniq), "unknown-device%u",dev->portnum);
