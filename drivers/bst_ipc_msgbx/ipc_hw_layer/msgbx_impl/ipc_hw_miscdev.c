...
static void msgbx_stats_print(struct msgbox_statis *stats)
{
	...
	u64 rx_overflow_stats = 0, rx_thrs_stats = 0;
			       ...
			       size_t count, loff_t *lf)
{
	...
	pr_info("send_fail_cnt:%u, last_msg_frame_cnt:%u\n", info.send_fail_cnt,
		info.send_frame_cnt);
	pr_info("recv_msg_1_cnt:%u, recv_msg_2_cnt:%u, recv_rw_msg_cnt:%u\n", info.recv_msg_1_cnt,
		info.recv_msg_2_cnt, info.recv_rw_msg_cnt);
	pr_info("send_start_time: %llu, send_end_time:%llu, ", info.send_start_time,
		info.send_end_time);
	pr_info("collate_time:%llu, get_msg_time:%llu\n", info.collate_time,
		info.get_msg_time);
	return count;
}
					 ...
					 void __user *arg)
{
	...
	ret = ipc_ioctl_parse_arg(&layer, arg, cmd);
	if (unlikely(ret)) {
		IPC_LOG_ERR("copy_to_user fail! k_role:%u", layer.k_role);
		return -EFAULT;
	}

#ifdef CONFIG_IPC_LAYER_START
	ret = ipc_trans_layer_start(layer.end_id, layer.k_role);
	if (ret < 0)
		IPC_LOG_WARNING("layer start %d fail!", layer.k_role);
#endif
...
static int32_t ipc_ioctl_layer_stop(unsigned int cmd, void __user *arg)
{
	...
	ret = ipc_ioctl_parse_arg(&end_id, arg, cmd);
	if (unlikely(ret)) {
		IPC_LOG_ERR("copy_to_user fail! end_id:%u", end_id);
		return -EFAULT;
	}

	ret = ipc_trans_layer_stop(end_id);;
	if (ret < 0)
		IPC_LOG_WARNING("layer stop endid%d fail!", end_id);
...
static int32_t ipc_ioctl_create_handle(unsigned int cmd, void __user *arg)
{
	...
	case MSGBX_SES_ROLE_CLIENT:
		func = ipc_trans_layer_proxy_create_handle;
		break;
	case MSGBX_SES_ROLE_SERVER:
		func = ipc_trans_layer_stub_create_handle;
		break;
	case MSGBX_SES_ROLE_FASTPATH:
		func = ipc_trans_layer_create_handle;
		break;
	default:
		ret = -EINVAL;
	}
	...
	if (likely(func)) {
		ret = func(handle.endid, handle.fid, handle.sid, handle.cid, &handle.handle);
		if (unlikely(ret))
			IPC_LOG_WARNING("layer can't create handle! ret = %d", ret);
	}
...
static int32_t ipc_ioctl_register_method(unsigned int cmd, void __user *arg)
{
	...
	if (unlikely(ret))
		IPC_LOG_WARNING("layer can't register method! ret = %d", ret);
...
static int32_t ipc_ioctl_unregister_method(unsigned int cmd, void __user *arg)
{
	...
	if (unlikely(ret))
		IPC_LOG_WARNING("layer can't unregister method! ret = %d", ret);
...
static int32_t ipc_ioctl_destory_handle(unsigned int cmd, void __user *arg)
{
	...
	if (unlikely(ret))
		IPC_LOG_WARNING("layer can't destroy handle! ret = %d", ret);
...
static int32_t ipc_ioctl_send_msg(unsigned int cmd, void __user *arg)
{
	...
	case MSGBX_MSG_TYPE_REPLY:
		func = ipc_trans_layer_stub_send_reply_msg;
		break;
	case MSGBX_MSG_TYPE_BROADCAST:
		func = ipc_trans_layer_stub_send_broadcast;
		break;
	case MSGBX_MSG_TYPE_METHOD:
		func = ipc_trans_layer_proxy_send_method;
		break;
	default:
		ret = -EINVAL;
	}
	...
	if (func) {
		ret = func(msg.endid, msg.handle, &msg.msg);
		if (unlikely(ret < 0))
			IPC_LOG_WARNING("layer can't send msg! ret = %d", ret);
	}
...
static int32_t ipc_ioctl_get_msg(unsigned int cmd, void __user *arg)
{
	...
	switch (msg.msg.header.typ) {
	case MSGBX_MSG_TYPE_REPLY:
		func = ipc_trans_layer_proxy_get_reply_msg;
		break;
	case MSGBX_MSG_TYPE_BROADCAST:
		func = ipc_trans_layer_proxy_get_broadcast_msg;
		break;
	case MSGBX_MSG_TYPE_METHOD:
		func = ipc_trans_layer_stub_get_method_msg;
		break;
	default:
		ret = -EINVAL;
	}
...
static int32_t ipc_ioctl_query_msg(unsigned int cmd, void __user *arg)
{
	...
	ret = ipc_trans_layer_query_msg(info.endid, info.handle);
#else
	ret = ipc_trans_layer_query_info(info.endid, info.handle, 1);
#endif
	if (unlikely(ret))
...
static int32_t ipc_ioctl_user_get_msg(unsigned int cmd, void __user *arg)
{
	...
	ret = ipc_trans_layer_get_msg(msg.end_id, msg.handle, msg.timeout,
...
static void ipc_smp_set_regs(void *info)
{
	...
	case REG_TYPE_LEN_FLT :
		if (fid == 0) {
			pr_err("End_filter1_MsgH_LenF_CFGR(0x%x) "
	...
	case REG_TYPE_MSGH_MASKH :
		if (fid == 0) {
			pr_err("End_filter1_MsgH_ResF_MaskHR(0x%x) "
				"is useful for non-default filter\n",
	...
	case REG_TYPE_MSGH_MINH :
		if (fid == 0) {
			pr_err("End_filter1_MsgH_ResF_MinHR(0x%x) "
	...
	case REG_TYPE_MSGH_MAXH :
		if (fid == 0) {
			pr_err("End_filter1_MsgH_ResF_MaxHR(0x%x) "
	...
	case REG_TYPE_MSGH_MASK :
		if (fid == 0) {
			pr_err("End_filter1_MsgH_ResF_MaskR(0x%x) "
	...
	case REG_TYPE_MSGH_MIN :
		if (fid == 0) {
			pr_err("End_filter1_MsgH_ResF_MinR(0x%x) "
	...
	case REG_TYPE_MSGH_MAX :
		if (fid == 0) {
			pr_err("End_filter1_MsgH_ResF_MaxR(0x%x) "
	...
	case REG_TYPE_FLT_CFGR :
		if (fid == 0) {
			pr_err("End_filter1_RxFIFO_CFGR(0x%x) "
	...
	case REG_TYPE_PAY_MASK :
		if (fid == 0) {
			pr_err("End_filter1_MsgPx_MaskR(0x%x) "
	...
	case REG_TYPE_PAY_MIN :
		if (fid == 0) {
			pr_err("End_filter1_MsgPx_MinR(0x%x) "
	...
	case REG_TYPE_PAY_MAX :
		if (fid == 0) {
			pr_err("End_filter1_MsgPx_MaxR(0x%x) "
...
static long ipc_msgbx_ioctl(struct file *filp, uint32_t cmd, unsigned long arg)
{
	...
	if (ret < 0)
		pr_debug("ipc_ioctl exit, ret: %d", ret);
	return ret;
}
 ...
 *          error code - failure
 */
int ipc_msgbx_miscdev_init(ST_MSGBX_END_PARA *st_end_para, u8 dev_id)
{
	int ret;
	...
	snprintf(ipc_msgbx_dev_name, sizeof(IPC_DRIVER_NAME) + IPC_MSGBX_DEV_ID_LEN,
		 "%s%01d", IPC_DRIVER_NAME, dev_id);
	pr_debug("create device name: %s", ipc_msgbx_dev_name);

	// init & register ipc_msgbx miscdev
	miscdev.minor = MISC_DYNAMIC_MINOR;
	miscdev.fops = &ipc_msgbx_fops;
	miscdev.name = devm_kstrdup(st_end_para->ipc_msgbx->dev, ipc_msgbx_dev_name, GFP_KERNEL);
	miscdev.nodename = devm_kstrdup(st_end_para->ipc_msgbx->dev, ipc_msgbx_dev_name, GFP_KERNEL);

	ret = misc_register(&miscdev);
	if (ret < 0)
		pr_err("register device name: %s error!", ipc_msgbx_dev_name);
...
static int msgbox_misc_dev_init(void)
{
	...
	ret = ipc_msgbx_miscdev_init(&msg_end_para[i], i);
	for (i = 0; i < ipc_msgbx->msgend_count; i++) {
		if (unlikely(ret)) {
			msg_end_para[i].mdev_is_init = false;
			goto ERR_SYS_INIT;
		} else
			msg_end_para[i].mdev_is_init = true;
	}
...
MODULE_ALIAS("platform: msgbox-msic");
