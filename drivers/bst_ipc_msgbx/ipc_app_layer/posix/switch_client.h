...
#ifndef SWITCH_CLIENT_H
#define SWITCH_CLIENT_H

#define IPC_RTE_KERNEL
#include <bst/ipc_app_client_utils.h>
#include "switch_datatype.h"
...
struct _switch_client_t {
	 ...
	 *
	 * @param sec The input argument of method switch_timesync.
	 * @param nsec The input argument of method switch_timesync.
	 * @return 0 if success, negative if fail.
	 * @note This is unreliable transmission, be used ONLY if message losing is accepted.
	 */
	int32_t (*switch_timesync_fire_and_forget)(
					const uint32_t sec,
					const uint32_t nsec
					);
...
#define switch_client_t struct _switch_client_t

/**
 *  The extend data used by the client.
 */
struct _switch_client_ext_t {
	uint8_t cid;
	uint8_t res[7];

};
#define switch_client_ext_t struct _switch_client_ext_t

/**
 * Initializes the client.
 *
 * @param data The data for com_client_data_t
 * @param client The data for switch_client_t
 * @param ext The data for switch_client_ext_t
 * @return 0 if success, negative if fail.
 */
int32_t switch_client_init(com_client_data_t *data,
			switch_client_t *client,
