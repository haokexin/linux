...
TimeSyncClient_t *ts_client = NULL;
TimeSyncClient_data_t ts_data = {0};
static TimeSyncClient_data_t *s_ins;
...
static int32_t dispatch_message(void)
{
	...
	while (true) {
		...
		if (ipc_trans_layer_proxy_get_broadcast_msg(data->pid, data->handle, des) >= 0) {
			has_message = true;
			if (des->header.pid == s_ins->safety_ext.cid)
				ret = s_ins->client.safety_client.dispatch_broadcast(des);
			else if (des->header.pid == s_ins->realtime_ext.cid)
				ret = s_ins->client.realtime_client.dispatch_broadcast(des);
			else if (des->header.pid == s_ins->switch_ext.cid)
				ret = s_ins->client.switch_client.dispatch_broadcast(des);
			else
		...
		if (ipc_trans_layer_proxy_get_reply_msg(data->pid, data->handle, des) >= 0) {
			has_message = true;
			if (des->header.pid == s_ins->safety_ext.cid)
				ret = s_ins->client.safety_client.dispatch_reply(des);
			else if (des->header.pid == s_ins->realtime_ext.cid)
				ret = s_ins->client.realtime_client.dispatch_reply(des);
			else if (des->header.pid == s_ins->switch_ext.cid)
				ret = s_ins->client.switch_client.dispatch_reply(des);
			else
...
static int32_t start(void)
{
	...
	if (ret != thrd_success) {
#elif defined IPC_RTE_KERNEL
	data->route_task = kthread_run(router_func, NULL, "TimeSyncClient_thread");
...
TimeSyncClient_t *TimeSyncClient_init(TimeSyncClient_data_t *ins)
{
	...
	// init clients
	ret = safety_client_init(data, &ins->client.safety_client, &ins->safety_ext);
	if (ret < 0)
		return NULL;
	ret = realtime_client_init(data, &ins->client.realtime_client, &ins->realtime_ext);
	if (ret < 0)
		return NULL;
	ret = switch_client_init(data, &ins->client.switch_client, &ins->switch_ext);
	if (ret < 0)
		return NULL;
...
int32_t TimeSyncClient_destroy(void)
{
	...
	safety_client_destroy();
	realtime_client_destroy();
	switch_client_destroy();
	...
	ipc_memset(s_ins, 0, sizeof(TimeSyncClient_data_t));
	s_ins = NULL;
	return ret;
}
