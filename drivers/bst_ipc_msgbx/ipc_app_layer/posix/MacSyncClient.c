...
static MacSyncClient_data_t *s_ins;
...
static int32_t receive_message(void)
{
	...
	sw_macsync_client_ext_t *sw_macsync_ext = &s_ins->sw_macsync_ext;
	...
	if (ret == QUERY_INFO_DST_STS_OFFLINE) {
		if (sw_macsync_ext->avail_changed_cb)
			sw_macsync_ext->avail_changed_cb(false, sw_macsync_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_ONLINE) {
		if (sw_macsync_ext->avail_changed_cb)
			sw_macsync_ext->avail_changed_cb(true, sw_macsync_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_CHANGED) {
		ret = ipc_trans_layer_get_endmap(data->pid, &map);
		if (ret == 0) {
			chip_map = sw_macsync_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & sw_macsync_ext->cid_mask) != 0 ? true : false;
			if (status != sw_macsync_ext->status) {
				sw_macsync_ext->status = status;
				if (sw_macsync_ext->avail_changed_cb)
					sw_macsync_ext->avail_changed_cb(status, sw_macsync_ext->avail_ext);
			}
...
static int32_t dispatch_message(void)
{
	...
	while (packet_obj) {
		...
		// dispatch message
		ret = -1;
		if (des->header.pid == s_ins->sw_macsync_ext.cid) {
			if (des->header.typ == MSGBX_MSG_TYPE_BROADCAST)
				ret = s_ins->client.sw_macsync_client.dispatch_broadcast(des);
			else if (des->header.typ == MSGBX_MSG_TYPE_REPLY)
				ret = s_ins->client.sw_macsync_client.dispatch_reply(des);
		}
		if (ret < 0)
...
static int32_t start(void)
{
	...
	if (ret != 0) {
#elif defined IPC_RTE_RTOS
	data->route_task = TaskCreate((task_func_t)router_func, "MacSyncClient_thread", 0, NULL,
		0, (uint8_t *)stack_buffer, &tcb_buffer, true);
	if (data->route_task == 0) {
#elif defined IPC_RTE_KERNEL
	data->route_task = kthread_run(router_func, NULL, "MacSyncClient_thread");
...
MacSyncClient_t *MacSyncClient_init(MacSyncClient_data_t *ins)
{
	...
	// init clients
	ret = sw_macsync_client_init(data, &ins->client.sw_macsync_client, &ins->sw_macsync_ext);
	if (ret < 0)
		return NULL;

	// create client handle.
	ret = ipc_trans_layer_proxy_create_handle(data->pid, data->fid, data->sid, ins->sw_macsync_ext.cid, ins->sw_macsync_ext.ccid, &data->handle);
	if (ret < 0)
	{
