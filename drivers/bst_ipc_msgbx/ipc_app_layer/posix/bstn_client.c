...
avail_changed_callback_t s_avail_cb = NULL;
void *s_avail_ext = NULL;
...
static void disp_req_sync_callback(const uint32_t rep, void *ext) {
...
static inline int32_t serialize_disp_req(serdes_t *ser, const uint32_t req) {
  int32_t ret = 0;
  ser->header.pid = PID;
  ser->header.cid = CID;
  ser->header.fid = FID;
  ser->header.sid = SID;
  ser->header.cmd = CMD_METHOD_DISP_REQ;
  ser->header.typ = IPC_MSG_TYPE_METHOD;
  ret = ipc_ser_put(ser, (uint8_t *)&req, sizeof(uint32_t));
  if (ret < 0)
    return -1;
  return 0;
}
...
static int32_t call_disp_req_sync(const uint32_t req,
                                  const uint32_t *rep, int64_t timeout_ms) {
  int32_t ret = 0;
  serdes_t serdes = {0};
  ...
  ret = serialize_disp_req(&serdes, req);
  if (ret != 0) {
    pr_err("Serialize disp_req fail.\n");
    return -1;
  }
  ...
  (void)add_registry(reg, (void *)disp_req_sync_callback, (void *)&out);
  serdes.header.tok = tok;
  ...
  if (ret < 0) {
    clear_registry(reg);
    pr_err("send method disp_req fail %d.\n", ret);
    return ret;
  }
  // pr_info("call_disp_req_sync has send and begin to wait.");
...
static inline int32_t call_disp_req_callback(serdes_t *des) {
  ...
  // pr_info("call_disp_req_callback\n");

  ret = ipc_des_get(des, (uint8_t *)&rep, sizeof(int32_t));
  if (ret < 0) {
    return -1;
  }
  ...
  // pr_info("call_disp_run_callback reg->busy %d\n", reg->busy);
  if (reg->busy) {
    disp_req_callback_t cb = (disp_req_callback_t)(reg->cb);
    if (cb)
      cb(rep, reg->ext);
    notify_callback_registry(reg);
    // pr_info("call_disp_req_callback notify_callback_registry\n");
    clear_registry(reg);
  } else {
    pr_err("callback registry is invalid.\n");
  }
...
static int32_t dispatch_message(void) {
  ...
  while (ipc_trans_layer_proxy_get_reply_msg(PID, s_handle, &reply_des) >= 0) {
    ...
    case CMD_METHOD_DISP_REQ:
      ret = call_disp_req_callback(&reply_des);
      break;
    default:
      break;
    }
...
static int32_t start(void) {
  ...
  route_task = kthread_run(router_func, NULL, "bstn_client_thread");
...
bstn_client *bstn_client_init(void) {
  ...
  s_client.disp_req_sync = call_disp_req_sync;
  s_client.disp_req_async = call_disp_req_async;
...
EXPORT_SYMBOL(bstn_client_init);
...
int32_t bstn_client_destroy(void) {
  ...
  s_client.disp_req_sync = NULL;
  s_client.disp_req_async = NULL;
...
EXPORT_SYMBOL(bstn_client_destroy);
