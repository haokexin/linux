...
// macro definitions
#define CID REALTIME_0
#define MAJOR 1U
#define MINOR 0U


#define CMD_METHOD_REALTIME_TIMESYNC 1U


// local variables
static com_client_data_t *s_data;
static realtime_client_ext_t *s_ext;
...
static inline int32_t serialize_realtime_timesync(
				serdes_t *ser,
				const uint32_t sec,
				const uint32_t nsec
				)
...
static int32_t call_realtime_timesync_fire_and_forget(const uint32_t sec,
				const uint32_t nsec)
{
	int32_t ret = 0;
	...
	ret = serialize_realtime_timesync(ser, sec, nsec);
	if (ret != 0) {
	...
	// send request
	ret = send_fire_and_forget_request(data, ser, s_ext->cid, CMD_METHOD_REALTIME_TIMESYNC);
	if (ret < 0) {
...
// initialize client
int32_t realtime_client_init(com_client_data_t *data, realtime_client_t *client,
			realtime_client_ext_t *ext)
{
	...
	client->realtime_timesync_fire_and_forget = call_realtime_timesync_fire_and_forget;
