...
static void hello(const char *name, char **message, test_ErrorEnum_t *error, ext_info_t* info)
{
    if (name)
    {
...
static void complex_method(const uint32_t in1, const char * in2, const byte_buffer in3, const test_MyArray_t in4, const test_MyStruct_t in5, const test_MyUnion_t in6,
        uint32_t *out1, char **out2, byte_buffer *out3, test_MyArray_t *out4, test_MyStruct_t *out5, test_MyUnion_t *out6, test_ErrorEnum_t *error, ext_info_t* info)
{
...
static void on_heartbeat_sub(uint8_t pid, uint8_t fid, uint8_t sid)
{
    printf("Recevie heartbeat subscription: pid %d, fid %d, sid %d\n", pid, fid, sid);
}

static void on_heartbeat_unsub(uint8_t pid, uint8_t fid, uint8_t sid)
{
    printf("Recevie heartbeat unsubscription: pid %d, fid %d, sid %d\n", pid, fid, sid);
}

static int32_t test_ipc_sesrver_main(void *arg)
{
    int32_t ret = 0;
    s_server = test_server_init((rw_msg_header_t *)arg);
    if (!s_server)
    {
    ...
    // get version
    ipc_inf_version_t version = s_server->version();
    ...
    (void)s_server->register_heartbeat_subcribed(on_heartbeat_sub);
    (void)s_server->register_heartbeat_unsubcribed(on_heartbeat_unsub);
...
{
    return  kthread_create(test_ipc_sesrver_main, data, "ipc-msgbox-%u", smp_processor_id());
}
EXPORT_SYMBOL(start_server_test);
