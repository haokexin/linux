...
#ifndef DB_SWITCH_DATATYPE_H
#define DB_SWITCH_DATATYPE_H
...
// user defined types
struct _db_switch_MyArray_t {
	uint8_t *data;
	uint32_t size;
};
#define db_switch_MyArray_t struct _db_switch_MyArray_t

enum _db_switch_ErrorEnum_t {
	DB_SWITCH_NO_ERROR = 0,
	DB_SWITCH_SERVER_FAIL = -1,
	DB_SWITCH_ERROR_2 = -2,
	DB_SWITCH_ERROR_3 = -3
};
#define db_switch_ErrorEnum_t enum _db_switch_ErrorEnum_t
...
/**
 * Deserialize db_switch_MyArray_t.
 *
 ...
 * @return 0 if success, negative if fail.
 */
static inline int32_t deserialize_db_switch_MyArray(
							des_buf_t *buf,
							db_switch_MyArray_t *out)
{
	uint32_t size = 0;
...
/**
 * Serialize db_switch_ErrorEnum_t.
 *
 * @param ser Pointer to serdes_t, which stores the serialized data.
 * @param in The input data to be serialized.
 * @return 0 if success, negative if fail.
 */
static inline int32_t serialize_db_switch_ErrorEnum(
							serdes_t *ser,
							const db_switch_ErrorEnum_t *in)
{
	int32_t ret = 0;
...
/**
 * Deserialize db_switch_ErrorEnum_t.
 *
 * @param des Pointer to serdes_t, which stores the serialized data.
 * @param out The output of deserialized data.
 * @param buf The buffer used to make the deserialized objects.
 * @return 0 if success, negative if fail.
 */
static inline int32_t deserialize_db_switch_ErrorEnum(
							des_buf_t *buf,
							db_switch_ErrorEnum_t *out)
{
	uint32_t *ptr = NULL;
