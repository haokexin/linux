...
static void its_mask_irq(struct irq_data *d)
{
	...
	/*
	 * We don't need to do anything here, let the LPI always unmasked
...
static void gic_reset_prop_table(void *va)
{
	...
	memset(va, LPI_PROP_DEFAULT_PRIO | LPI_PROP_GROUP1 | LPI_PROP_ENABLED, LPI_PROPBASE_SZ);
...
{
	if (gic_rdists->use_static_table) {
		lpi_id_bits = min_t(u32,
				    GICD_TYPER_ID_BITS(gic_rdists->gicd_typer),
				    ITS_MAX_LPI_NRBITS);
		gic_rdists->prop_table_pa = gic_rdists->static_table_pa;
		gic_rdists->prop_table_va = memremap(gic_rdists->prop_table_pa,
						     LPI_PROPBASE_SZ, MEMREMAP_WB);
		gic_reset_prop_table(gic_rdists->prop_table_va);
	} else if (gic_rdists->flags & RDIST_FLAGS_RD_TABLES_PREALLOCATED) {
		u64 val;
...
static int __init allocate_lpi_tables(void)
{
	...
	if ((val & GICR_CTLR_ENABLE_LPIS) &&
	    (enabled_lpis_allowed() || gic_rdists->use_static_table)) {
	...
	for_each_possible_cpu(cpu) {
		...
		if (gic_rdists->use_static_table) {
			phys_addr_t pa;
			void *va;
			int cpu_phy_id = cpu_physical_id(cpu);
			cpu_phy_id = (cpu_phy_id >> 16 & 0xF) * 4 + (cpu_phy_id >> 8 & 0xF);
			/* Must set currect cluster core number */
			WARN_ON(!CONFIG_ARCH_BSTC1200);
			pa = gic_rdists->static_table_pa + LPI_PROPBASE_SZ
					 + cpu_phy_id * LPI_PENDBASE_SZ;
			pr_info("GICv3: find physical CPU:%d, use static pending table: %llx\n",
				cpu_phy_id, pa);
			va = memremap(pa, LPI_PENDBASE_SZ, MEMREMAP_WB);
			memset(va, 0, LPI_PENDBASE_SZ);
			gic_flush_dcache_to_poc(va, LPI_PENDBASE_SZ);
			pend_page = phys_to_page(pa);
		} else {
...
static int __init its_of_probe(struct device_node *node)
{
	...
	for (np = of_find_matching_node(node, its_device_id); np;
		...
		if (mem_node) {
			...
			if (!rmem) {
				pr_err("%pOF: no reserved-memory property?\n", np);
				continue;
			}

			if (!gic_rdists->use_static_table) {
				gic_rdists->use_static_table = true;
				phys_addr = rmem->base;
				size = rmem->size;
				pr_info("ITS use static table address: 0x%pa, size: 0x%zx\n", &phys_addr, size);
				gic_rdists->static_table_pa = phys_addr;
			}
		}
