...
static void hwcv_dc_prepare(void *buf_priv)
{
	...
	dev_dbg(buf->dev, "%s: dma_addr[0x%llx], dma_len[%u], dma_dir[%d]",
		__func__, sg_dma_address(sgt->sgl), sg_dma_len(sgt->sgl),
		buf->dma_dir);
}
...
static void hwcv_dc_put(void *buf_priv)
{
	...
	dev_dbg(buf->dev, "%s: ref[%d]", __func__,
		refcount_read(&buf->refcount));
	if (!refcount_dec_and_test(&buf->refcount))
		return;

	dev_dbg(buf->dev, "%s: dma_addr[0x%llx], size[%lu], ref[%d]", __func__,
...
static int hwcv_dc_mmap(void *buf_priv, struct vm_area_struct *vma)
{
	...
	if (ret) {
		dev_err(buf->dev, "Failed to dma mmap, error: %d\n", ret);
		return ret;
	}
				     ...
				     struct dma_buf_attachment *dbuf_attach)
{
	...
	dev_dbg(buf->dev, "%s: %s attach", __func__,
				      ...
				      struct dma_buf_attachment *db_attach)
{
	...
	dev_dbg(buf->dev, "%s: %s detach", __func__, dev_name(db_attach->dev));
		       ...
		       enum dma_data_direction dma_dir)
{
	...
	dev_dbg(buf->dev, "%s: %s map attach", __func__,
				     ...
				     enum dma_data_direction dma_dir)
{
	...
	dev_dbg(buf->dev, "%s: %s unmap attach", __func__,
		dev_name(db_attach->dev));
}
...
static int hwcv_dc_map_dmabuf(void *mem_priv)
{
	...
	dev_dbg(buf->dev, "%s: dma_addr[0x%llx], size[%lu]", __func__,
				   ...
				   enum dma_data_direction dma_dir)
{
	...
	dev_dbg(dev, "%s: dma_dir[%d]", __func__, buf->dma_dir);
