...
static int pci_epf_test_init_dma_chan(struct pci_epf_test *epf_test)
{
	...
	dw_edma_func_num_set(dma_chan, epf->func_no, epf->vfunc_no);
...
static void pci_epf_test_unbind(struct pci_epf *epf)
{
	...
	bst_pcie_ep_db_irq_free(epc, epf->func_no, epf->vfunc_no, epf_test->db_irq);
...
static int pci_epf_test_doorbell_handler(int irq, void *arg)
{
	struct pci_epf *epf = arg;

	pr_info("Doorbell interrupt received: func_no=%d, vfunc_no=%d, irq=%d\n",
		epf->func_no, epf->vfunc_no, irq);
	return IRQ_HANDLED;
}

static int pci_epf_test_get_doorbell(struct pci_epf *epf)
{
	struct device *dev = &epf->dev;
	struct pci_epc *epc = epf->epc;
	int irq, ret;
	u32 bar_no, offset, msg;

	irq = bst_pcie_ep_db_irq_alloc(epc, epf->func_no, epf->vfunc_no);
	if (irq < 0) {
		dev_err(dev, "Failed to allocate doorbell interrupt\n");
		return irq;
	}
	ret = bst_pcie_ep_db_irq_request(epc, epf->func_no, epf->vfunc_no,
					 irq, pci_epf_test_doorbell_handler, epf);
	if (ret) {
		dev_err(dev, "Failed to request doorbell interrupt\n");
		return ret;
	}
	ret = bst_pcie_ep_db_info_get(epc, epf->func_no, epf->vfunc_no,
			       irq, &bar_no, &offset, &msg);
	if (ret) {
		dev_err(dev, "Failed to get doorbell info\n");
		return ret;
	}
	pr_info("Doorbell info: bar_no=%d, offset=%x, msg=%x\n", bar_no, offset, msg);
	return 0;
}
