...
static int __dw_pcie_prog_outbound_atu(struct dw_pcie *pci, u8 func_no, u8 vfunc_no,
				       ...
				       u64 pci_addr, u64 size)
{
	...
	dw_pcie_writel_atu_ob(pci, index, PCIE_ATU_REGION_CTRL3, !vfunc_no ? 0 :
				      (PCIE_ATU_VF_ACTIVE | PCIE_ATU_VFUNC_NUM(vfunc_no - 1)));
...
{
	return __dw_pcie_prog_outbound_atu(pci, 0, 0, index, type,
					   cpu_addr, pci_addr, size);
}

int dw_pcie_prog_ep_outbound_atu(struct dw_pcie *pci, u8 func_no, u8 vfunc_no,
				 int index, int type, u64 cpu_addr, u64 pci_addr,
				 u64 size)
{
	return __dw_pcie_prog_outbound_atu(pci, func_no, vfunc_no, index, type,
					   cpu_addr, pci_addr, size);
}
			     ...
			     int index, int type, u64 cpu_addr, u8 bar)
{
	...
	if (vfunc_no)
		dw_pcie_writel_atu_ib(pci, index, PCIE_ATU_REGION_CTRL3, PCIE_ATU_VF_ACTIVE | PCIE_ATU_VFUNC_NUM(vfunc_no - 1));
	dw_pcie_writel_atu_ib(pci, index, PCIE_ATU_REGION_CTRL2,
			      PCIE_ATU_ENABLE | PCIE_ATU_FUNC_NUM_MATCH_EN | (vfunc_no ? PCIE_ATU_VF_MATCH_EN : 0) |
...
__maybe_unused static int dw_pcie_edma_irq_vector(struct device *dev, unsigned int nr)
{
...
__maybe_unused static int bst_dma_irq_vector(struct device *dev, unsigned int nr)
{
	struct platform_device *pdev = to_platform_device(dev);
	char dma_name[16] = {0};
	__maybe_unused unsigned int irq;
	...
	snprintf(dma_name, sizeof(dma_name), "dma%d", irq);
	return platform_get_irq_byname(pdev, dma_name);
}


static struct dw_edma_plat_ops dw_pcie_edma_ops = {
#ifdef CONFIG_ARCH_BSTC1200
	.irq_vector = bst_dma_irq_vector,
#else
...
static int dw_pcie_edma_find_chip(struct dw_pcie *pci)
{
	...
	#ifdef CONFIG_ARCH_BSTC1200
	pci->edma.ll_wr_cnt = 16;
	pci->edma.ll_rd_cnt = 16;
	#else
