...
static watchdogClient_data_t *s_ins;
...
static int32_t receive_message(void)
{
	...
	watchdog_client_ext_t *watchdog_ext = &s_ins->watchdog_ext;
	...
	if (ret == QUERY_INFO_DST_STS_OFFLINE) {
		if (watchdog_ext->avail_changed_cb)
			watchdog_ext->avail_changed_cb(false, watchdog_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_ONLINE) {
		if (watchdog_ext->avail_changed_cb)
			watchdog_ext->avail_changed_cb(true, watchdog_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_CHANGED) {
		ret = ipc_trans_layer_get_endmap(data->pid, &map);
		if (ret == 0) {
			chip_map = watchdog_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & watchdog_ext->cid_mask) != 0 ? true : false;
			if (status != watchdog_ext->status) {
				watchdog_ext->status = status;
				if (watchdog_ext->avail_changed_cb)
					watchdog_ext->avail_changed_cb(status, watchdog_ext->avail_ext);
			}
...
static int32_t dispatch_message(void)
{
	...
	while (packet_obj) {
		...
		// dispatch message
		ret = -1;
		if (des->header.pid == s_ins->watchdog_ext.cid) {
			if (des->header.typ == MSGBX_MSG_TYPE_BROADCAST)
				ret = s_ins->client.watchdog_client.dispatch_broadcast(des);
			else if (des->header.typ == MSGBX_MSG_TYPE_REPLY)
				ret = s_ins->client.watchdog_client.dispatch_reply(des);
		}
		if (ret < 0)
...
static int32_t start(void)
{
	...
	if (ret != 0) {
#elif defined IPC_RTE_RTOS
	data->route_task = TaskCreate((task_func_t)router_func, "watchdogClient_thread", 0, NULL,
		0, (uint8_t *)stack_buffer, &tcb_buffer, true);
	if (data->route_task == 0) {
#elif defined IPC_RTE_KERNEL
	data->route_task = kthread_run(router_func, NULL, "watchdogClient_thread");
...
#endif
// watchdogClient_init
watchdogClient_t *watchdogClient_init(watchdogClient_data_t *ins)
{
	int32_t ret = 0;
	...
	// init clients
	ret = watchdog_client_init(data, &ins->client.watchdog_client, &ins->watchdog_ext);
	if (ret < 0)
		return NULL;

	// create client handle.
	ret = ipc_trans_layer_proxy_create_handle(data->pid, data->fid, data->sid, ins->watchdog_ext.cid, ins->watchdog_ext.ccid, &data->handle);
	if (ret < 0)
	{
...
// watchdogClient_destroy
int32_t watchdogClient_destroy(void)
{
	int32_t ret = 0;
	com_client_data_t *data = (com_client_data_t *)s_ins;
