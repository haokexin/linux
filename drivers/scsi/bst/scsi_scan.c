		...
		unsigned int id, u64 lun, enum scsi_scan_mode rescan)
{
	...
	#ifdef CONFIG_BST_UFS_DEVICE_NUMBER
		device_number = CONFIG_BST_UFS_DEVICE_NUMBER;
	#endif
	if (shost->this_id == id)
		/*
	 ...
	 */
	if (strncmp(shost->hostt->name, "ufshcd", 6) == 0) {			//ufs device
		if (device_number == 10) {
			...
			res = scsi_probe_and_add_lun(starget, device_number, &bflags, NULL, rescan, NULL);
		}
	} else {														//usb device
		res = scsi_probe_and_add_lun(starget, 0, &bflags, NULL, rescan, NULL);
		if (res == SCSI_SCAN_LUN_PRESENT || res == SCSI_SCAN_TARGET_PRESENT) {
