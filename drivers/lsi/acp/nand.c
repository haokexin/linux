...
_READL(const char *file, int line, void *address)
{
	...
	value = readl((volatile void __iomem *)address);
...
static inline void
_WRITEL(const char *file, int line, unsigned long value, void *address)
{
		   ...
		   char **start, off_t offset, int count, int *eof, void *data)
{
	...
	for (ofs = 0; ofs < mtd->size; ofs += mtd->erasesize) {
		...
		if (!chip->bbt)
			isbad = chip->block_bad(mtd, ofs, 0);
		else
			isbad = nand_isbad_bbt(mtd, ofs, 1);

		if (0 != isbad)
			length += sprintf((page + length), "0x%08llx ", ofs);
	}
...
{
	/* Destroy /proc entry. */
	remove_proc_entry("driver/lsinand", NULL);
