...
#include <bst-plat/memory_layout.h>

static bool reserve_mem_fail;
static bool minidump_cblock;

#define KMSG_SIZE	(_AC(1, UL) << CONFIG_LOG_BUF_SHIFT)

static unsigned long record_size = KMSG_SIZE;

static char kmsg_save[KMSG_SIZE] = {0};
...
static char *mrdump_addr;
static char *mrdump_addr_start;
static size_t mrdump_size;

static bool dump_all_cpus = true;

static void mrdump_mini_build_elf_misc(void);

static void mrdump_save_addr(unsigned long addr, unsigned long size,
		bool check_page)
{
	if (check_page) {
		struct mm_struct *mm;
		...
		do {
			...
			if (pgd_none(pgd)) {
				pr_debug("can't assert 0x%lx due to invalid pgd\n", addr);
				return;
			}
			...
			p4d = READ_ONCE(*p4dp);
			if (p4d_none(p4d)) {
				pr_debug("can't assert 0x%lx due to invalid p4d\n", addr);
				return;
			}
			...
			pud = READ_ONCE(*pudp);
			if (pud_none(pud)) {
				pr_debug("can't assert 0x%lx due to invalid pud\n", addr);
				return;
			}
			...
			pmd = READ_ONCE(*pmdp);
			if (pmd_none(pmd)) {
				pr_debug("can't assert 0x%lx due to invalid pmd\n", addr);
				return;
			}
			...
			pte = READ_ONCE(*ptep);
			if (pte_none(pte)) {
				pr_debug("can't assert 0x%lx due to invalid pte\n", addr);
				return;
			}
			pte_unmap(ptep);
		} while (0);
	...
	if (mrdump_size + size > MRDUMP_SIZE) {
		pr_err("no enough space to save 0x%lx\n", addr);
		return;
	}
#ifdef CONFIG_KASAN
	if ((addr >= KASAN_SR_START && addr <= KASAN_SR_END) ||
	    ((addr + size) >= KASAN_SR_START && ((addr + size) <= KASAN_SR_END)))
		return;
#endif //CONFIG_KASAN
	memcpy(mrdump_addr, (void *)addr, size);
	mrdump_addr += size;
	mrdump_size += size;
}

static void mrdump_save_elf_header(void *addr, unsigned long size)
{
	memcpy(mrdump_addr_start, addr, size);
}
...
__weak void get_kernel_log_buffer(unsigned long *addr, unsigned long *size,
		unsigned long *start)
{
	*addr = (unsigned long)kmsg_save;
	*size = record_size;
	*start = (unsigned long)kmsg_save;
}

__weak void get_kernel_conf_buffer(unsigned long *addr, unsigned long *size,
		unsigned long *start)
{
#ifdef CONFIG_IKCONFIG_PROC
	*addr = (unsigned long)&kernel_config_data;
	*size = (unsigned long) (&kernel_config_data_end - &kernel_config_data);
	*start = (unsigned long)&kernel_config_data;
#endif
}

__weak void get_timekeeper(unsigned long *addr, unsigned long *size,
		unsigned long *start)
{
	struct timekeeper *shadow_tk = ktime_get_shadow_timekeeper();
	*addr = (unsigned long)shadow_tk;
	*size = (unsigned long)sizeof(struct timekeeper);
	*start = (unsigned long)shadow_tk;
}
		...
		unsigned long size, unsigned long start, char *name)
{
	...
	for (i = 0; i < MRDUMP_MINI_NR_MISC; i++) {
		...
		mrdump_save_addr(va, size, false);
		break;
	}
}
		...
		unsigned long pa)
{
	...
	if (i < MRDUMP_MINI_NR_SECTION) {
		...
		if (minidump_cblock)
			mrdump_save_addr(start, end - start, true);
	}
}
		...
		struct task_struct *tsk, int stack)
{
	...
	if (!mrdump_virt_addr_valid(tsk)) {
		pr_err("mrdump: cpu:[%d] invalid task pointer:%p\n", cpu, tsk);
		if (cpu < num_possible_cpus())
			tsk = cpu_curr(cpu);
		else
			pr_err("mrdump: cpu:[%d] overflow with total:%d\n",
					cpu, num_possible_cpus());
	}
	if (!mrdump_virt_addr_valid(tsk))
		pr_err("mrdump: cpu:[%d] CAN'T get a valid task pointer:%p\n",
				cpu, tsk);
	else
	...
	pr_emerg("mrdump: cpu[%d] tsk:%p ti:%p\n", cpu, tsk, ti);
	if (!stack)
		return;
...
#define MAX_STACK_TRACE_DEPTH 32
	...
	if (!mrdump_virt_addr_valid(current_thread_info())) {
		pr_err("current thread info invalid\n");
		return;
	}
	cur = current;
	tsk = cur;
	...
	do {
		...
		sz += snprintf(symbol + sz, SYMBOL_SIZE - sz, "[%s, %d]", tsk->comm,
				tsk->pid);
		previous = tsk;
		tsk = tsk->real_parent;
		if (!mrdump_virt_addr_valid(tsk)) {
			pr_err("tsk(%p) invalid (previous: [%s, %d])\n", tsk,
					previous->comm, previous->pid);
			break;
		}
	} while (tsk && (tsk->pid != 0) && (tsk->pid != 1));
	if (!strncmp(cur_proc->process_path, symbol,
	    min(sz, SYMBOL_SIZE))) {
		pr_err("same process path\n");
		return;
	}

	memset_io(cur_proc, 0, sizeof(struct aee_process_info));
	memcpy(cur_proc->process_path, symbol, min(sz, AEE_PROCESS_NAME_LENGTH));
	...
	stack_trace_save_tsk(cur, (unsigned long *)&trace, MAX_STACK_TRACE_DEPTH, 0);
#endif
	/* Skip the entries -
	...
	for (i = 0; i < trace.nr_entries; i++) {
		...
		if (plen > 16) {
			sz = snprintf(symbol, SYMBOL_SIZE, "[<%p>] %pS\n",
...
int mrdump_modules_info(unsigned char *buffer, size_t sz_buf)
{
	...
	if (sz_buf < MODULES_INFO_BUF_SIZE || buffer == NULL)
		return -1;
...
{
	pr_err("mrdump: hang data 0x%lx size:0x%lx\n", vaddr, size);
...
void mrdump_mini_add_extra_misc(void)
{
	...
	if (once == 0) {
		...
		for (i = 0; i < EXTRA_TOTAL_NUM; i++) {
			...
			if (ret < 0)
				pr_err("mrdump: add %s:0x%lx sz:0x%lx failed\n",
					extra_members[i].dump_name,
					vaddr, size);
		}
	}
...
void mrdump_mini_ke_cpu_regs(struct pt_regs *regs)
{
	...
	if (minidump_cblock) {
		mrdump_save_addr(
		  ...
		  (unsigned long)mrdump_sram_cb.size,
		  true);
	...
	memcpy(mrdump_addr_start, &mrdump_size, sizeof(size_t));
	mrdump_addr_start += sizeof(size_t);
	mrdump_save_elf_header(mrdump_mini_ehdr, MRDUMP_MINI_BUF_SIZE);
}
...
#ifdef CONFIG_IKCONFIG_PROC
	memset_io(&misc, 0, sizeof(struct mrdump_mini_elf_misc));
	get_kernel_conf_buffer(&misc.vaddr, &misc.size, &misc.start);
	mrdump_mini_add_misc(misc.vaddr, misc.size, misc.start, "_KERNEL_CONF_");
#endif

	memset_io(&misc, 0, sizeof(struct mrdump_mini_elf_misc));
	get_timekeeper(&misc.vaddr, &misc.size, &misc.start);
	mrdump_mini_add_misc(misc.vaddr, misc.size, misc.start, "_TIME_KEEPER_");
...
static void mrdump_mini_add_loads(void)
{
	...
	for_each_online_cpu(id) {
		...
		tsk = (prstatus->common.pr_sigpend != 0) ?
			(struct task_struct *)prstatus->common.pr_sigpend : current;
		memcpy(&regs, &prstatus->pr_reg, sizeof(prstatus->pr_reg));
		if (prstatus->common.pr_pid >= 100) {
			for (i = 0; i < ELF_NGREG; i++)
				mrdump_mini_add_entry(
						((unsigned long *)&regs)[i],
						MRDUMP_MINI_SECTION_SIZE);
			cpu = prstatus->common.pr_pid - 100;
			...
			pr_err("mrdump: wrong pr_pid: %d\n", prstatus->common.pr_pid);
		}
	}
...
static void __init mrdump_mini_elf_header_init(void)
{
	if (MRDUMP_MINI_BUF_PADDR)
		...
		mrdump_mini_ehdr = kmalloc(MRDUMP_MINI_HEADER_SIZE + TASK_INFO_SIZE +
					PSTORE_SIZE, GFP_KERNEL);
	if (!mrdump_mini_ehdr)
		return;
	pr_debug("mirdump: reserved 0x%x+0x%lx->0x%p", MRDUMP_MINI_BUF_PADDR,
...
static bool mrdump_mini_reserve_mem_init(void)
{
	mrdump_addr = (char *)memremap(CONFIG_BST_MRDUMP_PHYS_BASE_ADDR,
			MRDUMP_SIZE, MEMREMAP_WB);
	if (!mrdump_addr) {
		pr_err("can't get valid virtual address for 0x%lx\n",
				CONFIG_BST_MRDUMP_PHYS_BASE_ADDR);
		return false;
	}
	mrdump_addr_start = mrdump_addr;

	memset(mrdump_addr, 0, MRDUMP_SIZE);
	/*
	 * The first 8 bytes is used for saving the total mrdump_size.
	 * The next MRDUMP_MINI_BUF_SIZE bytes is used for save the
	 * ELF header.
	 * The remainder reserved memory is used for saving core info.
	 */
	mrdump_addr += sizeof(size_t) + MRDUMP_MINI_BUF_SIZE;
	mrdump_size += MRDUMP_MINI_BUF_SIZE + sizeof(size_t);
 ...
 * end of the buffer.
 */
static void mrdump_dump(struct kmsg_dumper *dumper,
			enum kmsg_dump_reason reason)
{
	...
	if (unlikely(reason == KMSG_DUMP_OOPS))
		return;

	why = kmsg_dump_reason_str(reason);
	pr_info("dump reason : %s\n", why);

	memset(kmsg_save, 0, KMSG_SIZE);
	kmsg_dump_rewind(&iter);

	kmsg_get_ret = kmsg_dump_get_buffer(&iter, true, kmsg_save,
						KMSG_SIZE, &dump_size);
	if (kmsg_get_ret)
		record_size = dump_size;
}

static struct kmsg_dumper mrdump_dumper = {
	.dump = mrdump_dump,
};
 ...
 */
static void mrdump_register_kmsg(void)
{
	kmsg_dump_register(&mrdump_dumper);
}
...
int mrdump_mini_init(void)
{
	...
	if (reserve_mem_fail) {
		pr_err("minidump wrong reserved memory\n");
		WARN_ON(reserve_mem_fail);
	}

	if (!mrdump_mini_reserve_mem_init())
		return -EINVAL;

	mrdump_mini_elf_header_init();

	mrdump_register_kmsg();
...
static int __init mini_rdump_reserve_memory(struct reserved_mem *rmem)
{
	...
	if (rmem->base != KERN_MINIDUMP_BASE ||
		...
		pr_err("minidump: Check the reserved address(0x%llx) and size(0x%llx)\n",
			rmem->base, rmem->size);
		return -EINVAL;
	}
#endif
...
RESERVEDMEM_OF_DECLARE(reserve_memory_minidump, "bst,minidump",
