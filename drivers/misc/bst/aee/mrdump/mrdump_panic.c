...
static bool panic_die_en;
static char *panic_str = "Kernel Panic";
static int reboot_reason = -1;
static int glb_fiq_step = -1;
static struct pt_regs *cur_regs;
...
#ifdef CONFIG_BST_RAM_CONSOLE
	glb_fiq_step = glb_fiq_step < 0 ? AEE_FIQ_STEP_KE_IPANIC_START : glb_fiq_step;
#endif
	reboot_reason = reboot_reason < 0 ? AEE_REBOOT_MODE_KERNEL_PANIC : reboot_reason;
	if (unlikely(!cur_regs)) {
		mrdump_mini_save_regs(&saved_regs);
		cur_regs = &saved_regs;
	}
	return mrdump_common_die(glb_fiq_step, reboot_reason, panic_str, cur_regs);
}
...
#ifdef CONFIG_BST_RAM_CONSOLE
	glb_fiq_step = glb_fiq_step < 0 ? AEE_FIQ_STEP_KE_IPANIC_DIE : glb_fiq_step;
#endif
	panic_str = "Kernel Oops";
	panic_die_en = true;
	cur_regs = dargs->regs;
	reboot_reason = AEE_REBOOT_MODE_KERNEL_OOPS;
...
static int __init mrdump_panic_init(void)
{
	...
	mrdump_full_init();
#ifdef CONFIG_BST_AEE_MRDUMP_WDT
	mrdump_wdt_init();
#endif
...
inline void aee_print_bt(struct pt_regs *regs)
{
	...
	bottom = regs->reg_sp;
#else
	bottom = thread_saved_fp(current);
#endif
...
#else
	cur_frame.fp = thread_saved_fp(current);
#endif
	cur_frame.pc = regs->pc;
	cur_frame.sp = regs->sp;
	aee_nested_printf("\n[<%p>] %pS\n", (void *)cur_frame.pc,
			(void *)cur_frame.pc);
...
#else
	aee_nested_printf("bst watchdog not enable.\n");
#endif
