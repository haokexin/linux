...
#ifdef CONFIG_BST_SCHED_MONITOR
#include "bst_sched_mon.h"
#endif
#include <linux/io.h>
...
#include <bst-plat/bst_secure_api.h>
#ifdef CONFIG_BST_EIC_HISTORY_DUMP
#include <linux/irqchip/bst-eic.h>
#endif
...
/* AEE_BST_CPU_NUMS may not eaqual to real cpu numbers,
 * alloc buffer at initialization
 */
static char *wdt_percpu_log_buf[AEE_BST_CPU_NUMS];
static int wdt_percpu_log_length[AEE_BST_CPU_NUMS];
static char wdt_log_buf[WDT_LOG_DEFAULT_SIZE];
static int wdt_percpu_preempt_cnt[AEE_BST_CPU_NUMS];
static unsigned long
wdt_percpu_stackframe[AEE_BST_CPU_NUMS][MAX_EXCEPTION_FRAME];
static int wdt_log_length;
static atomic_t wdt_enter_fiq;
static char str_buf[AEE_BST_CPU_NUMS][PRINTK_BUFFER_SIZE];

#ifndef CONFIG_BST_RAM_CONSOLE
...
static struct stacks_buffer stacks_buffer_bin[AEE_BST_CPU_NUMS];
...
static struct regs_buffer regs_buffer_bin[AEE_BST_CPU_NUMS];
...
#ifdef CONFIG_BST_RAM_CONSOLE
#ifndef CONFIG_BST_WATCHDOG
	if (bst_rgu_status_is_sysrst() || bst_rgu_status_is_eintrst()) {
#ifdef CONFIG_BST_PMIC_COMMON
...
int __init mrdump_wdt_init(void)
{
	...
	atf_aee_debug_phy_addr = (phys_addr_t) (0x00000000FFFFFFFFULL &
					bst_secure_call(BST_SIP_KERNEL_WDT,
