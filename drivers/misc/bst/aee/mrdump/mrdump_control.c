...
static void mrdump_cblock_kallsyms_init(struct mrdump_ksyms_param *kparam)
{
	...
	if (!IS_ENABLED(CONFIG_KALLSYMS_BASE_RELATIVE))
		WARN_ON(!kallsyms_addresses);

	if (!kallsyms_addresses || !kallsyms_addresses[0]) {
		pr_err("invalid kallsyms_addresses\n");
		return;
	}
...
RESERVEDMEM_OF_DECLARE(reserve_memory_mrdump_cb, "bst,mrdump_cb",
		       mrdump_cb_reserve_memory);
