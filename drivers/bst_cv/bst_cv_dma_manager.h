...
#ifndef _BST_CV_DMA_MANAGER_H_
#define _BST_CV_DMA_MANAGER_H_
...
struct bst_cv_vmarea_handler {
	refcount_t *refcount;
	void (*put)(void *arg);
	void  *arg;
};

typedef struct bst_cv_memblock {
	struct device *dev;
	void *pbst_cv;
	void *vaddr;
	union {
	...
	/* MMAP related */
	struct bst_cv_vmarea_handler handler;
...
struct bst_cv_dma_ops {
	void *(*alloc) (struct device *dev,
					unsigned long attrs,
					unsigned long size,
					enum dma_data_direction dma_dir,
					gfp_t gfp_flags);
...
extern const struct bst_cv_dma_ops bst_cv_dma_memops;
