...
#include "uapi/gpu/bst/mali_r45/midgard/mali_kbase_mem_profile_debugfs_buf_size.h"
...
#include <uapi/gpu/bst/mali_r45/midgard/mali_kbase_ioctl.h>
#if !MALI_USE_CSF
...
static struct reset_control *rst_contrl;
...
static int kbase_platform_device_probe(struct platform_device *pdev)
{
	...
	rst_contrl = devm_reset_control_get_optional_exclusive(&pdev->dev, NULL);
	if (IS_ERR(rst_contrl)) {
		dev_err(&pdev->dev, "GPU failed to obtain reset information\n");
		return -ENODEV;
	}
