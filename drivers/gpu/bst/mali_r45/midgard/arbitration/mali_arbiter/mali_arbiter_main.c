		...
		u32 slices, u32 *old_slices)
{
	...
	if (!arb->rg_ops || !arb->rg_ops->get_slice_mask) {
		dev_err(arb->dev,
			"NULL function pointers for Resource Group.\n");
		return -EINVAL;
	}
...
static void grant_gpu_to_next_locked(struct mali_arb_partition *partition)
{
	...
	if (partition->active_vm) {
		...
		if (arb_lost_test_time_ms != 0)
			queue_delayed_work(partition->wq,
					&partition->aggr_lost_work,
					usecs_to_jiffies(
						arb_lost_test_time_ms));
 ...
 */
static inline void reset_utilisation_counters_locked(
			struct mali_arb_partition *partition, ktime_t t)
{
	lockdep_assert_held(&partition->mutex);
	...
	struct resource_interfaces res)
{
	...
	if (res.vm_assign) {
		...
		if (!rg_ops->get_slice_mask || !rg_ops->get_aw_mask) {
			dev_err(arb->dev,
				"NULL function pointers in rg_ops\n");
			return -EINVAL;
		}
