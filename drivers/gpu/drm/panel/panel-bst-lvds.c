...
// bst-added
static const struct drm_display_mode bst_evm_direct_lvds_modes[] = {
	{
		.clock = 148500,
		.hdisplay = 1920,
		.hsync_start = 1920 + 88,
				...
				struct drm_connector *connector)
{
	...
	unsigned int i,mode_num=0;
	for (i = 0; i < ARRAY_SIZE(bst_evm_direct_lvds_modes); i++) {
		if (bst_evm_direct_lvds_modes[i].clock){
			mode = drm_mode_create(connector->dev);
			if (!mode)
					return 0;
			memcpy(mode, &bst_evm_direct_lvds_modes[i], sizeof(*mode));
...
static const struct of_device_id panel_lvds_of_table[] = {
	...
	{
		.compatible = "bst,lvds1-panel",
	},
	{
		.compatible = "bst,lvds0-panel",
	},
	{ /* Sentinel */ },
};
