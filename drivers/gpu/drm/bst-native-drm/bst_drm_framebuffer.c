...
static int
bst_fb_afbc_size_check(struct bst_fb *kfb, struct drm_file *file,
...
static int
bst_fb_none_afbc_size_check(struct bst_dev *mdev, struct bst_fb *kfb,
		 ...
		 const struct drm_mode_fb_cmd2 *mode_cmd)
{
	...
	kfb->format_caps = bst_get_format_caps(&mdev->fmt_tbl,
	...
	if (kfb->base.modifier)
		...
		ret = bst_fb_none_afbc_size_check(mdev, kfb, file, mode_cmd);
	if (ret < 0)
		goto err_cleanup;
...
{
	struct drm_framebuffer *fb = &kfb->base;
	struct bst_dev *mdev = fb->dev->dev_private;
	u32 fourcc = fb->format->format;
	u64 modifier = fb->modifier;
	bool supported;

	supported = bst_format_mod_supported(&mdev->fmt_tbl, layer_type,
						fourcc, modifier, rot);
	if (!supported)
