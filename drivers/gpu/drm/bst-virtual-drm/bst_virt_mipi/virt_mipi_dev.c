...
#include "virt_mipi_dev.h"

static void virt_mipi_cleanup(struct bst_virt_device *vdev)
{
	struct virt_mipi_dev *mipi = vdev->virt_dev_data;
	if (!mipi)
		return;
...
static int virt_mipi_probe(struct bst_virt_device *vdev)
{
	struct virt_mipi_dev *mipi_dev;
	struct bst_display_submodule_header submodule_head = { 0 };
	struct bst_display_submodule_req submodule_req = { 0 };
	struct bst_display_mipi_probed_info *mipi_probed_info = vdev->dev_info.private;
	u32 i = 0, subdev_session;
	int err;

	mipi_dev = devm_kzalloc(vdev->dev, sizeof(*mipi_dev), GFP_KERNEL);
	if (!mipi_dev)
		return -ENOMEM;
	vdev->virt_dev_data = mipi_dev;
	mipi_dev->base_dev = vdev;
	subdev_session = vdev->subdev_session;

	if (mipi_probed_info->num_submodules > SUBMODULE_ID_MIPI_MAX)
		return -EINVAL;

	while (i < mipi_probed_info->num_submodules) {
		memset(&submodule_head, 0, sizeof(submodule_head));
		memset(&submodule_req, 0, sizeof(submodule_req));
		submodule_req.submodule_id = mipi_probed_info->submodule_ids[i];
		err = bst_display_conn_cmd_probe_submodule(subdev_session, &submodule_req, &submodule_head);
		if (err) {
			DRM_ERROR("probe mipi submodules failed.\n");
			goto err_cleanup;
		}

		if (submodule_req.submodule_id ==
			SUBMODULE_INFO_SUBMODULE_ID(submodule_head.submodule_info)) {
			err = virt_mipi_init_submodule(mipi_dev, &submodule_head);
			if (err)
				goto err_cleanup;
		}
		i++;
...
err_cleanup:
	virt_mipi_cleanup(vdev);
	return err;
}

static int virt_mipi_update(struct bst_virt_device *virt_dev, void *properties)
{
	return 0;
}

static void virt_mipi_flush(struct bst_virt_device *virt_dev)
{
}

static const struct bst_virt_device_funcs virt_mipi_dev_funcs = {
	.probe = virt_mipi_probe,
	.cleanup = virt_mipi_cleanup,
	.update = virt_mipi_update,
	.flush = virt_mipi_flush,
};

static uint32_t to_fw_mipi_device_type(uint32_t virt_drm_dev_type)
{
	uint32_t fw_mipi_dev_type;

	switch (virt_drm_dev_type) {
	case DEVICE_TYPE_VIRT_DSI0:
		fw_mipi_dev_type = BST_SUBDEV_DSI0;
		break;
	case DEVICE_TYPE_VIRT_DSI1:
		fw_mipi_dev_type = BST_SUBDEV_DSI1;
		break;
	default:
		fw_mipi_dev_type = BST_SUBDEV_INVAL;
	}

	return fw_mipi_dev_type;
}

static const struct bst_virt_device_funcs *
virt_mipi_identify(struct device *dev, struct bst_virt_platform_info *plat_info,
		   struct bst_virt_device_info *dev_info)
{
	...
	if (sizeof(*probed_info) > SUBDEV_PROBE_INFO_MAX_SIZE) {
		DRM_ERROR("err! mipi probe info size[%d] > max_size[%d].\n",
	...
	request.want_subdev = to_fw_mipi_device_type(plat_info->device_type);
	...
	if (!ret && response.base.status == DISP_COMM_REPLAY_OK) {
		...
		//if (dev_info->is_owner_device)
			return &virt_mipi_dev_funcs;
		//else
		//	return &virt_shared_conn_dev_funcs;
	}

	DRM_ERROR("virt_mipi_identify return null!\n");
	return NULL;
}

void bst_virt_mipi_destroy(struct bst_virt_device *vdev)
{
...
bst_virt_dsi_create(struct device *dev,
		     ...
		     struct bst_virt_pipe *pipe)
{
	...
	vdev->funcs = virt_mipi_identify(dev, plat_info, &vdev->dev_info);
	if (!vdev->funcs) {
		DRM_ERROR("Failed to identify mipi the HW.\n");
		err = -ENODEV;
		goto err_cleanup;
	}
	vdev->dev = dev;
	...
	DRM_INFO("Found BST-MIPI-%x, Device Session:%x, Device Role:%s\n",
	...
	if (err) {
		DRM_ERROR("enumerate mipi resource failed.\n");
		goto err_cleanup;
	}
	return vdev;

err_cleanup:
	bst_virt_mipi_destroy(vdev);
	return ERR_PTR(err);
}
