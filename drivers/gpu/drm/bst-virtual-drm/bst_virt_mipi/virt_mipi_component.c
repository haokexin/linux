...
static void mipi_enable(struct bst_virt_component *c)
{
	...
	struct bst_display_mipi_set_video_req video_req = {
	...
	ret = bst_display_mipi_cmd_set_video_stream(c->subdev_session, &video_req, &reply);
	if (ret) {
...
static void mipi_disable(struct bst_virt_component *c)
{
	...
	submodule_dis.submodule_type = MIPI_SUBMODULE_TYPE_VIDEO;
	submodule_dis.submodule_id = SUBMODULE_ID_MIPI_VIDEO;

	ret = bst_display_mipi_cmd_disable_submodule(subdev_session, &submodule_dis, &reply);
...
static int mipi_get_modes(struct bst_virt_component *c)
{
	...
	ret = bst_display_glb_cmd_get_cur_video_mode(&hw_mipi_req, &vm_info);
	if (ret) {
		       ...
		       struct bst_display_submodule_header *submodule)
{
	...
	uint32_t client_id = mipi->base_dev->dev_info.client_id;
	struct bst_display_mipi_info hw_mipi_info = { 0 };
	struct bst_display_mipi_req hw_mipi_req = { 0 };
	struct bst_display_mipi_probed_info *probed_info =(struct bst_display_mipi_probed_info *)mipi->base_dev->dev_info.private;
	...
	comp = bst_virt_component_add(mipi->base_dev->this_pipe, mipi->base_dev,
				      ...
				      &mipi_funcs, 0, 1, 1, client_id, "VIRT_MIPI-%d",
				      mipi_id);
	if (IS_ERR(comp)) {
	...
	hw_mipi_req.client_id = mipi->base_dev->dev_info.client_id;
	ret = bst_display_mipi_cmd_get_info(subdev_session, &hw_mipi_req,
				       &hw_mipi_info);
	if (ret) {
