...
static void bst_fb_destroy(struct drm_framebuffer *fb)
{
...
static int bst_fb_create_handle(struct drm_framebuffer *fb,
				struct drm_file *file, u32 *handle)
{
	return drm_gem_handle_create(file, fb->obj[0], handle);
}

static const struct drm_framebuffer_funcs bst_fb_funcs = {
	.destroy = bst_fb_destroy,
	.create_handle = bst_fb_create_handle,
};

static int bst_fb_afbc_size_check(struct bst_fb *kfb, struct drm_file *file,
				  const struct drm_mode_fb_cmd2 *mode_cmd)
{
	...
	case AFBC_FORMAT_MOD_BLOCK_SIZE_16x16:
		...
		break;
	default:
		DRM_WARN("Invalid AFBC_FORMAT_MOD_BLOCK_SIZE: %lld.\n",
	...
	bpp = bst_get_afbc_format_bpp(info, fb->modifier);
...
static int
bst_fb_none_afbc_size_check(struct bst_super_device *sdev,
				       struct bst_fb *kfb,
				       ...
				       const struct drm_mode_fb_cmd2 *mode_cmd)
{
	...
	if (bst_fb_check_src_coords(kfb, 0, 0, fb->width, fb->height))
		return -EINVAL;
	...
	for (i = 0; i < info->num_planes; i++) {
		...
		if ((fb->pitches[i] * block_h) % sdev->bus_width) {
			DRM_DEBUG_KMS("Pitch[%d]: 0x%x doesn't align to 0x%x\n",
				      i, fb->pitches[i], sdev->bus_width);
			return -EINVAL;
		}

		min_size = bst_fb_get_pixel_addr(kfb, 0, fb->height, i) -
...
struct drm_framebuffer *bst_fb_create(struct drm_device *dev,
				      struct drm_file *file,
				      const struct drm_mode_fb_cmd2 *mode_cmd)
{
	...
	kfb->format_caps = bst_get_format_caps(
		&sdev->fmt_tbl, mode_cmd->pixel_format, mode_cmd->modifier[0]);
	if (!kfb->format_caps) {
	...
	if (kfb->base.modifier)
		ret = bst_fb_afbc_size_check(kfb, file, mode_cmd);
	else
		ret = bst_fb_none_afbc_size_check(sdev, kfb, file, mode_cmd);
	if (ret < 0)
		goto err_cleanup;

	ret = drm_framebuffer_init(dev, &kfb->base, &bst_fb_funcs);
	if (ret < 0) {
...
int bst_fb_check_src_coords(const struct bst_fb *kfb, u32 src_x, u32 src_y,
			    u32 src_w, u32 src_h)
{
...
dma_addr_t bst_fb_get_pixel_addr(struct bst_fb *kfb, int x, int y, int plane)
{
	struct drm_framebuffer *fb = &kfb->base;
...
void* bst_fb_get_pixel_vaddr(struct bst_fb *kfb, int x, int y, int plane)
{
	struct drm_framebuffer *fb = &kfb->base;
	const struct drm_gem_dma_object *obj;
	u32 offset, plane_x, plane_y, block_w, block_sz;
	...
	return (void *)(obj->vaddr + offset);
}

bool bst_fb_is_layer_supported(struct bst_fb *kfb, u32 layer_type, u32 rot)
{
	struct drm_framebuffer *fb = &kfb->base;
	struct bst_super_device *sdev = fb->dev->dev_private;
	u32 fourcc = fb->format->format;
	u64 modifier = fb->modifier;
	bool supported;

	supported = bst_format_mod_supported(&sdev->fmt_tbl, layer_type, fourcc,
					     modifier, rot);
	if (!supported)
