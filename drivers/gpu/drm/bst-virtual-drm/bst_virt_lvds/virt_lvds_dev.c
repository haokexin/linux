...
static int virt_lvds_probe(struct bst_virt_device *vdev)
{
	...
	while (i < lvds_probed_info->num_submodules) {
		...
		err = bst_display_lvds_cmd_probe_submodule(subdev_session, &submodule_req, &submodule_head);
		if (err) {
		     ...
		     struct bst_virt_pipe *pipe)
{
	...
	DRM_INFO("Found BST-LVDS-%x, Client ID:0x%x, Device Session:%x, Device Role:%s\n",
