			  ...
			  uint32_t *usr_param, uint32_t size_param)
{
	...
	msg_data->cmdset = BST_DISPLAY_DSI_SUBDEV;
...
int bst_display_mipi_cmd_probe_submodule(uint32_t mipi_session,
			     ...
			     struct bst_display_submodule_header *submodule_head)
{
	...
	build_command(&msg_data, MIPI_CMD_PROBE_SUBMODULE, submodule_req->client_id,
		      mipi_session, (uint32_t *)submodule_req, sizeof(*submodule_req));
	transfer_fw_msg(&msg_data);

	return build_reply(&msg_data, (uint32_t *)submodule_head, submodule_head_size);
}

int bst_display_mipi_cmd_disable_submodule(uint32_t mipi_session,
				...
				struct bst_display_comm_reply* reply)
{
	...
	build_command(&msg_data, MIPI_CMD_DISABLE_SUBMODULE, submodule_dis->client_id,
		      mipi_session, (uint32_t *)submodule_dis, sizeof(*submodule_dis));
...
int bst_display_mipi_cmd_get_info(uint32_t mipi_session,
			     struct bst_display_mipi_req *req,
			     struct bst_display_mipi_info *info)
{
	struct fw_msg_data msg_data;
	uint32_t reply_size = sizeof(*info);

	if (reply_size > MAX_ACK_DATA)
		return -1;

	build_command(&msg_data, MIPI_CMD_GET_INFO, req->client_id,
		      mipi_session, (uint32_t *)req, sizeof(*req));
...
int bst_display_mipi_cmd_set_video_stream(uint32_t mipi_session,
				   ...
				   struct bst_display_comm_reply* reply)
{
	...
	build_command(&msg_data, MIPI_CMD_SET_VIDEO_STREAM, req->client_id, mipi_session,
		      (uint32_t *)req, sizeof(*req));
	transfer_fw_msg(&msg_data);

	return build_reply(&msg_data, (uint32_t *)reply, sizeof(*reply));
}

int bst_display_mipi_cmd_dump_debug_info(uint32_t mipi_session,
				...
				struct bst_display_comm_reply* reply)
{
	...
	build_command(&msg_data, MIPI_CMD_DUMP_DEBUG_INFO, dump_cfg->client_id,
			mipi_session, (uint32_t *)dump_cfg, sizeof(*dump_cfg));
