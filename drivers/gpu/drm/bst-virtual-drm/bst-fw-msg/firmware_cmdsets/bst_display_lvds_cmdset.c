			  ...
			  uint32_t *usr_param, uint32_t size_param)
{
	...
	msg_data->cmdset = BST_DISPLAY_LVDS_SUBDEV;
...
int bst_display_lvds_cmd_probe_submodule(uint32_t lvds_session,
			     ...
			     struct bst_display_submodule_header *submodule_head)
{
	...
	build_command(&msg_data, LVDS_CMD_PROBE_SUBMODULE, submodule_req->client_id,
		      lvds_session, (uint32_t *)submodule_req, sizeof(*submodule_req));
				...
				struct bst_display_comm_reply* reply)
{
	...
	build_command(&msg_data, LVDS_CMD_DISABLE_SUBMODULE, submodule_dis->client_id,
		      lvds_session, (uint32_t *)submodule_dis, sizeof(*submodule_dis));
...
int bst_display_lvds_cmd_get_info(uint32_t lvds_session,
			     ...
			     struct bst_display_lvds_info *info)
{
	...
	build_command(&msg_data, LVDS_CMD_GET_INFO, req->client_id,
		      lvds_session, (uint32_t *)req, sizeof(*req));
				   ...
				   struct bst_display_comm_reply* reply)
{
	...
	build_command(&msg_data, LVDS_CMD_SET_VIDEO_STREAM, req->client_id, lvds_session,
				...
				struct bst_display_comm_reply* reply)
{
	...
	build_command(&msg_data, LVDS_CMD_DUMP_DEBUG_INFO, dump_cfg->client_id,
			lvds_session, (uint32_t *)dump_cfg, sizeof(*dump_cfg));
