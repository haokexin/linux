...
int dw_edma_func_num_set(struct dma_chan *dchan, u8 func_no, u8 vfunc_no)
{
	struct dw_edma_chan *chan = dchan2dw_edma_chan(dchan);

	/* We must set requesterID when host is enabled iommu */
	chan->func_no = func_no;
	chan->vfunc_no = vfunc_no;

	return 0;
}
EXPORT_SYMBOL_GPL(dw_edma_func_num_set);
...
int dw_edma_probe(struct dw_edma_chip *chip)
{
	...
	dw->wr_ch_cnt = dw->chip->ll_wr_cnt;
	dw->rd_ch_cnt = dw->chip->ll_rd_cnt;
	#else
