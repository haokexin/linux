...
#ifdef CONFIG_PCIE_BST_EP_USE_OUTBOUND
#include "../../pci/controller/bst/pcie-bst.h"
#define SET_CH_32(dw, dir, ch, name, value) \
	bst_pcie_write_dbi(container_of(dw->chip, struct dw_pcie, edma), \
			   &(__dw_ch_regs(dw, dir, ch)->name), \
			   0, 4, value)

#define GET_CH_32(dw, dir, ch, name) \
	bst_pcie_read_dbi(container_of(dw->chip, struct dw_pcie, edma), \
			  &(__dw_ch_regs(dw, dir, ch)->name), \
			  0, 4)
...
#define SET_BOTH_CH_32(dw, ch, name, value) \
	do {					\
		bst_pcie_write_dbi(container_of(dw->chip, struct dw_pcie, edma), \
				   &(__dw_ch_regs(dw, EDMA_DIR_WRITE, ch)->name), \
				   0, 4, value); \
		bst_pcie_write_dbi(container_of(dw->chip, struct dw_pcie, edma), \
				   &(__dw_ch_regs(dw, EDMA_DIR_READ, ch)->name), \
				   0, 4, value); \
	} while (0)
#else
#define SET_CH_32(dw, dir, ch, name, value) \
...
static void dw_hdma_v0_core_start(struct dw_edma_chunk *chunk, bool first)
{
	...
	if (first) {
		...
		tmp = HDMA_V0_LOCAL_STOP_INT_EN | HDMA_V0_WATERMARK_INT_MASK;
		...
		SET_CH_32(dw, chan->dir, chan->id, control1,
			  HDMA_V0_LINKLIST_EN | HDMA_V0_MEM_TYPE);
		/* Linked list */
		...
		SET_CH_32(dw, chan->dir, chan->id, func_num,
			  (chan->vfunc_no ? HDMA_V0_VF(chan->vfunc_no - 1)
				| HDMA_V0_VF_EN : 0)
				| chan->func_no);
	}
	/* Set consumer cycle */
