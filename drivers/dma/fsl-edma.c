 ...
 *
 * Copyright 2013-2014, 2016 Freescale Semiconductor, Inc.
...
fsl_edma_irq_init(struct platform_device *pdev, struct fsl_edma_engine *fsl_edma)
{
	...
	if (is_s32cc_edma(fsl_edma)) {
		fsl_edma->irq_nos = devm_kzalloc(&pdev->dev, drvdata->n_irqs *
						 sizeof(*fsl_edma->irq_nos),
						 GFP_KERNEL);
		if (!fsl_edma->irq_nos)
			return -ENOMEM;
...
static unsigned int s32cc_mux_channel_mapping(u32 channel_id)
{
...
static struct fsl_edma_irq s32cc_edma_irqs[] = {
...
static struct fsl_edma_drvdata s32cc_data = {
	.version = v1,
	.dmamuxs = DMAMUX_NR,
	.n_irqs = ARRAY_SIZE(s32cc_edma_irqs),
	.irqs = s32cc_edma_irqs,
	.mux_channel_mapping = s32cc_mux_channel_mapping,
	.ops = &fsl_edma3_ops,
};
...
int is_s32cc_edma(struct fsl_edma_engine *data)
{
	return data->drvdata == &s32cc_data;
}
