...
{
	return ~readl(clocksource->clksrc_base + PITCVAL);
}

static int __init pit_clocksource_init(struct pit_timer *pit, unsigned long rate)
{
	clocksource = pit;
	writel(0,  clocksource->clksrc_base + PITTCTRL);
	writel(0xFFFFFFFF,  clocksource->clksrc_base + PITLDVAL);
	writel(PITTCTRL_TEN, clocksource->clksrc_base + PITTCTRL);
...
static void pit_timer_remove(struct platform_device *pdev)
{
}
