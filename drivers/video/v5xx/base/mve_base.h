...
#define MVE_BASE_BUFFER_FRAME_FLAG_MIRROR_HORI   0x00001000  /* HORI mirror*/
#define MVE_BASE_BUFFER_FRAME_FLAG_MIRROR_VERT   0x00002000  /* VERT mirror*/
#define MVE_BASE_BUFFER_FRAME_FLAG_MIRROR_MASK   0x00003000  /* mirror mask*/

#define MVE_BASE_BUFFER_FRAME_FLAG_SCALING_MASKX      0xFF000000 //8bit
#define MVE_BASE_BUFFER_FRAME_FLAG_SCALING_MASKY      0x00FE0000 //7bit
...
struct mve_base_roi_region
{
    ...
    uint16_t prio;       /* the priority of this region */
};

#define MVE_BASE_ROI_REGIONS_MAX 16

struct mve_omx_buffer_general_rows_uncomp_hdr
{
...
struct mve_omx_buffer_general_block_configs
{
    uint8_t blk_cfg_type;
        #define OMX_BLOCK_CONFIGS_TYPE_NONE       (0x00)
        #define OMX_BLOCK_CONFIGS_TYPE_ROW_UNCOMP (0xff)
    uint8_t reserved[3];
    union
    {
        struct mve_omx_buffer_general_rows_uncomp_hdr rows_uncomp;
    } blk_cfgs;
};

struct mve_omx_core_buffer_header_general
{
    //uint64_t user_data_tag;   // User supplied tracking identifier
    //uint64_t app_handle;    // Host buffer handle number
    uint16_t type;  // type of config, value is one of OMX_BUFFER_GENERAL_TYPE_X
        #define OMX_BUFFER_GENERAL_TYPE_BLOCK_CONFIGS (1) /* block_configs */
        #define OMX_BUFFER_GENERAL_TYPE_ENCODER_STATS (4) /* encoder_stats */
    uint16_t config_size;  // size of the configuration
    uint32_t buffer_size;
    struct mve_omx_buffer_general_block_configs config;
};
...
struct mve_base_buffer_details
{
    ...
    uint8_t reserved[2];                                          /**< Unused but required for alignment reasons */
    struct mve_base_roi_region regions[MVE_BASE_ROI_REGIONS_MAX]; /**< ROI data */
    struct mve_omx_core_buffer_header_general header_general;
};
...
enum mve_base_buffer_format
{
	...
	MVE_BASE_BUFFER_FORMAT_YUV420_P010        = 0x7f000110, /**< YUV420 10bit per component (2 planes) */
};
