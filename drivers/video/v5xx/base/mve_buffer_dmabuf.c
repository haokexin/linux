...
bool mve_buffer_dmabuf_map_physical_pages(struct mve_buffer_external *buffer)
{
    ...
    for_each_sg(buffer_external->sg->sgl, sgl, buffer_external->sg->nents, i)
    {
        ...
        if (j < npages)
        {
            MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_INFO, "[MVE] scatterlist size (%u pages) is bigger than the expected size (%u pages)", npages, num_pages);
        }
    }
    ...
    if (count < num_pages)
    {
        MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_WARNING,"[MVE] scatterlist doesn't contain enough pages (found: %d expected: %d pages)\n", count, num_pages);
        goto error;
    }
