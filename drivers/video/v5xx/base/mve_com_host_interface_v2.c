...
static int queue_cache_ops(struct mve_rsrc_dma_mem_t *area, int flag)
{
    ...
    pa = mve_rsrc_dma_mem_get_pages(area);
    if(NULL == pa)
    {
        return -1;
    }
    ...
    {
        mve_rsrc_mem_invalidate_cache_range(pa[0], area->size);
    }
    else if(QUEUE_CLEAN_CACHE == flag)
    {
        mve_rsrc_mem_clean_cache_range(pa[0], area->size);
    }
    else
    {
        return -1;
...
uint32_t mve_com_message_ready(struct mve_session *session, struct mve_rsrc_dma_mem_t *host, struct mve_rsrc_dma_mem_t *mve)
{
                                       ...
                                       enum mve_com_buffer_type type)
{
    ...
    switch (type)
    {
        ...
        default:
            //WARN_ON(true);
            MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session, MVE_LOG_ERROR, session,"add_input_buffer BAD_PARAMETER");
            ret = MVE_BASE_ERROR_BAD_PARAMETER;
            break;
    }
                                        ...
                                        enum mve_com_buffer_type type)
{
    ...
    switch (type)
    {
        ...
        default:
            ...
            MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session, MVE_LOG_ERROR, session,"add_output_buffer BAD_PARAMETER");
            ret = MVE_BASE_ERROR_BAD_PARAMETER;
            break;
    }
                                       ...
                                       mve_com_buffer *buffer)
{
    ...
    switch (msg_header.code) {
        ...
        case MVE_BUFFER_CODE_GENERAL:
            ...
            break;
        default:
            MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session, MVE_LOG_ERROR, session, "Unrecognized code. code=%u", msg_header.code);
            break;
    }
#if 0
...
const struct mve_mem_virt_region *mve_com_hi_v2_get_mem_virt_region(void)
{
    return mem_regions;
}
