...
static void free_session(struct kref *ref)
{
    ...
    MVE_LOG_PRINT(&mve_rsrc_log_session, MVE_LOG_DEBUG, "[%s:%d]session free finish.\n", __func__, __LINE__);
}
...
static void response_received(struct mve_session *session)
{
    ...
    if (false == res)
    {
        MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session, MVE_LOG_ERROR, session, "Invalid state change");
    }
...
static void enqueue_pending_buffers(struct mve_session *session, struct list_head *list, bool empty_this_buffer)
{
    ...
    list_for_each_safe(pos, next, list)
    {
        ...
        ret = mve_session_buffer_enqueue_internal(session, &request->param, empty_this_buffer, true);

        /* If request timed out, then firmware queue is full and we need to retry again later. */
        if (MVE_BASE_ERROR_TIMEOUT == ret)
        {
            MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_DEBUG,"[%s:%d]session:%px comm queue is full and retry again later, empty_this_buffer: %d\n", __func__, __LINE__, session,(int)empty_this_buffer);
            break;
        }
...
static void switchout_session(struct mve_session *session)
{
        ...
        STATE_SWITCHING_IN != session->state.state)
    {
        ...
        if (false != ret)
        {
            ...
            mver_irq_signal_mve(session->lsid);
        }
    }
    ...
    switch (event->code)
    {
        ...
        case MVE_BASE_EVENT_INPUT:
        {
            ...
            else
            {
                ...
                MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_WARNING, "This buffer has already been sent to userspace. Don't send it again.");
                return;
            }
            ...
            if (0 != (session->state.flush_target & MVE_BASE_FLUSH_QUICK))
            {
                    ...
                    session->state.flush_state)
                {
                    ...
                    list_for_each_safe(pos, next, &session->quick_flush_buffers)
                    {
                        ...
                        if (0 > ptr->in_use)
                        {
                            MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_WARNING, "in_use should be gt 0, in_use: %d ", ptr->in_use);
                        }
                        ...
                        else
                        {
                           if (MVE_BASE_ERROR_TIMEOUT == ret)
                           {
                               ...
                               {
                                   MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_WARNING, "[%s:%d]session:%px alloc request failed.", __func__, __LINE__, session);
                               }
                           }
                           else
                           {
                           	MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_WARNING, "[%s:%d]session:%px enqueue quick flush pending bufffer failed.",  __func__, __LINE__, session);
                           }
                        }
...
static void session_watchdog(struct mve_session *session, struct timespec64 *tv)
{
    ...
    if (false != res)
    {
        ...
        {
            ktime_get_real_ts64(&session->state.timestamp);
            cnt = mve_com_message_ready(session, session->msg_in_queue, session->msg_out_queue);
            MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session, MVE_LOG_DEBUG, session, "[%s]msg in message queue size:%u.", __func__, cnt);
...
static void session_destroy_instance(struct mve_session *session)
{
    ...
    MVE_LOG_PRINT_SESSION(&mve_rsrc_log_session, MVE_LOG_DEBUG, session,"[%s]enter.", __func__);
    mver_scheduler_stop((mver_session_id)session);
    mver_scheduler_unregister_session((mver_session_id)session);
...
struct mve_session *mve_session_create(struct file *filep)
{
    ...
    MVE_LOG_PRINT(&mve_rsrc_log_session, MVE_LOG_DEBUG,"[%s:%d]session:%p enter.\n", __func__, __LINE__,  session);
...
struct mve_session *mve_session_get_by_file(struct file *filep, bool clean)
{
    struct list_head *pos, *next;
    struct mve_session *session = NULL;
    int sem_taken = -1;
...
void mve_session_cleanup_client(struct file *filep)
{
    ...
    down(&sessions_sem);
    session = mve_session_get_by_file(filep, true);
    if (NULL != session)
    {
        list_del(&session->list);
        up(&sessions_sem);
                                          ...
                                          bool empty_this_buffer)
{
    ...
    ret = mve_session_buffer_enqueue_internal(session, param, empty_this_buffer, false);
...
uint32_t mve_session_poll(struct file *filp, struct poll_table_struct *poll_table)
{
    ...
    session = mve_session_get_by_file(filp, false);
    if (NULL != session)
    {
        lock_session(session);
