...
static void unmap_external_buffer_mve(struct mve_session *session, struct mve_buffer_external *buffer)
{
    ...
    mver_scheduler_flush_tlb(session->lsid);
}

struct mve_buffer_client *mve_session_buffer_get_external_descriptor(struct mve_session *session,
                                                         ...
                                                         struct mve_buffer_info *info)
{
    ...
    mve_mem_virt_region_get(region_type, &buffer->mapping.region);
                                                    ...
                                                    struct mve_base_buffer_userspace *userspace_buffer)
{
    ...
    buffer = mve_session_buffer_get_external_descriptor(session, userspace_buffer->buffer_id);
    if (NULL != buffer)
    {
                                                      ...
                                                      mve_base_buffer_handle_t buffer_id)
{
    ...
    buffer = mve_session_buffer_get_external_descriptor(session, buffer_id);
    if (NULL == buffer)
    {
    ...
    else
    {
        if (0 != buffer->in_use)
        {
            ...
            //WARN_ON(true);
            MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_WARNING, "The buffer is still in use! Do not unmap it.");
                                                   ...
                                                   bool empty_this_buffer, bool pending_buffer)
{
    ...
    if (false == res && false == pending_buffer)
    {
        ...
        MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_DEBUG, "[%s:%d]session:%px there are already buffers queued waiting to be added to comm queue, empty_this_buffer: %d\n", __func__, __LINE__, session, (int)empty_this_buffer);
        return MVE_BASE_ERROR_TIMEOUT;
    }

    buffer_client = mve_session_buffer_get_external_descriptor(session, param->buffer_id);
    ...
    if (MVE_BASE_ERROR_NONE != ret)
    {
        ...
        {
            MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_WARNING, "in_use should  be eq 0. in_use: %d ", buffer_client->in_use);
        }
                                                   ...
                                                   mve_com_buffer *buffer)
{
    ...
    buffer_client = mve_session_buffer_get_external_descriptor(session, buffer_id);
    if (NULL == buffer_client)
    {
        /* The supplied user-space allocated buffer has not been registered
         * or is invalid. */
    ...
    if (0 > buffer_client->in_use)
    {
        MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_WARNING, "in_use should not be lt 0. in_use: %d ", buffer_client->in_use);
    }
                                                                    ...
                                                                    mve_base_buffer_handle_t buffer_id)
{
    ...
    return mve_com_add_message(session, MVE_REQUEST_CODE_RELEASE_REF_FRAME, sizeof(data), (uint32_t *)&data);
}
