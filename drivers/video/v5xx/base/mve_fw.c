...
static bool construct_fw_page_table(struct mve_firmware_descriptor *desc)
{
    ...
    {
        MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_INFO, "Detected firmware version v3. major=%u, minor=%u.", desc->fw_version.major, desc->fw_version.minor);
    }
    else
    {
                                                    ...
                                                    int ncores)
{
    ...
    for (i = 0; i < (unsigned int)ncores; ++i)
    {
        ...
        mve_mem_virt_region_get(fw_regions[i], &region);
