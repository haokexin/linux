...
static void stop_session(struct mve_session_entry *session)
{
    ...
    MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_DEBUG, "[%s]enter session->session_id:%p\n", __func__, session->session_id);
    if (NO_LSID != lsid)
    {
        tCS *regs;
        int ignore_me;
        MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_DEBUG, "[%s]session->lsid:%u\n", __func__, session->lsid);
        regs = mver_reg_get_coresched_bank();
        if (0 != mver_reg_read32(&regs->LSID[lsid].ALLOC))
        {
            ...
            while (0 != val && --ntries);
            MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_DEBUG, "[%s]session->session_id:%p, write terminate, val:%d\n", __func__, session->session_id, val);
...
bool mver_scheduler_execute(mver_session_id session_id)
{
    ...
    if(0 != sem_taken)
    {
        MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_DEBUG, "[%s:%d]session_id:%px down_interruptible() interrupted by sigal. return false.\n", __func__, __LINE__, session_id);
        return false;
    }
    ...
    if (NULL != entry)
    {
        ...
        if (NO_LSID == entry->lsid)
        {
            ...
            else
            {
                ...
                sem_taken = down_interruptible(&scheduler_sem);
                mver_irq_signal_mve(entry->lsid);
				if (0 == sem_taken)
				{
					up(&scheduler_sem);
				}
...
void mver_scheduler_stop(mver_session_id session_id)
{
    ...
    MVE_LOG_PRINT(&mve_rsrc_log, MVE_LOG_DEBUG, "[%s]enter session_id:%p\n", __func__, session_id);
    down(&scheduler_sem);
...
void mver_scheduler_flush_tlb(int lsid)
{
        if (NO_LSID != lsid)
        {
