...
#include "dma-buf-test-exporter.h"
...
#endif
{
	...
	if (alloc->fail_attach)
		return -EFAULT;
	dev_dbg(te_device.this_device, "%s \n", __func__);
...
static int dma_buf_te_mmap(struct dma_buf *dmabuf, struct vm_area_struct *vma)
{
	...
	dev_dbg(te_device.this_device, "%s dma_addr %llx pages %ld\n", __func__,alloc->contig_dma_addr,alloc->nr_pages);
...
static int do_dma_buf_te_ioctl_alloc(struct dma_buf_te_ioctl_alloc __user *buf, bool contiguous)
{
	...
	if (contiguous) {
		...
		dev_info(te_device.this_device, "%s dma_addr %llx pages %ld\n", __func__,alloc->contig_dma_addr,alloc->nr_pages);
	} else {
		for (i = 0; i < alloc->nr_pages; i++) {
...
static int dmabuf_driver_remove(struct platform_device *pdev)
{
	return 0;
}

#ifdef CONFIG_OF
static const struct of_device_id dma_buf_of_match[] = {
        { .compatible = "bst,dma_buf_te", },
        { /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, dma_buf_of_match);
#endif

#define DMA_BUF_TE_DRIVER_NAME "dma_buf_te"

static struct platform_driver dmabuf_te_driver = {
        .driver = {
                .name = DMA_BUF_TE_DRIVER_NAME,
                .of_match_table = of_match_ptr(dma_buf_of_match),
        },
        .probe = dmabuf_driver_probe,
        .remove = dmabuf_driver_remove,
};
static int __init dma_buf_te_init(void)
{
	...
	te_device.this_device->coherent_dma_mask = DMA_BIT_MASK(64);//add for dma reserve memory with 64 bit addrass

	dev_info(te_device.this_device, "dma_buf_te ready\n");

    platform_driver_register(&dmabuf_te_driver);
	return 0;
...
static void __exit dma_buf_te_exit(void)
{
    ...
    platform_driver_unregister(&dmabuf_te_driver);
}
