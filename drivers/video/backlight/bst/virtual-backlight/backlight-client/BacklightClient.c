...
#include "BacklightClient.h"

#define PID CPU_0
#define FID DEF
#define SID 9U

static BacklightClient_data_t *s_ins;
...
static int32_t receive_message(void)
{
	...
	backlight_client_ext_t *backlight_ext = &s_ins->backlight_ext;
	...
	if (ret == QUERY_INFO_DST_STS_OFFLINE) {
		if (backlight_ext->avail_changed_cb)
			backlight_ext->avail_changed_cb(false, backlight_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_ONLINE) {
		if (backlight_ext->avail_changed_cb)
			backlight_ext->avail_changed_cb(true, backlight_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_CHANGED) {
		ret = ipc_trans_layer_get_endmap(data->pid, &map);
		if (ret == 0) {
			chip_map = backlight_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & backlight_ext->cid_mask) != 0 ? true : false;
			if (status != backlight_ext->status) {
				backlight_ext->status = status;
				if (backlight_ext->avail_changed_cb)
					backlight_ext->avail_changed_cb(status, backlight_ext->avail_ext);
			}
...
static int32_t dispatch_message(void)
{
	...
	while (packet_obj) {
		...
		// dispatch message
		ret = -1;
		if (des->header.pid == s_ins->backlight_ext.cid) {
			if (des->header.typ == MSGBX_MSG_TYPE_BROADCAST)
				ret = s_ins->client.backlight_client.dispatch_broadcast(des);
			else if (des->header.typ == MSGBX_MSG_TYPE_REPLY)
				ret = s_ins->client.backlight_client.dispatch_reply(des);
		}
		if (ret < 0)
...
static int32_t start(void)
{
	...
	if (ret != 0) {
#elif defined IPC_RTE_RTOS
	data->route_task = TaskCreate((task_func_t)router_func, "BacklightClient_thread", 0, NULL,
		0, (uint8_t *)stack_buffer, &tcb_buffer, true);
	if (data->route_task == 0) {
#elif defined IPC_RTE_KERNEL
	data->route_task = kthread_run(router_func, NULL, "BacklightClient_thread");
...
#endif
// BacklightClient_init
BacklightClient_t *BacklightClient_init(BacklightClient_data_t *ins)
{
	int32_t ret = 0;
	...
	// init clients
	ret = backlight_client_init(data, &ins->client.backlight_client, &ins->backlight_ext);
	if (ret < 0)
		return NULL;

	// create client handle.
	ret = ipc_trans_layer_proxy_create_handle(data->pid, data->fid, data->sid, ins->backlight_ext.cid, ins->backlight_ext.ccid, &data->handle);
	if (ret < 0)
	{
...
int32_t BacklightClient_destroy(void)
{
	int32_t ret = 0;
	com_client_data_t *data = (com_client_data_t *)s_ins;
