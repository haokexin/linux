...
uint32_t max_blk1_freq = 0;

void mver_blk1_dvfs_init(struct device *dev,
                    const mver_dvfs_get_session_status_fptr get_session_status_callback)
{
    max_blk1_freq = mver_blk1_pm_read_max_frequency();
}

void mver_blk1_dvfs_deinit(struct device *dev)
{
    /* Nothing to do */
}

bool mver_blk1_dvfs_register_session(const mver_session_id session_id)
{
    /* Request the maximum frequency. It's then up to the integration layer to
     * decide which frequency is actually used */
    mver_blk1_pm_request_frequency(max_blk1_freq);

    return true;
}

void mver_blk1_dvfs_unregister_session(const mver_session_id session_id)
{
    /* Nothing to do */
}

void mver_blk1_dvfs_request_max_frequency(void)
{
    /* Nothing to do */
}
...
{
    uint32_t old = mver_blk1_pm_read_frequency();
    uint32_t new;
#if (1 == DVFS_DEBUG_MODE)
    bool do_burst = false;
#endif
    ...
    {
        new = min(old + steps * up_step_freq, max_blk1_freq);
    }

    if (old != new)
    {
        mver_blk1_pm_request_frequency(new);
#if (1 == DVFS_DEBUG_MODE)
...
{
    int old_freq = (int)mver_blk1_pm_read_frequency();
    int new_freq;
    ...
    {
        mver_blk1_pm_request_frequency(new_freq);
#if (1 == DVFS_DEBUG_MODE)
        atomic_long_inc(&changes_cnt);
#endif
    }
...
static void update_sessions(void)
{
    ...
    list_for_each_safe(entry, safe, &sessions)
    {
        ...
        if (!status_received)
        {
            MVE_LOG_PRINT(&mve_blk1_rsrc_log_scheduler, MVE_LOG_WARNING,
...
{
    MVE_LOG_PRINT(&mve_blk1_rsrc_log_scheduler, MVE_LOG_INFO, "DVFS polling thread started");
    ...
    MVE_LOG_PRINT(&mve_blk1_rsrc_log_scheduler, MVE_LOG_INFO, "DVFS polling thread finished");
    return 0;
}
                                 ...
                                 char *buf)
{
    ...
    struct list_head *entry;
    struct session *session;
    uint32_t freq = mver_blk1_pm_read_frequency();

    num += scnprintf(buf + num, PAGE_SIZE - num,
                     "freq: %4u, max_blk1_freq: %4u, up_step_freq: %3u, down_step_freq: %3u",
                     freq, max_blk1_freq, up_step_freq, down_step_freq);
#if (1 == DVFS_DEBUG_MODE)
    num += scnprintf(buf + num, PAGE_SIZE - num,
                       ...
                       const char *buf, size_t count)
{
    ...
    if (!failed)
    {
        mver_blk1_pm_request_frequency((uint32_t)freq);
    }
    return (failed) ? failed : count;
}
                                  ...
                                  const char *buf, size_t count)
{
    ...
    if (!failed)
    {
        up_step_freq = ratio(max_blk1_freq, up_step_percent);
    }
    return (failed) ? failed : count;
}
                                    ...
                                    const char *buf, size_t count)
{
    ...
    if (!failed)
    {
        down_step_freq = ratio(max_blk1_freq, down_step_percent);
    }
    return (failed) ? failed : count;
}
...
static void sysfs_register_devices(struct device *dev)
{
    ...
    while (i--)
    {
        ...
        if (err < 0)
        {
            MVE_LOG_PRINT(&mve_blk1_rsrc_log_scheduler, MVE_LOG_ERROR,
...
EXPORT_SYMBOL(mver_blk1_dvfs_request_max_frequency);
