...
#include "mve_blk1_rsrc_mem_cache.h"
...
void mve_blk1_rsrc_mem_cache_init(void)
{
...
void mve_blk1_rsrc_mem_cache_deinit(void)
{
...
static struct kmem_cache *mve_blk1_rsrc_get_cachep(uint32_t size)
{
    ...
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_ERROR, "No allocation slab found. size=%u.", size);
    }

    return cachep;
}

void *mve_blk1_rsrc_mem_cache_alloc(uint32_t size, uint32_t flags)
{
    void *ptr;
    struct kmem_cache *cachep;

    cachep = mve_blk1_rsrc_get_cachep(size);
...
void mve_blk1_rsrc_mem_cache_free(void *ptr, uint32_t size)
{
    struct kmem_cache *cachep;
...
EXPORT_SYMBOL(mve_blk1_rsrc_mem_cache_alloc);
EXPORT_SYMBOL(mve_blk1_rsrc_mem_cache_free);
