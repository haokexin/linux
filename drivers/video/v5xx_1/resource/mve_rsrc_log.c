...
static struct mve_blk1_rsrc_log log;
...
struct mve_rsrc_log_group mve_blk1_rsrc_log;
struct mve_rsrc_log_group mve_blk1_rsrc_log_scheduler;
struct mve_rsrc_log_group mve_blk1_rsrc_log_fwif;
struct mve_rsrc_log_group mve_blk1_rsrc_log_session;

EXPORT_SYMBOL(mve_blk1_rsrc_log);
EXPORT_SYMBOL(mve_blk1_rsrc_log_fwif);
EXPORT_SYMBOL(mve_blk1_rsrc_log_session);
...
static ssize_t group_drain_write(struct file *file, const char __user *user_buffer, size_t count, loff_t *position)
{
    /* File path mve/group/<group>/drain. */
    struct mve_rsrc_log_group *group = file->f_path.dentry->d_parent->d_inode->i_private;
    struct mve_blk1_rsrc_log *log = file->f_path.dentry->d_parent->d_parent->d_parent->d_inode->i_private;
    ...
    drain_str[count] = '\0';
    mve_blk1_rsrc_log_trim(drain_str);
 ...
 */
static void mve_rsrc_log_group_add(struct mve_blk1_rsrc_log *log, const char *name, struct mve_rsrc_log_group *group)
{
 ...
 */
static void mve_rsrc_log_drain_add(struct mve_blk1_rsrc_log *log, const char *name, struct mve_rsrc_log_drain *drain)
{
 ...
 * @param drain     Pointer to drain.
 */
static void mve_rsrc_log_drain_ram_add(struct mve_blk1_rsrc_log *log, const char *name, struct mve_rsrc_log_drain_ram *drain)
{
 ...
 * @param log       Pointer to log.
 */
static void mve_rsrc_log_construct(struct mve_blk1_rsrc_log *log)
{
    ...
    log->mve_dir = debugfs_create_dir("mve_blk1", NULL);
 ...
 * @param log       Pointer to log.
 */
static void mve_rsrc_log_destruct(struct mve_blk1_rsrc_log *log)
{
    debugfs_remove_recursive(log->mve_dir);
}
...
void mve_blk1_rsrc_log_init(void)
{
    ...
    mve_rsrc_log_group_construct(&mve_blk1_rsrc_log, "MVE1", MVE_LOG_WARNING, drain_default);
    mve_rsrc_log_group_add(&log, "generic", &mve_blk1_rsrc_log);

    mve_rsrc_log_group_construct(&mve_blk1_rsrc_log_scheduler, "MVE1 scheduler", MVE_LOG_WARNING, drain_default);
    mve_rsrc_log_group_add(&log, "scheduler", &mve_blk1_rsrc_log_scheduler);

    mve_rsrc_log_group_construct(&mve_blk1_rsrc_log_fwif, "MVE1 fwif", MVE_LOG_INFO, drain_ram);
    mve_rsrc_log_group_add(&log, "firmware_interface", &mve_blk1_rsrc_log_fwif);

    mve_rsrc_log_group_construct(&mve_blk1_rsrc_log_session, "MVE1 session", MVE_LOG_WARNING, drain_default);
    mve_rsrc_log_group_add(&log, "session", &mve_blk1_rsrc_log_session);
}

void mve_blk1_rsrc_log_destroy(void)
{
    /* Destroy objects in reverse order. */
    mve_rsrc_log_destruct(&log);

    mve_rsrc_log_group_destruct(&mve_blk1_rsrc_log);
    mve_rsrc_log_group_destruct(&mve_blk1_rsrc_log_scheduler);
    mve_rsrc_log_group_destruct(&mve_blk1_rsrc_log_fwif);
    mve_rsrc_log_group_destruct(&mve_blk1_rsrc_log_session);
...
const char *mve_blk1_rsrc_log_strrchr(const char *s)
{
...
struct dentry *mve_blk1_rsrc_log_get_parent_dir(void)
{
    return log.mve_dir;
}

EXPORT_SYMBOL(mve_blk1_rsrc_log_strrchr);
EXPORT_SYMBOL(mve_blk1_rsrc_log_get_parent_dir);
