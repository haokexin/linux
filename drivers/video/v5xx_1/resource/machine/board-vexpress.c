...
#include "mve_blk1_power_management.h"
#include "mve_blk1_dvfs.h"
#include "mve_blk1_config.h"
...
{
    MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_INFO, "Enable power.");
...
{
    MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_INFO, "Disable power.");
    /* No need to do anything on the FPGA */
}
...
static void dvfs_cores_pause(void)
{
    ...
    mver_blk1_reg_write32(&regs->CLKPAUSE, req);
    do
    {
        val = mver_blk1_reg_read32(&regs->CLKIDLE);
    }
    ...
    if (req != val)
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_WARNING,
...
static void dvfs_cores_resume(void)
{
    ...
    regs = mver_blk1_reg_get_coresched_bank();
    mver_blk1_reg_write32(&regs->CLKPAUSE, 0);
    mver_blk1_reg_put_coresched_bank(&regs);
}
...
static int dvfs_get_ncores(void)
{
    ...
    regs = mver_blk1_reg_get_coresched_bank();
    ncores = mver_blk1_reg_read32(&regs->NCORES);
    mver_blk1_reg_put_coresched_bank(&regs);
...
{
    MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_INFO, "Enable clock.");
}

static void disable_clock(void)
{
    MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_INFO, "Disable clock.");
}

static void set_clock_rate(uint32_t clk_rate)
{
    MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_DEBUG, "Setting clock rate. rate=%u (%u)", clk_rate, dvfs_freq);
#if defined(ENABLE_DVFS_FREQ_SIM)
    dvfs_sim_init();
...
static void stop(void)
{
    ...
    if (dvfs_is_started)
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_DEBUG, "Stopping thread.");
...
struct mve_config_attribute *mve_blk1_device_get_config(void)
{
    return attributes;
}
