...
#include "machine/mve_blk1_port_attributes.h"
...
struct mve_rsrc_driver_data mve_blk1_rsrc_data;
static BLOCKING_NOTIFIER_HEAD(mve_rsrc_notifier_chain);
static bool mve_rsrc_ready = false;

extern struct mve_config_attribute *mve_blk1_device_get_config(void);

/* Mali-V500 can handle physical addresses 40 bits wide */
static uint64_t mv500_dma_mask = DMA_BIT_MASK(40);

static int mver_blk1_driver_open(struct inode *inode, struct file *file)
{
    return 0;
}

static int mver_blk1_driver_release(struct inode *inode, struct file *file)
{
    return 0;
}
...
static struct of_device_id mve_rsrc_of_match[] =
{
    ...
    {
        .compatible = "arm china,linlon-v5-1",
    },
    {}
};
...
{
    .owner = THIS_MODULE,
    .open = mver_blk1_driver_open,
    .release = mver_blk1_driver_release,
};
...
{
    struct mve_rsrc_driver_data *private = &mve_blk1_rsrc_data;
    if(reset)
    {
 ...
 */
int register_driver_mve_rsrc_blk1_notifier(struct notifier_block *nb)
{
    int ret;
...
EXPORT_SYMBOL(register_driver_mve_rsrc_blk1_notifier);
 ...
 * @return int 0 on success, negative error code on failure
 */
int unregister_driver_mve_rsrc_blk1_notifier(struct notifier_block *nb)
{
    return blocking_notifier_chain_unregister(&mve_rsrc_notifier_chain, nb);
}
EXPORT_SYMBOL(unregister_driver_mve_rsrc_blk1_notifier);
...
static void reset_hw(void)
{
    ...
    mver_blk1_reg_write32(&regs->RESET, 1);
    while (corelsid_mask !=  mver_blk1_reg_read32(&regs->CORELSID))
        ;
    /* Let the hardware power down */
    mver_blk1_reg_write32(&regs->CLKFORCE, 0);
    mver_blk1_reg_put_coresched_bank(&regs);
}

static int mver_blk1_driver_probe(struct platform_device *pdev)
{
    struct mve_rsrc_driver_data *private = &mve_blk1_rsrc_data;
    int ret = 0;
    struct resource res;
    ...
    private->config = mve_blk1_device_get_config();
    ...
    attr_fptr = (mve_port_attributes_callback_fptr)
                mve_blk1_config_get_value(private->config, MVE_CONFIG_DEVICE_ATTR_BUS_ATTRIBUTES);
    if (NULL == attr_fptr)
    {
    ...
    mver_blk1_reg_init();
    mve_blk1_rsrc_mem_init(&pdev->dev);
    mve_blk1_rsrc_log_init();
    mver_blk1_irq_handler_init(&pdev->dev);
    mver_blk1_pm_init(&pdev->dev);
    mver_blk1_scheduler_init(&pdev->dev);
    pm_runtime_set_suspended(&pdev->dev);
    pm_runtime_enable(&pdev->dev);
    pm_runtime_get_sync(&pdev->dev);
    reset_hw();
    regs = mver_blk1_reg_get_coresched_bank();
    private->nlsid = mver_blk1_reg_read32(&regs->NLSID);
    private->ncore = mver_blk1_reg_read32(&regs->NCORES);
    private->fuse = mver_blk1_reg_read32(&regs->FUSE);
    private->hw_version = mver_blk1_reg_read32(&regs->VERSION);
    if (private->hw_version >= MVE_V52_V76)
    {
        ...
        mver_blk1_reg_write32(&regs->BUSCTRL, sysbusctrl);
    }
    ...
    dev_info(&pdev->dev, "MVE_blk1 resource driver loaded successfully (nlsid=%u, cores=%u, version=0x%X).\n",
...
static int mver_blk1_driver_remove(struct platform_device *pdev)
{
    mver_blk1_scheduler_deinit(&pdev->dev);
    mver_blk1_irq_handler_deinit(&pdev->dev);
    mve_blk1_rsrc_log_destroy();
    mve_blk1_rsrc_mem_deinit(&pdev->dev);
    mver_blk1_pm_deinit(&pdev->dev);
    pm_runtime_disable(&pdev->dev);

    iounmap(mve_blk1_rsrc_data.regs);
    release_mem_region(mve_blk1_rsrc_data.mem_res->start, resource_size(mve_blk1_rsrc_data.mem_res));
    unregister_chrdev(mve_dev_major, MVE_RSRC_DRIVER_NAME);

    dev_info(&pdev->dev, "MVE_blk1 resource driver unloaded successfully.\n");
    return 0;
}

static const struct dev_pm_ops rsrc_pm =
{
    .suspend = mver_blk1_pm_suspend,
    .resume = mver_blk1_pm_resume,
    .runtime_suspend = mver_blk1_pm_runtime_suspend,
    .runtime_resume = mver_blk1_pm_runtime_resume,
    .runtime_idle = mver_blk1_pm_runtime_idle,
};

static struct platform_driver mv500_rsrc_driver =
{
    .probe = mver_blk1_driver_probe,
    .remove = mver_blk1_driver_remove,
...
static int __init mver_blk1_driver_init(void)
{
    ...
    {
        printk("MVE_blk1 Resource driver probe not called, check your device tree.\n");
        return -EINVAL;
    }
}

static void __exit mver_blk1_driver_exit(void)
{
    platform_driver_unregister(&mv500_rsrc_driver);
    MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_INFO, "MVE_blk1 resource driver unregistered.");
}

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Mali-V500 resource driver");

module_init(mver_blk1_driver_init);
module_exit(mver_blk1_driver_exit);

EXPORT_SYMBOL(mve_blk1_rsrc_data);
