...
struct mve_rsrc_dma_mem_t *mve_blk1_rsrc_dma_mem_alloc_uncached(uint32_t size)
{
...
#ifndef USE_IN_A1000B_BOARD
		dma_set_mask_and_coherent(&mve_blk1_rsrc_data.pdev->dev, DMA_BIT_MASK(64));//add for linlon with 64 bit address
#endif
        mem->cpu_addr = dma_alloc_coherent(&mve_blk1_rsrc_data.pdev->dev, size, &mem->dma_handle, GFP_KERNEL);
...
void mve_blk1_rsrc_dma_mem_free_uncached(struct mve_rsrc_dma_mem_t *mem)
{
    struct dma_mem_uncached_t *ptr = (struct dma_mem_uncached_t *)mem;

    dma_free_coherent(&mve_blk1_rsrc_data.pdev->dev, ptr->header.size, ptr->cpu_addr, ptr->dma_handle);
    if (NULL != ptr->pages)
    {
...
void mve_blk1_rsrc_dma_mem_clean_cache_uncached(struct mve_rsrc_dma_mem_t *mem)
{
    /* Nop */
    mve_blk1_rsrc_mem_flush_write_buffer();
}

void mve_blk1_rsrc_dma_mem_invalidate_cache_uncached(struct mve_rsrc_dma_mem_t *mem)
{
    /* Nop */
    mve_blk1_rsrc_mem_flush_write_buffer();
}

void *mve_blk1_rsrc_dma_mem_map_uncached(struct mve_rsrc_dma_mem_t *mem)
{
    struct dma_mem_uncached_t *ptr = (struct dma_mem_uncached_t *)mem;
...
void mve_blk1_rsrc_dma_mem_unmap_unchached(struct mve_rsrc_dma_mem_t *mem)
{
    /* Nop */
}

phys_addr_t *mve_blk1_rsrc_dma_mem_get_pages_uncached(struct mve_rsrc_dma_mem_t *mem)
{
    struct dma_mem_uncached_t *ptr = (struct dma_mem_uncached_t *)mem;
