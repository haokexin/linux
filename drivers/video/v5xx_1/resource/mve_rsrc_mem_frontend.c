...
static struct memory_entry *find_memory_entry(void *os_ptr)
{
    ...
    if (entry == NULL)
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_ERROR, "Cannot find_memory_entry: %p", os_ptr);
    }
...
void mve_blk1_rsrc_mem_init(struct device *dev)
{
#if (1 == MVE_MEM_DBG_SUPPORT)
...
#ifdef CONFIG_SYSFS
    {
        ...
        for (i = 0; i < NELEMS(sysfs_files); ++i)
        {
            ...
            if (err < 0)
            {
                MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_ERROR, "Unable to create sysfs file.");
            }
        }
    }
#endif /* CONFIG_SYSFS */
    ...
    mve_blk1_rsrc_mem_cache_init();
}

void mve_blk1_rsrc_mem_deinit(struct device *dev)
{
#if (1 == MVE_MEM_DBG_SUPPORT)
#ifndef EMULATOR
    int i;
    ...
    mve_blk1_rsrc_mem_cache_deinit();
}
