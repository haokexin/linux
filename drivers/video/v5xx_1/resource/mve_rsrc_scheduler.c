...
#include "mve_blk1_rsrc_circular_buffer.h"
#include "mve_blk1_rsrc_log.h"
#include "mve_blk1_rsrc_dvfs.h"
...
static struct mve_session_entry *get_session_cache_entry(mver_session_id session_id)
{
    ...
    list_for_each(pos, &sessions)
    {
        ...
        if (NULL != entry && entry->session_id == session_id)
        {
            return entry;
        }
    }
...
static void disable_scheduling(tCS *regs)
{
    ...
    mver_blk1_reg_write32(&regs->ENABLE, 0);
    do
    {
        val = mver_blk1_reg_read32(&regs->ENABLE);
    }
    while (0 != val);
}
...
{
    wmb();
    mver_blk1_reg_write32(&regs->ENABLE, 1);
}
...
static enum LSID jobqueue_currently_executing(void)
{
    ...
    regs = mver_blk1_reg_get_coresched_bank();
    core_lsid = mver_blk1_reg_read32(&regs->CORELSID);
    mver_blk1_reg_put_coresched_bank(&regs);
...
static bool session_is_executing(struct mve_session_entry *session)
{
    ...
    regs = mver_blk1_reg_get_coresched_bank();
    job_queue = mver_blk1_reg_read32(&regs->JOBQUEUE);
    core_lsid = mver_blk1_reg_read32(&regs->CORELSID);
    mver_blk1_reg_put_coresched_bank(&regs);
...
static bool jobqueue_enqueue_session(struct mve_session_entry *session)
{
    ...
    if (false != job_added)
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log_scheduler, MVE_LOG_DEBUG, "Enqueueing session. session=%p, lsid=%u, jobqueue=0x%08X, corelsid=0x%X.",
                      session->session_id, lsid, job_queue, mver_blk1_reg_read32(&regs->CORELSID));
        mver_blk1_reg_write32(&regs->JOBQUEUE, job_queue);
    }
...
static void jobqueue_dequeue_lsid(tCS *regs, enum LSID lsid)
{
    ...
    mver_blk1_reg_write32(&regs->JOBQUEUE, new_job_queue);
}
                                            ...
                                            char *buf)
{
    ...
    for (i = 0; i < mve_blk1_rsrc_data.nlsid; ++i)
    {
...
static void session_ram_preserve(uint32_t *session_ram, tCS *regs, int lsid)
{
    ...
    {
        session_ram[i] = mver_blk1_reg_read32(&regs->SESSIONRAM[offset + i]);
    }
}
                        ...
                        const char *buf, size_t count)
{
    ...
    regs = mver_blk1_reg_get_coresched_bank();
    phys_cores = mver_blk1_reg_read32(&regs->NCORES);
    mver_blk1_reg_put_coresched_bank(&regs);
    ...
    /* Remember core count. */
    mve_blk1_rsrc_data.ncore = num_cores;
...
void mver_blk1_scheduler_init(struct device *dev)
{
    INIT_LIST_HEAD(&sessions);
    sema_init(&scheduler_sem, 1);
    queued_sessions = mver_blk1_circular_buffer_create(64);

    mver_blk1_dvfs_init(dev, get_session_status);
#if defined(CONFIG_SYSFS) && defined(_DEBUG)
    {
        int err;
        int i;
        ...
        for (i = 0; i < NELEMS(sysfs_files); ++i)
        {
            ...
            if (err < 0)
            {
                MVE_LOG_PRINT(&mve_blk1_rsrc_log_scheduler, MVE_LOG_ERROR, "Unable to create sysfs file.");
            }
        }
    }
#endif
...
void mver_blk1_scheduler_deinit(struct device *dev)
{
    int sem_taken;

    mver_blk1_dvfs_deinit(dev);
    /* Destroy queued sessions. */
    sem_taken = down_interruptible(&scheduler_sem);

    mver_blk1_circular_buffer_destroy(queued_sessions);
...
static void switch_out_session(struct mve_session_entry *session)
{
    ...
    val = mver_blk1_reg_read32(&regs->LSID[lsid].ALLOC);
    WARN_ON(val == 0);
    if (0 != val)
    {
        int sem_taken;
        ...
        mver_blk1_reg_write32(&regs->LSID[lsid].SCHED, 0);

        ncore = mver_blk1_reg_read32(&regs->NCORES);

        /* Make sure all firmware memory writes have completed */
        do
        {
            /* Get the current mask of which core is running what */
            corelsid_mask =  mver_blk1_reg_read32(&regs->CORELSID);
        ...
        mver_blk1_reg_write32(&regs->LSID[lsid].ALLOC, 0);
        session_by_lsid[session->lsid] = NULL;
        ((struct mve_session *)(session->session_id))->lsid = NO_LSID;
        session->lsid = NO_LSID;

        MVE_LOG_PRINT(&mve_blk1_rsrc_log_scheduler, MVE_LOG_DEBUG, "Switching out session. session=%p, lsid=%u, corelsid=0x%X.",
                      session->session_id, lsid, mver_blk1_reg_read32(&regs->CORELSID));
        ...
        if (SCHEDULE_STATE_SLEEP != state && 0 == session->enqueues)
        {
            ...
            mver_blk1_circular_buffer_add(queued_sessions, session);
            session->enqueues++;
        }
...
static bool map_session(struct mve_session_entry *session, enum LSID lsid)
{
    ...
    ((struct mve_session *)(session->session_id))->lsid = lsid;
    session->lsid = lsid;
    session_by_lsid[lsid] = session;

    regs = mver_blk1_reg_get_coresched_bank();

    WARN_ON(mver_blk1_reg_read32(&regs->LSID[lsid].ALLOC) != 0);

    if (false == session->secure)
    {
        mver_blk1_reg_write32(&regs->LSID[lsid].ALLOC, 1);
    }
    else
    {
        mver_blk1_reg_write32(&regs->LSID[lsid].ALLOC, 2);
    }
    ...
    mver_blk1_reg_write32(&regs->LSID[lsid].TERMINATE, 1);
    ntries = WAIT_FOR_TERMINATE_NTRIES;
    do
    {
        val = mver_blk1_reg_read32(&regs->LSID[lsid].TERMINATE);
    }
    while (0 != val && --ntries);
    ...
    mver_blk1_reg_write32(&regs->LSID[lsid].MMU_CTRL, session->mmu_ctrl);
    mver_blk1_reg_write32(&regs->LSID[lsid].CTRL, disallow_cores | (session->ncores << CORESCHED_LSID_CTRL_MAXCORES));

    mver_blk1_reg_write32(&regs->LSID[lsid].BUSATTR[0], mve_blk1_rsrc_data.port_attributes[0]);
    mver_blk1_reg_write32(&regs->LSID[lsid].BUSATTR[1], mve_blk1_rsrc_data.port_attributes[1]);
    mver_blk1_reg_write32(&regs->LSID[lsid].BUSATTR[2], mve_blk1_rsrc_data.port_attributes[2]);
    mver_blk1_reg_write32(&regs->LSID[lsid].BUSATTR[3], mve_blk1_rsrc_data.port_attributes[3]);

    mver_blk1_reg_write32(&regs->LSID[lsid].FLUSH_ALL, 0);

    /* Clear all interrupts */
    mver_blk1_reg_write32(&regs->LSID[lsid].IRQVE, 0);
    mver_blk1_reg_write32(&regs->LSID[lsid].IRQHOST, 0);
    wmb();
    mver_blk1_reg_write32(&regs->LSID[lsid].SCHED, 1);
...
static bool switch_in_session(struct mve_session_entry *session)
{
    ...
    MVE_LOG_PRINT(&mve_blk1_rsrc_log_scheduler, MVE_LOG_DEBUG, "Switching in session. session=%p, lsid=%u, enqueued=%u.", session->session_id, session->lsid, nr_enqueues);
...
static void switch_in_pending_sessions(void)
{
    ...
    num = mver_blk1_circular_buffer_get_num_entries(queued_sessions);
    if (0 == num)
    {
        /* No pending sessions */
        return;
    ...
    do
    {
        mver_blk1_circular_buffer_peek(queued_sessions, (void **)&session);
        res = switch_in_session(session);
        if (false != res)
        {
            mver_blk1_circular_buffer_remove(queued_sessions, (void **)&session);
            session->enqueues--;
...
static void stop_session(struct mve_session_entry *session)
{
    ...
    MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_DEBUG, "[%s]enter session->session_id:%p\n", __func__, session->session_id);
    if (NO_LSID != lsid)
    {
        tCS *regs;
        int ignore_me;
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_DEBUG, "[%s]session->lsid:%u\n", __func__, session->lsid);
        regs = mver_blk1_reg_get_coresched_bank();
        if (0 != mver_blk1_reg_read32(&regs->LSID[lsid].ALLOC))
        {
            uint32_t val;
            int ntries;
            ...
            while (0 != val && --ntries);
            MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_DEBUG, "[%s]session->session_id:%p, write terminate, val:%d\n", __func__, session->session_id, val);
            /* Print a warning if hardware was unresponsive */
            WARN_ON(0 != val);

            mver_blk1_reg_write32(&regs->LSID[lsid].ALLOC, 0);

            MVE_LOG_PRINT(&mve_blk1_rsrc_log_scheduler, MVE_LOG_DEBUG, "Session stopped. Switching out session. session=%p.", session->session_id);
        }
...
bool mver_blk1_scheduler_register_session(mver_session_id session_id,
                                     uint32_t mmu_ctrl,
                                     int ncores,
                                     bool secure,
                                     ...
                                     session_get_restricting_buffer_count_fptr get_restricting_buffer_count_callback)
{
    ...
    mver_blk1_dvfs_register_session(session_id);
...
void mver_blk1_scheduler_unregister_session(mver_session_id session_id)
{
    struct mve_session_entry *entry;
    int sem_taken;
    ...
    mver_blk1_dvfs_unregister_session(session_id);
    entry = get_session_cache_entry(session_id);
    if (NULL != entry)
    {
...
static void process_session_irq(struct mve_session_entry *entry)
{
    ...
    else if (SCHEDULE_STATE_RESCHEDULE == state && 0 == entry->enqueues)
    {
        ...
        if (false == is_executing)
        {
            ...
            mver_blk1_circular_buffer_add(queued_sessions, entry);
            entry->enqueues++;
        }
    }
...
bool mver_blk1_scheduler_execute(mver_session_id session_id)
{
    struct mve_session_entry *entry;
    int sem_taken;
    bool ret = false;
    ...
    if(0 != sem_taken)
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_DEBUG, "[%s:%d]session_id:%px down_interruptible() interrupted by sigal. return false.\n", __func__, __LINE__, session_id);
        return false;
    }
    ...
    if (NULL != entry)
    {
        ...
        if (NO_LSID == entry->lsid)
        {
            ...
            else
            {
                ...
                sem_taken = down_interruptible(&scheduler_sem);
                mver_blk1_irq_signal_mve(entry->lsid);
				if (0 == sem_taken)
				{
					up(&scheduler_sem);
				}
...
void mver_blk1_scheduler_stop(mver_session_id session_id)
{
    struct mve_session_entry *entry;
    
    MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_DEBUG, "[%s]enter session_id:%p\n", __func__, session_id);
    down(&scheduler_sem);
    ...
    if (NULL != entry)
    {
        mver_blk1_circular_buffer_remove_all_occurences(queued_sessions, entry);
        stop_session(entry);
    }
...
void mver_blk1_scheduler_handle_irq(int lsid)
{
    struct mve_session_entry *entry;
    int sem_taken;
...
void mver_blk1_scheduler_flush_tlb(int lsid)
{
        if (NO_LSID != lsid)
        {
            tCS *regs = mver_blk1_reg_get_coresched_bank();
            mver_blk1_reg_write32(&regs->LSID[lsid].FLUSH_ALL, 0);
            mver_blk1_reg_put_coresched_bank(&regs);
        }
    }

enum LSID mver_blk1_scheduler_get_session_lsid(mver_session_id session_id)
{
    struct mve_session_entry *entry;
    enum LSID lsid = NO_LSID;
...
void mver_blk1_scheduler_suspend(void)
{
    int sem_taken;
    int i;
    bool done = false;
    ...
    MVE_LOG_PRINT(&mve_blk1_rsrc_log_scheduler, MVE_LOG_INFO, "All sessions suspended.");
}

void mver_blk1_scheduler_resume(void)
{
    ...
    MVE_LOG_PRINT(&mve_blk1_rsrc_log_scheduler, MVE_LOG_INFO, "Sessions resumed.");
}

int mver_blk1_scheduler_get_ncores(void)
{
#ifdef EMULATOR
    return 1;
#else
    return mve_blk1_rsrc_data.ncore;
#endif
}

EXPORT_SYMBOL(mver_blk1_scheduler_register_session);
EXPORT_SYMBOL(mver_blk1_scheduler_unregister_session);
EXPORT_SYMBOL(mver_blk1_scheduler_execute);
EXPORT_SYMBOL(mver_blk1_scheduler_stop);
EXPORT_SYMBOL(mver_blk1_scheduler_flush_tlb);
EXPORT_SYMBOL(mver_blk1_scheduler_get_ncores);
