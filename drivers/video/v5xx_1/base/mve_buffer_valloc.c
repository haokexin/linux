...
struct mve_buffer_external *mve_blk1_buffer_valloc_create_buffer_external(struct mve_buffer_info *info)
{
...
void mve_blk1_buffer_valloc_destroy_buffer_external(struct mve_buffer_external *buffer)
{
    MVE_RSRC_MEM_CACHE_FREE(buffer, sizeof(struct mve_buffer_external));
}

bool mve_blk1_buffer_valloc_map_physical_pages(struct mve_buffer_external *buffer)
{
    uint32_t size;
    void *ptr;
    uint32_t num_pages;
    ...
    pages = mve_blk1_rsrc_mem_map_virt_to_phys(ptr - offset, size, buffer->mapping.write);
    if (NULL == pages)
    {
        return false;
    }
...
bool mve_blk1_buffer_valloc_unmap_physical_pages(struct mve_buffer_external *buffer)
{
    /* mve_mem_unmap_virt_to_phys frees the memory allocated for the array
     * buffer->pages. */
    mve_blk1_rsrc_mem_unmap_virt_to_phys(buffer->mapping.pages, buffer->mapping.num_pages);
...
void mve_blk1_buffer_valloc_set_owner(struct mve_buffer_external *buffer,
                                 ...
                                 enum mve_port_index port)
{
    ...
    for (i = 0; i < buffer->mapping.num_pages; ++i)
    {
        ...
        sync_func(&mve_blk1_rsrc_data.pdev->dev, buffer->mapping.pages[i] + offset, size, direction);
    }
}
