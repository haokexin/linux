...
#include "mve_blk1_session_buffer.h"

#include "mve_blk1_rsrc_mem_frontend.h"
#include "mve_blk1_rsrc_mem_backend.h"

#include "mve_blk1_rsrc_irq.h"
#include "mve_blk1_rsrc_register.h"
#include "mve_blk1_rsrc_scheduler.h"
#include "mve_blk1_rsrc_driver.h"
#include "mve_blk1_rsrc_pm.h"
#include "mve_blk1_rsrc_log.h"

#include <host_interface_v3/mve_blk1_protocol_def.h>
#include "mve_blk1_com_host_interface_v2.h"
...
static struct semaphore session_destroy_sem;
...
static void free_session(struct kref *ref)
{
    ...
    mve_blk1_session_buffer_unmap_all(session);

    mve_blk1_queue_destroy(session->response_events);
    mve_blk1_queue_destroy(session->mve_events);

    mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_MSG_IN_QUEUE, &region);
    mve_blk1_mmu_unmap_pages(session->mmu_ctx, region.start);

    mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_MSG_OUT_QUEUE, &region);
    mve_blk1_mmu_unmap_pages(session->mmu_ctx, region.start);

    mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_INPUT_BUFFER_IN, &region);
    mve_blk1_mmu_unmap_pages(session->mmu_ctx, region.start);

    mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_INPUT_BUFFER_OUT, &region);
    mve_blk1_mmu_unmap_pages(session->mmu_ctx, region.start);

    mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_OUTPUT_BUFFER_IN, &region);
    mve_blk1_mmu_unmap_pages(session->mmu_ctx, region.start);

    mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_OUTPUT_BUFFER_OUT, &region);
    mve_blk1_mmu_unmap_pages(session->mmu_ctx, region.start);

    mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_RPC_QUEUE, &region);
    mve_blk1_mmu_unmap_pages(session->mmu_ctx, region.start);

    mve_blk1_rsrc_dma_mem_free(session->msg_in_queue);
    mve_blk1_rsrc_dma_mem_free(session->msg_out_queue);
    mve_blk1_rsrc_dma_mem_free(session->buf_input_in);
    mve_blk1_rsrc_dma_mem_free(session->buf_input_out);
    mve_blk1_rsrc_dma_mem_free(session->buf_output_in);
    mve_blk1_rsrc_dma_mem_free(session->buf_output_out);
    mve_blk1_rsrc_dma_mem_free(session->rpc_area);

    MVE_RSRC_MEM_FREE(session->role);
    ret = mve_blk1_fw_unload(session->mmu_ctx, session->fw_inst);
    if (false == ret)
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log_session, MVE_LOG_DEBUG, "Firmware unload failed.");
    }
    mve_blk1_mmu_destroy_ctx(session->mmu_ctx);

    MVE_RSRC_MEM_FREE(session);

    MVE_LOG_PRINT(&mve_blk1_rsrc_log_session, MVE_LOG_DEBUG, "[%s:%d]session free finish.\n", __func__, __LINE__);
}
...
static bool change_session_state(struct mve_session *session, enum MVE_SESSION_STATE to_state)
{
    ...
    MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_DEBUG, session, "Changing state. '%s' -> '%s'.", state_to_char[session->state.state], state_to_char[to_state]);
    ...
    {
        MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, session, "Invalid state change. '%s' -> '%s'.", state_to_char[session->state.state], state_to_char[to_state]);
    }
...
static void response_received(struct mve_session *session)
{
    ...
    if (false == res)
    {
        MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, session, "Invalid state change");
    }
                                      ...
                                      void *data)
{
    ...
    ret = mve_blk1_com_add_message(session, code, size, data);
...
{
    mve_blk1_queue_add_event(session->mve_events, event);
#ifdef EMULATOR
    {
...
static void return_pending_buffer_to_userspace(struct mve_session *session, struct session_enqueue_request *request, bool empty_this_buffer)
{
    ...
    event = mve_blk1_queue_create_event(event_code, sizeof(struct mve_base_buffer_userspace));
    if (NULL == event)
    {
        return;
    }
...
static mve_base_error enqueue_job(struct mve_session *session)
{
    ...
    cmd.cores = mver_blk1_scheduler_get_ncores();
...
static void enqueue_pending_buffers(struct mve_session *session, struct list_head *list, bool empty_this_buffer)
{
    ...
    list_for_each_safe(pos, next, list)
    {
        ...
        ret = mve_blk1_session_buffer_enqueue_internal(session, &request->param, empty_this_buffer, true);

        /* If request timed out, then firmware queue is full and we need to retry again later. */
        if (MVE_BASE_ERROR_TIMEOUT == ret)
        {
            MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_DEBUG,"[%s:%d]session:%px comm queue is full and retry again later, empty_this_buffer: %d\n", __func__, __LINE__, session,(int)empty_this_buffer);
            break;
        }
...
static bool schedule_session(struct mve_session *session)
{
    ...
    {
        ret = mver_blk1_scheduler_execute((mver_session_id)session);
    }
...
static void switchout_session(struct mve_session *session)
{
        ...
        STATE_SWITCHING_IN != session->state.state)
    {
        ...
        if (false != ret)
        {
            ...
            mver_blk1_irq_signal_mve(session->lsid);
        }
    }
...
{
    struct mve_base_fw_version *ver = mve_blk1_fw_get_version(session->fw_inst);
    bool ret = false;
...
static void process_generic_event(struct mve_session *session, struct mve_base_event_header *event)
{
    ...
    switch (event->code)
    {
        ...
        case MVE_BASE_EVENT_INPUT:
        {
            ...
            {
                mve_blk1_com_get_output_buffer(session, &buffer);
            }
            else
            {
                mve_blk1_com_get_input_buffer(session, &buffer);
            }
            ...
            if (NULL != buffer_client)
            {
                ...
                else
                {
                    ...
                    res = mve_blk1_session_buffer_dequeue_internal(session, buffer_client->buffer->info.buffer_id, &buffer);
                        ...
                        MVE_BASE_BUFFERFLAG_EOS == (buffer_client->flags & MVE_BASE_BUFFERFLAG_EOS))
                    {
                        ...
                        MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_DEBUG, session, "Resetting eos_queued.");
                    }

                    usr_event = mve_blk1_queue_create_event(event->code, sizeof(struct mve_base_buffer_userspace));
                    if (NULL == usr_event)
                    {
                        return;
                    }
                    ...
                    mve_blk1_session_buffer_convert_to_userspace(buf, buffer_client);
            ...
            else
            {
                ...
                MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_WARNING, "This buffer has already been sent to userspace. Don't send it again.");
                return;
            }
            ...
            if (0 != (session->state.flush_target & MVE_BASE_FLUSH_QUICK))
            {
                    ...
                    session->state.flush_state)
                {
                    ...
                    list_for_each_safe(pos, next, &session->quick_flush_buffers)
                    {
                        ...
                        if (0 > ptr->in_use)
                        {
                            MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_WARNING, "in_use should be gt 0, in_use: %d ", ptr->in_use);
                        }
                        ...
                        param.timestamp = 0;
                        ret = mve_blk1_session_buffer_enqueue_internal(session,
                                                                  &param,
                                                                  ptr->port_index == 0 ? true : false, true);
                        ...
                        else
                        {
                           if (MVE_BASE_ERROR_TIMEOUT == ret)
                           {
                               ...
                               {
                                   MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_WARNING, "[%s:%d]session:%px alloc request failed.", __func__, __LINE__, session);
                               }
                           }
                           else
                           {
                           	MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_WARNING, "[%s:%d]session:%px enqueue quick flush pending bufffer failed.",  __func__, __LINE__, session);
                           }
                        }
        ...
        case MVE_BASE_EVENT_FW_ERROR:
        {
            ...
            {
                MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, session, "Unsupported firmware.");
                ...
                if (STATE_WAITING_FOR_RESPONSE == session->state.state)
                {
                    mve_blk1_queue_interrupt(session->response_events);
                }
            }
            else if (NULL != e && MVE_ERROR_WATCHDOG == e->error_code && !strcmp(e->message, "Watchdog"))
            {
                ...
                host_area = mve_blk1_rsrc_dma_mem_map(session->msg_in_queue);
                mve_area = mve_blk1_rsrc_dma_mem_map(session->msg_out_queue);

                if (host_area != NULL && mve_area != NULL)
                {
                    MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, session, "host_area wpos: %d mve_area rpos: %d.", host_area->in_wpos, mve_area->in_rpos);
                }
                mve_blk1_rsrc_dma_mem_unmap(session->msg_in_queue);
                mve_blk1_rsrc_dma_mem_unmap(session->msg_out_queue);
            }
            break;
        }
        ...
        case MVE_BASE_EVENT_REF_FRAME_RELEASED:
        {
            ...
            if (NULL != buffer_client)
            {
                ...
                struct mve_base_event_header *usr_event;
                usr_event = mve_blk1_queue_create_event(event->code, sizeof(mve_base_buffer_handle_t));
                if (NULL == usr_event)
                {
                    return;
                }
        ...
        default:
        {
            MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, session, "Illegal event code. code=%u.", event->code);
            return;
        }
    }
...
static void handle_mve_message(struct mve_session *session)
{
    ...
    data = mve_blk1_com_get_message(session, &header);
    while (NULL != data)
    {
        enum mve_base_event_code event_code;
        struct mve_base_event_header *event;
        ...
        event = mve_blk1_queue_create_event(event_code, size);
        if (NULL != event)
        {
            ...
            if (MVE_BASE_EVENT_FW_ERROR == event->code)
            {
                ...
                MVE_LOG_PRINT(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, "Firmware error. reason=%u, msg=%s.", err->reason, err->msg);
            }
            ...
            switch (event_code)
            {
                case MVE_BASE_EVENT_SET_PARAMCONFIG:
                    ...
                    response_received(session);
                    mve_blk1_queue_add_event(session->response_events, event);
#ifdef EMULATOR
                    {
                        int t = (int)(((uintptr_t)session->filep) & 0xFFFFFFFF);
                        write(t, &t, sizeof(int));
...
mve_base_error mve_blk1_session_handle_rpc_mem_alloc(struct mve_session *session, int fd)
{
    ...
    err = mve_blk1_com_get_rpc_message(session, &rpc);
    if (MVE_BASE_ERROR_NONE == err)
    {
        ...
        buffer = mve_blk1_buffer_create_buffer_external(&info, 0);
        if (NULL != buffer)
        {
            bool ret = mve_blk1_buffer_map_physical_pages(buffer);
        ...
        if (NULL != pages)
        {
            ...
            {
                mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_PROTECTED, &region);
            }
            else
            {
                mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_OUT_BUF, &region);
            }

            ret = mve_blk1_mmu_map_pages_and_reserve(session->mmu_ctx,
                                                pages,
                                                &region,
                                                num_pages,
                                                max_pages,
            ...
            mve_blk1_buffer_unmap_physical_pages(buffer);
            mve_blk1_buffer_destroy_buffer_external(buffer);
            buffer = NULL;
        }

        MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_DEBUG, session,
...
error:
    ...
    mve_blk1_com_put_rpc_message(session, &rpc);
...
mve_base_error mve_blk1_session_handle_rpc_mem_resize(struct mve_session *session, int fd)
{
    struct mve_com_rpc rpc;
    mve_base_error err;
    ...
    if (MVE_BASE_ERROR_NONE == err)
    {
        ...
        ret = mve_blk1_mmu_map_info(session->mmu_ctx,
                               mve_addr,
                               &mapped_pages,
                               &max_pages);
        ...
        if (NULL != buffer)
        {
            ret = mve_blk1_buffer_map_physical_pages(buffer);
            if (false != ret && num_new_pages <= buffer->mapping.num_pages)
            {
                ret = mve_blk1_mmu_map_resize(session->mmu_ctx,
...
error:
        mver_blk1_scheduler_flush_tlb(session->lsid);

        MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_DEBUG, session, "RPC resize requested. new_size=%d, fd: %d, num_pages=%d, addr=0x%X ret_addr= %#x.", new_size, fd, num_pages, mve_addr, ret_addr);
...
static void handle_rpc_request(struct mve_session *session)
{
    ...
    if (MVE_BASE_ERROR_NONE == err)
    {
        ...
        {
            MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_INFO, session, "<- RPC %s", print_rpc_code(rpc.call_id));
        }
        ...
        switch (rpc.call_id)
        {
            case MVE_COM_RPC_FUNCTION_DEBUG_PRINTF:
            {
                ...
                event = mve_blk1_queue_create_event(MVE_BASE_EVENT_RPC_PRINT, size);
                if (NULL == event)
                {
                    return;
                }
            ...
            case MVE_COM_RPC_FUNCTION_MEM_ALLOC:
            {
                ...
                phys_addr_t *pages;
                uint32_t mve_addr = 0;
                bool secure = mve_blk1_fw_secure(session->fw_inst);
                ...
                if (false != secure)
                {
                    ...
                    MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_DEBUG, session, "%s send event to userspace size: %d", __FUNCTION__, num_pages * MVE_MMU_PAGE_SIZE);

                    event = mve_blk1_queue_create_event(MVE_BASE_EVENT_RPC_MEM_ALLOC, sizeof(struct mve_base_rpc_memory));
                    if (NULL != event)
                    {
		    ...
		    if(NULL != event)
		    {
                ...
                MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_INFO, session,
            ...
            case MVE_COM_RPC_FUNCTION_MEM_RESIZE:
            {
                ...
                if (false != secure)
                {
                    ...
                    bool ret = mve_blk1_mmu_map_info(session->mmu_ctx,
                                                mve_addr,
                                                &mapped_pages,
                                                &max_pages);
                    if (false == ret || num_pages > max_pages)
                    ...
                    if (num_pages > mapped_pages)
                    {
                        ...
                        MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_DEBUG, session, "%s send event to userspace old: %d extra: %d",
                                              __FUNCTION__, mapped_pages * MVE_MMU_PAGE_SIZE, num_new_pages * MVE_MMU_PAGE_SIZE);

                        event = mve_blk1_queue_create_event(MVE_BASE_EVENT_RPC_MEM_RESIZE, sizeof(struct mve_base_rpc_memory));
                        if (NULL != event)
                        {
                            rpc_memory = (struct mve_base_rpc_memory *)event->data;
                            rpc_memory->region = VIRT_MEM_REGION_PROTECTED == mve_blk1_mem_virt_region_type_get(mve_addr) ?
                ...
                {
                    bool ret = mve_blk1_mmu_map_resize(session->mmu_ctx,
                                                  NULL,
                                                  mve_addr,
                                                  num_pages,
                                                  ATTRIB_SHARED_RW,
...
resize_error:
                ...
                MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_INFO, session, "RPC resize requested. new_size=%d, num_pages=%d, mve_addr=0x%X, ret_addr: 0x%X.", new_size, num_pages, mve_addr, ret_addr);
            ...
            case MVE_RPC_FUNCTION_MEM_FREE:
            {
                ...
                {
                    mve_blk1_mmu_unmap_pages(session->mmu_ctx, rpc.params.mem_free.ve_pointer);
                }

                mver_blk1_scheduler_flush_tlb(session->lsid);

                MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_INFO, session, "RPC free requested. addr=0x%X.", rpc.params.mem_free.ve_pointer);
            ...
            default:
            {
                MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, session, "Unsupported RPC request. id=%u.", rpc.call_id);
                rpc.size = 0;
            }
        }
 ...
 */
void mve_blk1_session_irq_callback(mver_session_id session_id)
{
    ...
    /* Update the timestamp marking the most recent interaction with the HW */
        ...
        ktime_get_real_ts64(&session->state.timestamp);
#else
...
static void mve_session_hung(struct mve_session *session)
{
    ...
    event = mve_blk1_queue_create_event(MVE_BASE_EVENT_SESSION_HUNG, 0);
    if (NULL == event)
    {
        return;
    }
    ...
    mver_blk1_scheduler_stop((mver_session_id)session);
    mver_blk1_scheduler_unregister_session((mver_session_id)session);
}
...
static void mve_session_dump(struct mve_session *session)
{
...
static void session_watchdog(struct mve_session *session, struct timespec64 *tv)
{
    ...
    if (false != res)
    {
        ...
        {
            ktime_get_real_ts64(&session->state.timestamp);
            cnt = mve_blk1_com_message_ready(session, session->msg_in_queue, session->msg_out_queue);
            MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_DEBUG, session, "[%s]msg in message queue size:%u.", __func__, cnt);
            if (MAX_NUM_PINGS > session->state.ping_count)
            {
                mve_session_dump(session);
                //msleep(10);
...
static void session_destroy_instance(struct mve_session *session)
{
    ...
    MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_DEBUG, session,"[%s]enter.", __func__);
    mver_blk1_scheduler_stop((mver_session_id)session);
    mver_blk1_scheduler_unregister_session((mver_session_id)session);
    ...
    MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_INFO, session, "Session destroyed.");

    mver_blk1_pm_request_suspend();
...
static void mve_session_switchin_callback(mver_session_id session_id)
{
    ...
    else
    {
        ...
        MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session,
                              MVE_LOG_DEBUG,
                              session,
...
static void mve_session_switchout_completed_callback(mver_session_id session_id)
{
    ...
    mve_blk1_session_irq_callback(session_id);
...
#define TRACE_SIZE 128 * 1024
    ...
    if (NULL == str)
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, "Unable to allocate memory for FW trace buffer.");
        return -EBUSY;
    }
                ...
                if ((1 << rasc) & fw_trace->rasc_mask)
                {
                    ...
                    if (0 != rasc_addr)
                    {
                        ...
                        if (NULL != buffer)
                        {
                            ...
                            mve_blk1_mmu_read_buffer(session->mmu_ctx, rasc_addr, (uint8_t *)buffer, size);
...
void mve_blk1_session_init(struct device *dev)
{
    sema_init(&sessions_sem, 1);
    sema_init(&session_destroy_sem,1);
    num_sessions = 0;
...
#if defined(_DEBUG) && !defined(EMULATOR)
    {
        ...
        parent_dentry = mve_blk1_rsrc_log_get_parent_dir();
        dentry = debugfs_create_file("fw_trace_blk1", 0400, parent_dentry, NULL, &trace_fops);
        if (IS_ERR_OR_NULL(dentry))
        {
            MVE_LOG_PRINT(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, "Unable to create debugfs file 'fw_trace'.");
        }
    }
...
#if defined(CONFIG_SYSFS)
    {
        ...
        for (i = 0; i < NELEMS(sysfs_files); ++i)
        {
            ...
            if (err < 0)
            {
                MVE_LOG_PRINT(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, "Unable to create sysfs file.");
            }
        }
    }
#endif /* CONFIG_SYSFS */
...
void mve_blk1_session_deinit(struct device *dev)
{
#ifndef EMULATOR
#if defined(CONFIG_SYSFS) && defined(_DEBUG)
    int i;
...
struct dentry *parent_dentry;
    ...
    mve_blk1_session_destroy_all_sessions();
}

void mve_blk1_session_destroy_all_sessions(void)
{
    struct list_head *pos, *next;
    ...
    list_for_each_safe(pos, next, &sessions)
    {
        ...
        if (NULL != ptr)
        {
            ...
            sem_taken = down_interruptible(&session_destroy_sem);
            session_destroy_instance(ptr);
            if (0 == sem_taken)
            {
                up(&session_destroy_sem);
            }
        }
    }
}

struct mve_session *mve_blk1_session_create(struct file *filep)
{
    struct mve_session *session;
    struct mve_mem_virt_region region;
    bool ret;
    ...
    MVE_LOG_PRINT(&mve_blk1_rsrc_log_session, MVE_LOG_DEBUG,"[%s:%d]session:%p enter.\n", __func__, __LINE__,  session);
    session->mmu_ctx = mve_blk1_mmu_create_ctx();
    ...
    session->msg_in_queue   = mve_blk1_rsrc_dma_mem_alloc(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->msg_out_queue  = mve_blk1_rsrc_dma_mem_alloc(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->buf_input_in   = mve_blk1_rsrc_dma_mem_alloc(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->buf_input_out  = mve_blk1_rsrc_dma_mem_alloc(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->buf_output_in  = mve_blk1_rsrc_dma_mem_alloc(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->buf_output_out = mve_blk1_rsrc_dma_mem_alloc(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    session->rpc_area       = mve_blk1_rsrc_dma_mem_alloc(MVE_MMU_PAGE_SIZE, DMA_MEM_TYPE_UNCACHED);
    ...
    mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_MSG_IN_QUEUE, &region);
    pages = mve_blk1_rsrc_dma_mem_get_pages(session->msg_in_queue);
    ret = mve_blk1_mmu_map_pages(session->mmu_ctx, pages,
    ...
    mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_MSG_OUT_QUEUE, &region);
    pages = mve_blk1_rsrc_dma_mem_get_pages(session->msg_out_queue);
    ret = mve_blk1_mmu_map_pages(session->mmu_ctx, pages,
                            region.start, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_INPUT_BUFFER_IN, &region);
    pages = mve_blk1_rsrc_dma_mem_get_pages(session->buf_input_in);
    ret = mve_blk1_mmu_map_pages(session->mmu_ctx, pages,
                            region.start, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_INPUT_BUFFER_OUT, &region);
    pages = mve_blk1_rsrc_dma_mem_get_pages(session->buf_input_out);
    ret = mve_blk1_mmu_map_pages(session->mmu_ctx, pages,
                            region.start, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_OUTPUT_BUFFER_IN, &region);
    pages = mve_blk1_rsrc_dma_mem_get_pages(session->buf_output_in);
    ret = mve_blk1_mmu_map_pages(session->mmu_ctx, pages,
                            region.start, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_OUTPUT_BUFFER_OUT, &region);
    pages = mve_blk1_rsrc_dma_mem_get_pages(session->buf_output_out);
    ret = mve_blk1_mmu_map_pages(session->mmu_ctx, pages,
                            region.start, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    /* Map RPC area */
    mve_blk1_mem_virt_region_get(VIRT_MEM_REGION_RPC_QUEUE, &region);
    pages = mve_blk1_rsrc_dma_mem_get_pages(session->rpc_area);
    ret = mve_blk1_mmu_map_pages(session->mmu_ctx, pages,
                            region.start, 1, ATTRIB_SHARED_RW, ACCESS_READ_WRITE, false);
    if (false == ret)
    {
    ...
    session->mve_events = mve_blk1_queue_create(16384);
    session->response_events = mve_blk1_queue_create(8192);
    ...
    MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_INFO, session, "Session created.");
...
void mve_blk1_session_destroy(struct mve_session *session)
{
    bool ret;
    int sem_taken;
...
struct mve_session *mve_blk1_session_get_by_file(struct file *filep, bool clean)
{
    struct list_head *pos, *next;
    struct mve_session *session = NULL;
    int sem_taken = -1;
...
void mve_blk1_session_cleanup_client(struct file *filep)
{
    struct mve_session *session;

    down(&sessions_sem);
    session = mve_blk1_session_get_by_file(filep, true);
    if (NULL != session)
    {
        list_del(&session->list);
		up(&sessions_sem);
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_INFO,"%s session non NULL, release semaphore. then destory instance \n", __func__);
    ...
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_INFO,"%s session is NULL, release semaphore\n", __func__);
        up(&sessions_sem);
    }
    
}

void mve_blk1_session_destroy_all(void)
{
    struct list_head *pos, *next;
...
void mve_blk1_session_set_role(struct mve_session *session, char *role)
{
    bool ret;
...
bool mve_blk1_session_activate(struct mve_session *session, uint32_t *version,
                          struct mve_base_fw_secure_descriptor *fw_secure_desc)
{
    ...
    ncores = mver_blk1_scheduler_get_ncores();
    ...
    session->fw_inst = mve_blk1_fw_load(session->mmu_ctx, fw_secure_desc, session->role, ncores);
    if (NULL != session->fw_inst)
    {
        ...
        ver = mve_blk1_fw_get_version(session->fw_inst);
        if (NULL != ver)
        {
        ...
        error = mve_blk1_com_set_interface_version(session, session->prot_ver);
        mve_blk1_fw_log_fw_binary(session->fw_inst, session);

        if (error == MVE_BASE_ERROR_NONE)
        {
            l1_page = mve_blk1_mmu_get_id(session->mmu_ctx);
            mver_blk1_scheduler_register_session((mver_session_id)session,
                                            mve_mmu_make_l0_entry(ATTRIB_PRIVATE, l1_page),
                                            ncores,
                                            mve_blk1_fw_secure(session->fw_inst),
                                            mve_blk1_session_irq_callback,
...
mve_base_error mve_blk1_session_enqueue_flush_buffers(struct mve_session *session, uint32_t flush)
{
    bool res;
...
mve_base_error mve_blk1_session_enqueue_state_change(struct mve_session *session, enum mve_base_hw_state state)
{
    bool res;
    mve_base_error ret = MVE_BASE_ERROR_NONE;
...
struct mve_base_event_header *mve_blk1_session_get_event(struct mve_session *session, uint32_t timeout)
{
    ...
    if (true == session->fw_loaded)
    {
        event = mve_blk1_queue_wait_for_event(session->mve_events, timeout);
    }
...
mve_base_error mve_blk1_session_set_paramconfig(struct mve_session *session,
                                           uint32_t size,
                                           void *data,
                                           ...
                                           enum mve_base_command_type entity)
{
    ...
    if (MVE_BASE_ERROR_NONE == ret)
    {
        ...
        {
            unlock_session(session);
            event = mve_blk1_queue_wait_for_event(session->response_events, 10000);
            lock_session(session);

            if (NULL == event)
            {
                if (false != mve_blk1_queue_interrupted(session->response_events))
                {
                    ret = MVE_BASE_ERROR_HARDWARE;
                }
                else
...
mve_base_error mve_blk1_session_buffer_register(struct mve_session *session,
                                           uint32_t port_index,
                                           struct mve_base_buffer_userspace *descriptor)
{
    ...
    {
        uint32_t fuse = mver_blk1_reg_get_fuse();
    ...
    lock_session(session);
    ret = mve_blk1_session_buffer_register_internal(session, port_index, descriptor);
    unlock_session(session);
    release_session(session);
...
mve_base_error mve_blk1_session_buffer_unregister(struct mve_session *session,
                                             mve_base_buffer_handle_t buffer_id)
{
    ...
    lock_session(session);
    ret = mve_blk1_session_buffer_unregister_internal(session, buffer_id);
    unlock_session(session);
    release_session(session);
	if (0 == sem_taken)
	{ 
...
mve_base_error mve_blk1_session_buffer_enqueue(struct mve_session *session,
                                          struct mve_base_buffer_details *param,
                                          bool empty_this_buffer)
{
    ...
    if (false != empty_this_buffer)
    {
        ...
        {
            MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_DEBUG, session, "Setting eos_queued.");
        }
    }
    else
    {
    ...
    if (flush_in || flush_out)
    {
        ...
        {
            struct mve_buffer_client *buffer_client = mve_blk1_session_buffer_get_external_descriptor(session, param->buffer_id);
    ...
    ret = mve_blk1_session_buffer_enqueue_internal(session, param, empty_this_buffer, false);
...
mve_base_error mve_blk1_session_buffer_notify_ref_frame_release(struct mve_session *session,
                                                           mve_base_buffer_handle_t buffer_id)
{
    mve_base_error ret = MVE_BASE_ERROR_NONE;
    bool res;
    ...
    lock_session(session);
    mve_blk1_session_buffer_notify_ref_frame_release_internal(session, buffer_id);
    unlock_session(session);
    res = schedule_session(session);
    if (false != res)
    {
...
mve_base_error mve_blk1_session_enqueue_message(struct mve_session *session,
                                           uint32_t code,
                                           uint32_t size,
                                           void *data)
{
...
#ifndef EMULATOR
uint32_t mve_blk1_session_poll(struct file *filp, struct poll_table_struct *poll_table)
{
    struct mve_session *session;
    uint32_t mask = 0;

    session = mve_blk1_session_get_by_file(filp, false);
    if (NULL != session)
    {
        lock_session(session);
        ...
        if (false != mve_blk1_queue_event_ready(session->mve_events))
        {
