...
struct mve_queue *mve_blk1_queue_create(uint32_t size)
{
...
void mve_blk1_queue_destroy(struct mve_queue *queue)
{
    if (NULL != queue)
    {
        MVE_RSRC_MEM_VFREE(queue->buffer);
...
void mve_blk1_queue_interrupt(struct mve_queue *queue)
{
    queue->interrupted = true;
    wake_up_interruptible(&queue->wait_queue);
}

bool mve_blk1_queue_interrupted(struct mve_queue *queue)
{
    return queue->interrupted;
}

struct mve_base_event_header *mve_blk1_queue_wait_for_event(struct mve_queue *queue, uint32_t timeout)
{
    ...
    event = mve_blk1_queue_create_event(header.code, header.size);
    if (NULL == event)
    {
...
void mve_blk1_queue_add_event(struct mve_queue *queue,
                         struct mve_base_event_header *event)
{
        ...
        queue->rpos > buf_size)
    {
        /* Drop this event! */
        /* WARN_ON(true); */
        printk("mve_queue_add_event drop this event: %d \n", event->code);
        up(&queue->consumer_lock);
        return;
    }
...
struct mve_base_event_header *mve_blk1_queue_create_event(int code, int size)
{
    struct mve_base_event_header *event;
...
bool mve_blk1_queue_event_ready(struct mve_queue *queue)
{
