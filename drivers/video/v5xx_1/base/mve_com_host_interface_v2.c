...
#include "host_interface_v2/mve_blk1_protocol_def.h"
...
#include "mve_blk1_rsrc_log_ram.h"
...
#define NEED_QUEUE_CACHE_OPS

typedef enum
{
    QUEUE_INVALIDATE_CACHE,
    QUEUE_CLEAN_CACHE,
}QUEUE_CACHE_OPS;
 ...
 * Functions
 ****************************************************************************/
static int queue_cache_ops(struct mve_rsrc_dma_mem_t *area, int flag)
{
#ifdef NEED_QUEUE_CACHE_OPS
    phys_addr_t *pa = NULL;

    pa = mve_blk1_rsrc_dma_mem_get_pages(area);
    if(NULL == pa)
    {
        return -1;
    }

    if(QUEUE_INVALIDATE_CACHE == flag)
    {
        mve_blk1_rsrc_mem_invalidate_cache_range(pa[0], area->size);
    }
    else if(QUEUE_CLEAN_CACHE == flag)
    {
        mve_blk1_rsrc_mem_clean_cache_range(pa[0], area->size);
    }
    else
    {
        return -1;
                        ...
                        void *data)
{
    ...
    MVE_LOG_DATA(&mve_blk1_rsrc_log_fwif, MVE_LOG_INFO, vec, vecs);
}
...
uint32_t mve_blk1_com_message_ready(struct mve_session *session, struct mve_rsrc_dma_mem_t *host, struct mve_rsrc_dma_mem_t *mve)
{
    ...
    host_area = mve_blk1_rsrc_dma_mem_map(host);
    mve_area = mve_blk1_rsrc_dma_mem_map(mve);
    ...
    mve_blk1_rsrc_dma_mem_invalidate_cache(mve);

    /* Check that there is message in queue or not. */
    ...
    mve_blk1_rsrc_mem_flush_write_buffer();

out:
    mve_blk1_rsrc_dma_mem_clean_cache(host);
    mve_blk1_rsrc_dma_mem_unmap(host);
    mve_blk1_rsrc_dma_mem_unmap(mve);
                                         ...
                                         uint32_t size)
{
    ...
    queue_cache_ops(mve, QUEUE_INVALIDATE_CACHE);
    ...
    *(uint32_t *)msg_header = read_32(mve_area, &rpos);
    ...
    MVE_LOG_EXECUTE(&mve_blk1_rsrc_log_fwif, MVE_LOG_INFO,
                    log_message(session, channel, MVE_LOG_FWIF_DIRECTION_FIRMWARE_TO_HOST, msg_header, data));

out:
    queue_cache_ops(host, QUEUE_CLEAN_CACHE);
                                         ...
                                         struct mve_com_buffer_frame *frame)
{
    ...
    mve_frame.frame_flags |= (frame->nMVEFlags & MVE_BASE_BUFFER_FRAME_FLAG_MIRROR_HORI) ? MVE_BUFFER_FRAME_FLAG_MIRROR_HORI : 0;
    mve_frame.frame_flags |= (frame->nMVEFlags & MVE_BASE_BUFFER_FRAME_FLAG_MIRROR_VERT) ? MVE_BUFFER_FRAME_FLAG_MIRROR_VERT : 0;

    mve_frame.frame_flags |= (frame->nMVEFlags & MVE_BASE_BUFFER_FRAME_FLAG_SCALING_MASKX);
    mve_frame.frame_flags |= (frame->nMVEFlags & MVE_BASE_BUFFER_FRAME_FLAG_SCALING_MASKY);
    ...
    switch (frame->format)
    {
        ...
        case MVE_BASE_BUFFER_FORMAT_YUV420_I420_10:
            ...
            break;
		case MVE_BASE_BUFFER_FORMAT_YUV420_P010:
			mve_frame.format = MVE_FORMAT_YUV420_P010;
			break;	
        ...
        case MVE_BASE_BUFFER_FORMAT_ABGR_8888:
            ...
            break;
        default:
            MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, session, "Illegal com buffer format. format=%u", frame->format);
...
static mve_base_error write_buffer_general(struct mve_session *session,
                                             struct mve_rsrc_dma_mem_t *host,
                                             struct mve_rsrc_dma_mem_t *mve,
                                             enum mve_log_fwif_channel channel,
                                             struct mve_com_buffer_general *gerneral)
{
    mve_base_error ret;
    struct mve_buffer_general mve_general;

    /* Convert com- to fw buffer. */
    mve_general.header.host_handle = gerneral->header.host_handle;
    mve_general.header.user_data_tag = gerneral->header.user_data_tag;
    mve_general.header.buffer_ptr = gerneral->header.buffer_ptr;
    mve_general.header.buffer_size = gerneral->header.buffer_size;
    mve_general.header.config_size= gerneral->header.config_size;
    mve_general.header.type = gerneral->header.type;
    memcpy(&mve_general.config, &gerneral->config, sizeof(gerneral->config));
    ret = write_to_queue_v2(session, host, mve, channel, MVE_BUFFER_CODE_GENERAL, sizeof(mve_general), &mve_general);
                                       ...
                                       struct mve_com_buffer_roi *roi)
{
    ...
    struct mve_buffer_param mve_qp;
    int i;

    mve_qp.type = MVE_BUFFER_PARAM_TYPE_QP;
    mve_qp.data.qp.qp = roi->qp;
    ret = write_to_queue_v2(session,
                            host,
                            mve,
                            channel,
                            MVE_BUFFER_CODE_PARAM,
                            sizeof(mve_qp.data.qp) + sizeof(mve_qp.type),
                            &mve_qp);
    ...
    for (i = 0; i < roi->nRegions; ++i)
    {
        ...
        mve_regions.data.regions.region[i].prio       = roi->regions[i].prio;
    }
    ret = write_to_queue_v2(session,
                            host,
                            mve,
...
static void convert_buffer_general(struct mve_buffer_general *mve_general,
                                        struct mve_com_buffer_general *general){
    //currently we do nothing here
    general->header.host_handle = mve_general->header.host_handle;
    general->header.buffer_size = mve_general->header.buffer_size;
    general->header.config_size = mve_general->header.config_size;
    general->header.buffer_ptr = mve_general->header.buffer_ptr;
    general->config.block_configs.blk_cfgs.rows_uncomp.n_cols_minus1 =
                    mve_general->config.block_configs.blk_cfgs.rows_uncomp.n_cols_minus1;
    general->config.block_configs.blk_cfgs.rows_uncomp.n_rows_minus1 =
                    mve_general->config.block_configs.blk_cfgs.rows_uncomp.n_rows_minus1;
}

static void convert_buffer_frame(struct mve_buffer_frame *mve_frame,
                                        struct mve_com_buffer_frame *frame){
...
static void convert_buffer_bitstream(struct mve_buffer_bitstream *mve_bitstream,
                    struct mve_com_buffer_bitstream *bitstream){
                                   ...
                                   size_t size)
{
    ...
    if (header->size != size)
    {
        MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_WARNING, session, "Com message size did not match expected size. code=%u, size=%u, expected=%u.",
    ...
    *out = MVE_RSRC_MEM_CACHE_ALLOC(size, GFP_KERNEL);
    if (*out == NULL)
    {
        MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_WARNING, session, "Failed to allocate memory for com message.");
        header->size = 0;
        return;
    }
                             ...
                             struct mve_msg_header *header)
{
    ...
    switch (header->code)
    {
        ...
        case MVE_RESPONSE_CODE_EVENT:
        {
            ...
            switch (response_v2.event.event_code)
            {
                ...
                case MVE_EVENT_PROCESSED:
                {
                    ...
                    {
                        MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, session, "Com event processed message should have size 0 or %u. size=%u.",
                ...
                default:
                {
                    MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, session, "Received com message with illegal event code. code=%u.", response_v2.event.event_code);
                    break;
                }
            }
            break;
        ...
        default:
        {
            MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, session, "Received com message with illegal code. code=%u.", header->code);
            break;
        }
    }
                                       ...
                                       enum mve_com_buffer_type type)
{
    ...
    switch (type)
    {
        ...
        case MVE_COM_BUFFER_TYPE_ROI:
            ...
            break;
        case MVE_COM_BUFFER_TYPE_GENERAL:
            ret = write_buffer_general(session, session->buf_input_in, session->buf_input_out, MVE_LOG_FWIF_CHANNEL_INPUT_BUFFER, &buffer->general);
            break;
        default:
            //WARN_ON(true);
            MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, session,"add_input_buffer BAD_PARAMETER");
            ret = MVE_BASE_ERROR_BAD_PARAMETER;
            break;
    }
                                        ...
                                        enum mve_com_buffer_type type)
{
    ...
    switch (type)
    {
        ...
        default:
            ...
            MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, session,"add_output_buffer BAD_PARAMETER");
            ret = MVE_BASE_ERROR_BAD_PARAMETER;
            break;
    }
                                       ...
                                       mve_com_buffer *buffer)
{
    ...
    ret = read_from_queue_v2(session, session->buf_input_in, session->buf_input_out, MVE_LOG_FWIF_CHANNEL_OUTPUT_BUFFER, &msg_header, &fw_msg, sizeof(fw_msg));
    if (ret != MVE_BASE_ERROR_NONE)
    {
        return ret;
    }
    ...
    switch (msg_header.code) {
        case MVE_BUFFER_CODE_FRAME:
            convert_buffer_frame(&fw_msg.mve_frame, &buffer->frame);
            break;
        case MVE_BUFFER_CODE_BITSTREAM:
            convert_buffer_bitstream(&fw_msg.mve_bitstream, &buffer->bitstream);
            break;
        case MVE_BUFFER_CODE_GENERAL:
            convert_buffer_general(&fw_msg.mve_general, &buffer->general);
            break;
        default:
            MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session, MVE_LOG_ERROR, session, "Unrecognized code. code=%u", msg_header.code);
            break;
    }
#if 0
                                      ...
                                      mve_com_rpc *rpc)
{
    ...
    rpc_area = mve_blk1_rsrc_dma_mem_map(session->rpc_area);
    if (NULL == rpc_area)
    {
        return MVE_BASE_ERROR_INSUFFICIENT_RESOURCES;
    }

    queue_cache_ops(session->rpc_area, QUEUE_INVALIDATE_CACHE);

    mve_blk1_rsrc_dma_mem_invalidate_cache(session->rpc_area);
    ...
    if (MVE_RPC_STATE_PARAM == rpc_area->state)
    {
        ...
        switch (rpc_area->call_id)
        {
            case MVE_RPC_FUNCTION_DEBUG_PRINTF:
                ...
                mve_blk1_rsrc_log_trim(rpc_area->params.debug_print.string);
                break;
    ...
    mve_blk1_rsrc_dma_mem_unmap(session->rpc_area);
                                      ...
                                      mve_com_rpc *rpc)
{
    ...
    wmb();
    mve_blk1_rsrc_dma_mem_clean_cache(session->rpc_area);
...
struct mve_com *mve_blk1_com_host_interface_v2_new(void)
{
    ...
    if (com == NULL)
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_WARNING, "Failed to allocate com object.");
        return NULL;
    }
