 ...
 */
static mve_base_error mve_blk1_command_create_session_helper(void *data,
                                                        struct file *filep)
{
    ...
    if (false == is_role_enabled(role))
    {
        ...
        MVE_LOG_PRINT(&mve_blk1_rsrc_log_session,
                      MVE_LOG_ERROR,
    ...
    {
        session = mve_blk1_session_create(filep);
        if (NULL == session)
        {
        ...
        {
            mve_blk1_session_set_role(session, role);
        }
    }
...
struct mve_response *mve_blk1_command_execute(struct mve_base_command_header *header,
                                         void *data,
                                         struct file *filep)
{
#define MVE_LOG_COMMAND(_severity)                                                                                   \
    ...
    if (NULL == ret)
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_ERROR, "Failed to allocate memory for the ioctl return struct.");
        return NULL;
    }
    ...
    */
    if (NULL == data)
    {
        ...
        {
            MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_ERROR, " data is NULL.");
    ...
    session = mve_blk1_session_get_by_file(filep, false);
    ...
    switch (header->cmd)
    {
        case MVE_BASE_CREATE_SESSION:
            ...
            {
                ret->error = mve_blk1_command_create_session_helper(data, filep);
                MVE_LOG_COMMAND(MVE_LOG_INFO);
            }
            break;
        case MVE_BASE_DESTROY_SESSION:
            MVE_LOG_COMMAND(MVE_LOG_INFO);
            mve_blk1_session_destroy(session);
            break;
        case MVE_BASE_ACTIVATE_SESSION:
        {
            ...
            if (NULL != version)
            {
                res = mve_blk1_session_activate(session, version, fw_secure_desc);
                if (false != res)
                {
                    ret->data = version;
        ...
        case MVE_BASE_ENQUEUE_FLUSH_BUFFERS:
        {
            ...
            ret->error = mve_blk1_session_enqueue_flush_buffers(session, flush);
            break;
        }
        case MVE_BASE_ENQUEUE_STATE_CHANGE:
        {
            ...
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_blk1_session_enqueue_state_change(session, state);
            break;
        }
        case MVE_BASE_GET_EVENT:
        {
            ...
            event = mve_blk1_session_get_event(session, timeout);
            if (NULL == event)
            {
        ...
        {
	        //MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_blk1_session_set_paramconfig(session,
                                                     header->size,
                                                     data,
                                                     &ret->firmware_error,
                                                     header->cmd);
        ...
        case MVE_BASE_REGISTER_BUFFER:
        {
            ...
            if (header->size != sizeof(uint32_t) + sizeof(struct mve_base_buffer_userspace))
            {
                MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session,
                                      MVE_LOG_ERROR,
                                      session,
                                      "MVE_REGISTER_BUFFER data size no match.");
            }
            ...
            {
                ret->error = mve_blk1_session_buffer_register(session, port_index, descriptor);
            }
            break;
        }
        case MVE_BASE_UNREGISTER_BUFFER:
        {
            ...
            {
                MVE_LOG_COMMAND(MVE_LOG_INFO);
                ret->error = mve_blk1_session_buffer_unregister(session, buffer_id);
            }
            break;
        }
        case MVE_BASE_FILL_THIS_BUFFER:
        {
            ...
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_blk1_session_buffer_enqueue(session, param, false);
            break;
        }
        case MVE_BASE_EMPTY_THIS_BUFFER:
        {
            ...
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_blk1_session_buffer_enqueue(session, param, true);
            break;
        }
        case MVE_BASE_NOTIFY_REF_FRAME_RELEASE:
        {
            ...
            MVE_LOG_COMMAND(MVE_LOG_DEBUG);
            ret->error = mve_blk1_session_buffer_notify_ref_frame_release(session, buffer_id);
            break;
        }
        case MVE_BASE_REQUEST_MAX_FREQUENCY:
        {
            ...
            mver_blk1_dvfs_request_max_frequency();
            ret->error = MVE_BASE_ERROR_NONE;
            break;
        }
        case MVE_BASE_READ_HW_INFO:
        {
            ...
            else
            {
                ...
                hw_info->fuse = mver_blk1_reg_get_fuse();
                hw_info->version = mver_blk1_reg_get_version();
                hw_info->ncores = mver_blk1_scheduler_get_ncores();
            }
            break;
        }
        case MVE_BASE_RPC_MEM_ALLOC:
        {
            ...
            fd = *((int32_t *)data);
            ret->error = mve_blk1_session_handle_rpc_mem_alloc(session, fd);
#endif      /* EMULATOR */
            break;
        }
        case MVE_BASE_RPC_MEM_RESIZE:
        {
            ...
            fd = *((int32_t *)data);
            ret->error = mve_blk1_session_handle_rpc_mem_resize(session, fd);
#endif      /* EMULATOR */
            break;
        }
#ifdef UNIT
        case MVE_BASE_DEBUG_READ_REGISTER:
            ...
            else
            {
                ...
                value = mver_blk1_reg_read32(base + offset);
        ...
        case MVE_BASE_DEBUG_WRITE_REGISTER:
            ...
            WARN_ON(header->size != sizeof(uint32_t) * 2);
            {
                ...
                mver_blk1_reg_write32(base + offset, value);
                mver_blk1_reg_put_coresched_bank(&regs);
            }
            break;
        case MVE_BASE_DEBUG_INTERRUPT_COUNT:
            ...
            {
                *((uint32_t *)ret->data) = mve_blk1_rsrc_data.interrupts;
                ret->size = sizeof(uint32_t);
            }
            break;
    ...
    if (ret->data != NULL && ret->size == 0)
    {
        MVE_LOG_PRINT_SESSION(&mve_blk1_rsrc_log_session,
                                  MVE_LOG_WARNING,
                                  session,
                                  "return data is not NULL but size is zero, pls check it.");
    }
