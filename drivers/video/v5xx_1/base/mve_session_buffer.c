...
#include "mve_blk1_buffer.h"
#include "mve_blk1_mmu.h"
...
#define OMX_BUFFERFLAG_GENERAL 0x800
...
static bool map_external_buffer_mve(struct mve_session *session,
                                    ...
                                    uint32_t alignment)
{
    return mve_blk1_mmu_map_pages_and_reserve(session->mmu_ctx,
...
{
    mve_blk1_mmu_unmap_pages(session->mmu_ctx, buffer->mapping.region.start);
    mver_blk1_scheduler_flush_tlb(session->lsid);
}

struct mve_buffer_client *mve_blk1_session_buffer_get_external_descriptor(struct mve_session *session,
...
void mve_blk1_session_buffer_unmap_all(struct mve_session *session)
{
                                                         ...
                                                         struct mve_buffer_info *info)
{
    ...
    buffer = mve_blk1_buffer_create_buffer_external(info, port_index);
    if (NULL == buffer)
    {
        return NULL;
    }

    mve_blk1_mem_virt_region_get(region_type, &buffer->mapping.region);

    res = mve_blk1_buffer_map_physical_pages(buffer);
    if (false == res)
    {
        mve_blk1_buffer_destroy_buffer_external(buffer);
        return NULL;
    ...
    else
    {
        if (false != info->do_cache_maintenance)
        {
            ...
            mve_blk1_buffer_set_owner(buffer, MVE_BUFFER_OWNER_CPU, port_index);
        }
...
mve_base_error mve_blk1_session_buffer_register_internal(struct mve_session *session,
                                                    uint32_t port_index,
                                                    struct mve_base_buffer_userspace *userspace_buffer)
{
    ...
    buffer = mve_blk1_session_buffer_get_external_descriptor(session, userspace_buffer->buffer_id);
    if (NULL != buffer)
    {
    ...
    res = mve_blk1_buffer_is_valid(&buffer_info) &&
          (0 != crc_info.handle ? mve_blk1_buffer_is_valid(&crc_info) : true);
    if (false == res)
    {
#ifdef _DEBUG
        WARN_ON(true);
...
mve_base_error mve_blk1_session_buffer_unregister_internal(struct mve_session *session,
                                                      mve_base_buffer_handle_t buffer_id)
{
    struct mve_buffer_client *buffer;
    mve_base_error ret = MVE_BASE_ERROR_NONE;

    buffer = mve_blk1_session_buffer_get_external_descriptor(session, buffer_id);
    if (NULL == buffer)
    {
    ...
    else
    {
        if (0 != buffer->in_use)
        {
            ...
            //WARN_ON(true);
            MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_WARNING, "The buffer is still in use! Do not unmap it.");
...
mve_base_error mve_blk1_session_buffer_enqueue_internal(struct mve_session *session,
                                                   struct mve_base_buffer_details *param,
                                                   bool empty_this_buffer, bool pending_buffer)
{
    ...
    if (false == res && false == pending_buffer)
    {
        /* There are already buffers queued waiting to be added to the firmware queue. */
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_DEBUG, "[%s:%d]session:%px there are already buffers queued waiting to be added to comm queue, empty_this_buffer: %d\n", __func__, __LINE__, session, (int)empty_this_buffer);
        return MVE_BASE_ERROR_TIMEOUT;
    }

    buffer_client = mve_blk1_session_buffer_get_external_descriptor(session, param->buffer_id);
    ...
    if (false != empty_this_buffer && param->flags & OMX_BUFFERFLAG_GENERAL) {
        buffer_type = MVE_COM_BUFFER_TYPE_GENERAL;
        buf.general.header.buffer_size = param->header_general.buffer_size;
        buf.general.header.config_size = param->header_general.config_size;
        buf.general.header.type = param->header_general.type;
        memcpy(&buf.general.config.block_configs, &param->header_general.config, sizeof(param->header_general.config));
        buf.general.header.host_handle = buffer_client->mve_handle;
        buf.general.header.buffer_ptr = external_buffer->mapping.region.start + offset_in_page;

        ret = mve_blk1_com_add_input_buffer(session, &buf, buffer_type);
        ...
        case MVE_BASE_BUFFER_FORMAT_YV12:
            ...
            if (MVE_BASE_BUFFER_FORMAT_YV12 == buffer_info->format)
            {
                ...
                buf.frame.data.planar.plane_top[1] = buf.frame.data.planar.plane_top[2] +
                                                     buf.frame.data.planar.stride[2] * ((max_height + 1) >> 1);
            }
            else /* if (MVE_BASE_BUFFER_FORMAT_YUV420_PLANAR == buffer_info->format) */
            {
                ...
                buf.frame.data.planar.plane_top[2] = buf.frame.data.planar.plane_top[1] +
                                                     buf.frame.data.planar.stride[1] * ((max_height + 1) >> 1);
            }
        ...
        0 != param->nRegions)
    {
        ...
        roi_buf.roi.qp = param->qp;
        memcpy(roi_buf.roi.regions, param->regions, param->nRegions * sizeof(param->regions[0]));

        ret = mve_blk1_com_add_input_buffer(session, &roi_buf, MVE_COM_BUFFER_TYPE_ROI);
        //WARN_ON(ret != MVE_BASE_ERROR_NONE);
        if (ret != MVE_BASE_ERROR_NONE)
        {
            printk("Send ROI rectangles to the FW failed \n");
        }
    }
    ...
    {
        mve_blk1_buffer_set_owner(buffer_client->buffer, MVE_BUFFER_OWNER_DEVICE, buffer_client->port_index);
        if (NULL != buffer_client->crc)
        {
            mve_blk1_buffer_set_owner(buffer_client->crc, MVE_BUFFER_OWNER_DEVICE, buffer_client->port_index);
        }
    }

    if (false == empty_this_buffer)
    {
        ret = mve_blk1_com_add_output_buffer(session, &buf, buffer_type);
    }
    else
    {
        ret = mve_blk1_com_add_input_buffer(session, &buf, buffer_type);
    ...
    if (MVE_BASE_ERROR_NONE != ret)
    {
        ...
        buffer_client->mve_handle = MVE_HANDLE_INVALID;
        //WARN_ON(0 < buffer_client->in_use);
        if (0 < buffer_client->in_use)
        {
            MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_WARNING, "in_use should  be eq 0. in_use: %d ", buffer_client->in_use);
        }
...
mve_base_error mve_blk1_session_buffer_dequeue_internal(struct mve_session *session,
                                                   mve_base_buffer_handle_t buffer_id,
                                                   mve_com_buffer *buffer)
{
    ...
    buffer_client = mve_blk1_session_buffer_get_external_descriptor(session, buffer_id);
    if (NULL == buffer_client)
    {
        /* The supplied user-space allocated buffer has not been registered
         * or is invalid. */
    ...
    if (0 > buffer_client->in_use)
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_WARNING, "in_use should not be lt 0. in_use: %d ", buffer_client->in_use);
    }
             ...
             MVE_BASE_BUFFER_FORMAT_YVU420_SEMIPLANAR == buffer_info->format)
    {
        if (buffer->frame.nMVEFlags & (MVE_BUFFER_FRAME_FLAG_TOP_PRESENT | MVE_BUFFER_FRAME_FLAG_BOT_PRESENT))
        {
            ...
            uint32_t c_width = (buffer->frame.decoded_width+1)>>1;
            uint32_t c_height = (buffer->frame.decoded_height+1)>>1;
            buffer_client->filled_len = buffer->frame.decoded_width * buffer->frame.decoded_height +
                                                            c_width * c_height * 2;
        }
    ...
    else if (MVE_BASE_BUFFER_FORMAT_YUV420_I420_10 == buffer_info->format || MVE_BASE_BUFFER_FORMAT_YUV420_P010 == buffer_info->format)
    {
        if (buffer->frame.nMVEFlags & (MVE_BUFFER_FRAME_FLAG_TOP_PRESENT | MVE_BUFFER_FRAME_FLAG_BOT_PRESENT))
        {
    ...
    if (false != buffer_info->do_cache_maintenance)
    {
         ...
         * call handles cache management */
        mve_blk1_buffer_set_owner(buffer_client->buffer, MVE_BUFFER_OWNER_CPU, buffer_client->port_index);
        if (NULL != buffer_client->crc)
        {
            mve_blk1_buffer_set_owner(buffer_client->crc, MVE_BUFFER_OWNER_CPU, buffer_client->port_index);
        }
    }
...
mve_base_error mve_blk1_session_buffer_notify_ref_frame_release_internal(struct mve_session *session,
                                                                    mve_base_buffer_handle_t buffer_id)
{
    struct mve_buffer_client *buffer_client;
    ...
    return mve_blk1_com_add_message(session, MVE_REQUEST_CODE_RELEASE_REF_FRAME, sizeof(data), (uint32_t *)&data);
}

void mve_blk1_session_buffer_convert_to_userspace(struct mve_base_buffer_userspace *dst,
