...
struct mve_buffer_external *mve_blk1_buffer_ashmem_create_buffer_external(struct mve_buffer_info *info)
{
...
void mve_blk1_buffer_ashmem_destroy_buffer_external(struct mve_buffer_external *buffer)
{
    MVE_RSRC_MEM_CACHE_FREE(buffer, sizeof(struct mve_buffer_external));
}

bool mve_blk1_buffer_ashmem_map_physical_pages(struct mve_buffer_external *buffer)
{
    uint32_t size;
    void *ptr;
    uint32_t num_pages;
...
bool mve_blk1_buffer_ashmem_unmap_physical_pages(struct mve_buffer_external *buffer)
{
    if (NULL == buffer)
    {
        return false;
...
void mve_blk1_buffer_ashmem_set_owner(struct mve_buffer_external *buffer,
