...
{
    {
        "video_decoder.avc",   "mvx/h264dec.fwb", NULL
    },
    {
        "video_encoder.avc",   "mvx/h264enc.fwb", NULL
    },
    {
        "video_decoder.hevc",  "mvx/hevcdec.fwb", NULL
    },
    {
        "video_encoder.hevc",  "mvx/hevcenc.fwb", NULL
    },
    {
        "video_decoder.h264",  "mvx/h264dec.fwb", NULL
    },
    {
        "video_decoder.vp8",   "mvx/vp8dec.fwb", NULL
    },
    {
        "video_encoder.vp8",   "mvx/vp8enc.fwb", NULL
    },
    {
        "video_decoder.vp9",   "mvx/vp9dec.fwb", NULL
    },
    {
        "video_encoder.vp9",   "mvx/vp9enc.fwb", NULL
    },
    {
        "video_decoder.rv",    "mvx/rvdec.fwb", NULL
    },
    {
        "video_decoder.mpeg2", "mvx/mpeg2dec.fwb", NULL
    },
    {
        "video_decoder.mpeg4", "mvx/mpeg4dec.fwb", NULL
    },
    {
        "video_decoder.h263",  "mvx/mpeg4dec.fwb", NULL
    },
    {
        "video_decoder.vc1",   "mvx/vc1dec.fwb", NULL
    },
    {
        "video_encoder.jpeg",  "mvx/jpegenc.fwb", NULL
    },
    {
        "video_decoder.jpeg",  "mvx/jpegdec.fwb", NULL
    },
    {
        "video_decoder.avs",   "mvx/avsdec.fwb", NULL
    },
    {
        "video_decoder.avs2",   "mvx/avs2dec.fwb", NULL
    }
};
...
static struct mve_firmware_descriptor *load_firmware(const char *filename)
{
    ...
    ret = request_firmware(&fw, filename, mve_blk1_device);
    if (0 > ret)
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_ERROR, "Unable to load firmware file. file=%s.", filename);
        goto error;
    }
    ...
    while (0 < bytes_left)
    {
        ...
        void *dst = mve_blk1_rsrc_mem_cpu_map_page(desc->data[i]);
        ...
        mve_blk1_rsrc_mem_cpu_unmap_page(desc->data[i]);
        mve_blk1_rsrc_mem_clean_cache_range(desc->data[i], PAGE_SIZE);
        i++;
        bytes_left -= size;
    }
...
static bool construct_fw_page_table(struct mve_firmware_descriptor *desc)
{
    ...
    header = (struct fw_header *)mve_blk1_rsrc_mem_cpu_map_page(desc->data[0]);
    if (NULL == header)
    {
        return false;
    }
    ...
    for (j = 0; j < header->bss_bitmap_size; j++)
    {
        ...
        else if ((word_idx < 16) &&(header->bss_bitmap[word_idx] & (1 << bit_idx)) != 0)
        {
    ...
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_INFO, "Detected firmware version v2. major=%u, minor=%u.", desc->fw_version.major, desc->fw_version.minor);
    }
    else if (MVE_BASE_FW_MAJOR_PROT_V3 == desc->fw_version.major)
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_INFO, "Detected firmware version v3. major=%u, minor=%u.", desc->fw_version.major, desc->fw_version.minor);
    }
    else
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_ERROR, "Unknown firmware version. major=%u, minor=%u.", desc->fw_version.major, desc->fw_version.minor);
        desc->fw_version.major = MVE_BASE_FW_MAJOR_PROT_UNKNOWN;
    }
    ret = true;

out:
    mve_blk1_rsrc_mem_cpu_unmap_page(desc->data[0]);
...
static struct mve_firmware_descriptor *get_fw_descriptor(const char *role)
{
    ...
    for (i = 0; i < NELEMS(firmware_cache); ++i)
    {
        if (0 == strcmp(role, firmware_cache[i].role))
        {
                         ...
                         struct mve_fw_instance *inst)
{
    ...
    if (true == first_core || FW_PAGETYPE_BSS == page_type)
    {
        ...
        for (j = first_bss_idx; j <= last_bss_idx; j++)
        {
            ...
            mve_blk1_rsrc_mem_write32(dst_page + MVE_MMU_PAGE_TABLE_ENTRY_SIZE * j, entry);
        }
    }
                                                    ...
                                                    int ncores)
{
    ...
    for (i = 0; i < (unsigned int)ncores; ++i)
    {
        ...
        mve_blk1_mem_virt_region_get(fw_regions[i], &region);
        ...
        if (true == first_core)
        {
            ...
            for (j = 0; j < (MVE_MMU_PAGE_SIZE / sizeof(mve_mmu_entry_t)); j++)
            {
                ...
                if (FW_GET_TYPE(fw_mmu_page_line) == FW_PAGETYPE_TEXT)
                {
                    ...
                    mve_blk1_rsrc_mem_write32(l2page + 4 * j, entry);
                }
            }
        }
        ...
        ret = mve_blk1_mmu_map_pages_merge(ctx,
                                      l2page,
                                      region.start,
                                      MVE_MMU_PAGE_SIZE / sizeof(uint32_t));
        if (false == ret)
...
error:
    ...
    if (NULL != desc)
    {
        ...
        for (i = 0; i < ncores_setup; ++i)
        {
            ...
            mve_blk1_mem_virt_region_get(fw_regions[i], &region);
            mve_blk1_mmu_unmap_pages(ctx, region.start);
        }
                                                  ...
                                                  int ncores)
{
    ...
    for (i = 0; i < (unsigned int)ncores; ++i)
    {
        ...
        ret = mve_blk1_mmu_map_page_replace(ctx,
                                       l2page,
                                       region.start);
        if (false == ret)
        {
 ...
 * @return A firmware instance on success, NULL on failure.
 */
struct mve_fw_instance *mve_blk1_fw_load(struct mve_mmu_ctx *ctx,
                                    struct mve_base_fw_secure_descriptor *fw_secure_desc,
                                    const char *role,
                                    int ncores)
{
...
bool mve_blk1_fw_unload(struct mve_mmu_ctx *ctx, struct mve_fw_instance *inst)
{
    int i;
    int sem_taken = -1;
...
struct mve_base_fw_version *mve_blk1_fw_get_version(struct mve_fw_instance *inst)
{
    if (NULL == inst)
    {
        return NULL;
...
bool mve_blk1_fw_secure(struct mve_fw_instance *inst)
{
    if (NULL == inst)
    {
        return false;
...
void mve_blk1_fw_log_fw_binary(struct mve_fw_instance *inst, struct mve_session *session)
{
    struct fw_header *header;
    struct
    {
    ...
    header = (struct fw_header *)mve_blk1_rsrc_mem_cpu_map_page(inst->desc->data[0]);
    if (NULL == header)
    {
        return;
    }
    ...
    MVE_LOG_DATA(&mve_blk1_rsrc_log_fwif, MVE_LOG_WARNING, vec, 2);

    /* Unmap first page. */
    mve_blk1_rsrc_mem_cpu_unmap_page(inst->desc->data[0]);
}
