...
static phys_addr_t get_l2_line_addr(struct mve_mmu_ctx *ctx, mve_addr_t mve_addr)
{
    ...
    mve_mmu_entry_t l1_entry = (mve_mmu_entry_t)mve_blk1_rsrc_mem_read32(l1_entry_addr);
    ...
    if (0 == l1_entry)
    {
        ...
        mve_blk1_rsrc_mem_write32(l1_entry_addr, l1_entry);
        mve_blk1_rsrc_mem_clean_cache_range(l1_entry_addr, sizeof(mve_mmu_entry_t));
    }
...
struct mve_mmu_ctx *mve_blk1_mmu_create_ctx(void)
{
                            ...
                            uint32_t num_pages)
{
    ...
    while (curr < end)
    {
        ...
        do
        {
            ...
            entry = mve_blk1_rsrc_mem_read32(l2_line);
        ...
        if (0 < curr - start)
        {
            while (start < curr)
            {
                ...
                addr = mve_mmu_entry_get_paddr(mve_blk1_rsrc_mem_read32(l2_line));
                ...
                do
                {
                    ...
                    mve_mmu_entry_t entry = mve_blk1_rsrc_mem_read32(l2_line);
                    pptr = mve_blk1_rsrc_mem_cpu_map_page(mve_mmu_entry_get_paddr(entry));
                    /* Unmap entry immediately. We only want to know if the pages are linear
                     * in the virtual address space. */
                    mve_blk1_rsrc_mem_cpu_unmap_page(mve_mmu_entry_get_paddr(entry));
                ...
                /* Unmap pstart */
                mve_blk1_rsrc_mem_cpu_unmap_page(addr);
                ...
                int err = mveemul_add_memory(ctx->mmu_id,
                                             mve_blk1_rsrc_mem_cpu_map_page(addr),
                ...
                if (err != MVEEMUL_RET_OK)
                {
                    ...
                    MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_ERROR, "mveemul_add_memory failed. error=%u.", err);
                }
                mve_blk1_rsrc_mem_cpu_unmap_page(addr);
                start = ptr;
            }
                              ...
                              struct mve_mmu_mapping *entry)
{
    ...
    while (curr < end)
    {
        ...
        do
        {
            ...
            l2_entry = mve_blk1_rsrc_mem_read32(l2_line);
            curr += MVE_MMU_PAGE_SIZE;
        }
        ...
        if (0 < curr - start)
        {
            ...
            if (err != MVEEMUL_RET_OK)
            {
                /* Exit with a fatal error if mveemul_add_memory fails, as this
                 * indicates that the emulator is in a bad state. */
                MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_ERROR, "mveemul_remove_memory failed. error=%u.", err);
            }
        }
    }
}
...
void mve_blk1_mmu_destroy_ctx(struct mve_mmu_ctx *ctx)
{
    int i;
    ...
    list_for_each_safe(pos, next, &(ctx->mappings->list))
    {
        ...
        if (NULL == map->pages && 0 < map->num_pages)
        {
             ...
             * on unmap. */
            MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_ERROR, "MMU leaked region. addr=0x%X-0x%X.",
                          map->mve_addr,
    ...
    for (i = 0; i < NUM_L1_ENTRIES; ++i)
    {
        ...
        l1_entry = mve_blk1_rsrc_mem_read32(ctx->l1_page + i * sizeof(mve_mmu_entry_t));
        if (0 != l1_entry)
        {
...
#ifdef EMULATOR
    {
        ...
        if (MVEEMUL_RET_OK != ret)
        {
            MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_ERROR, "Unable to remove memory map.");
        }
    }
#endif
...
bool mve_blk1_mmu_map_page_replace(struct mve_mmu_ctx *ctx,
                              phys_addr_t l2_page,
                              mve_addr_t mve_addr)
{
...
bool mve_blk1_mmu_map_pages_merge(struct mve_mmu_ctx *ctx,
                             phys_addr_t l2_page,
                             mve_addr_t mve_addr,
                             uint32_t num_pages)
{
    ...
    for (curr_addr = start_addr; curr_addr < end_addr; curr_addr += MVE_MMU_PAGE_SIZE)
    {
        ...
        l2_entry = mve_blk1_rsrc_mem_read32(addr);
        if (0 != l2_entry)
        {
    ...
    for (i = 0; i < num_pages; ++i)
    {
        ...
        entry = mve_blk1_rsrc_mem_read32(l2_page + i * sizeof(mve_mmu_entry_t));
        mve_blk1_rsrc_mem_write32(addr, entry);
        mve_blk1_rsrc_mem_clean_cache_range(addr, sizeof(mve_mmu_entry_t));
    }
...
bool mve_blk1_mmu_map_pages(struct mve_mmu_ctx *ctx,
                       phys_addr_t *pages,
                       mve_addr_t mve_addr,
                       uint32_t num_pages,
...
void mve_blk1_mmu_unmap_pages(struct mve_mmu_ctx *ctx,
                         mve_addr_t mve_addr)
{
    uint32_t i;
    ...
    do
    {
        ...
        {
            mve_blk1_rsrc_mem_write32(addr, 0);
            addr += sizeof(mve_mmu_entry_t);
        }
        /* Flush the cache */
        mve_blk1_rsrc_mem_clean_cache_range(start, to_clear_current_iteration * sizeof(mve_mmu_entry_t));
...
bool mve_blk1_mmu_map_pages_and_reserve(struct mve_mmu_ctx *ctx,
                                   phys_addr_t *pages,
                                   ...
                                   bool free_pages_on_unmap)
{
    ...
    do
    {
        ...
        for (i = 0; i < max_pages && true == space_found && curr_addr < end_addr; ++i)
        {
            ...
            {
                space_found = mve_blk1_rsrc_mem_read32(addr) == 0;
            }
        ...
        if (false == space_found)
        {
            ...
            while (curr_addr < end_addr)
            {
                ...
                if (0 != addr)
                {
                    mve_mmu_entry_t entry = mve_blk1_rsrc_mem_read32(addr);
                    if (0 == entry)
                    {
                        break;
                    }
...
bool mve_blk1_mmu_map_info(struct mve_mmu_ctx *ctx,
                      mve_addr_t mve_addr,
                      uint32_t *num_pages,
                      uint32_t *max_pages)
{
...
BullseyeCoverageOn
#else
bool mve_blk1_mmu_map_resize(struct mve_mmu_ctx *ctx,
                        phys_addr_t *new_pages,
                        mve_addr_t mve_addr,
                        uint32_t num_pages,
                        enum mve_mmu_attrib attrib,
    ...
    if (map->num_pages != num_pages)
    {
        ...
        for (i = new_pages_start; i < map->max_pages; ++i)
        {
            ...
            else
            {
                ...
                mve_mmu_entry_t entry = mve_blk1_rsrc_mem_read32(l2_line_addr);

                if (NULL != new_pages && i < num_pages)
                {
                    /* Map this entry to a real page */
                ...
                mve_blk1_rsrc_mem_write32(l2_line_addr, entry);
                mve_blk1_rsrc_mem_clean_cache_range(l2_line_addr, sizeof(mve_mmu_entry_t));
            }
        }
...
phys_addr_t mve_blk1_mmu_get_id(struct mve_mmu_ctx *ctx)
{
    if (NULL == ctx)
    {
        return 0;
...
phys_addr_t mve_blk1_mmu_get_l1_page(struct mve_mmu_ctx *ctx)
{
    if (NULL == ctx)
    {
        return 0;
...
bool mve_blk1_mmu_read_buffer(struct mve_mmu_ctx *ctx,
                         mve_addr_t src,
                         uint8_t *dst,
                         uint32_t size)
{
    while (size > 0)
    {
        ...
        l2_entry = (mve_mmu_entry_t)mve_blk1_rsrc_mem_read32(l2addr);
        ...
        mve_blk1_rsrc_mem_invalidate_cache_range(paddr, copy_size);
        ptr = mve_blk1_rsrc_mem_cpu_map_page(paddr);
        if (NULL != ptr)
        {
            memcpy(dst, ptr, copy_size);
            mve_blk1_rsrc_mem_cpu_unmap_page(paddr);
        }
