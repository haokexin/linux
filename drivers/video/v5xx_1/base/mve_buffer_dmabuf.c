...
struct mve_buffer_external *mve_blk1_buffer_dmabuf_create_buffer_external(struct mve_buffer_info *info)
{
    ...
    buffer_external->attachment = dma_buf_attach(buffer_external->handle, &mve_blk1_rsrc_data.pdev->dev);
...
void mve_blk1_buffer_dmabuf_destroy_buffer_external(struct mve_buffer_external *buffer)
{
...
bool mve_blk1_buffer_dmabuf_map_physical_pages(struct mve_buffer_external *buffer)
{
    struct dmabuf_buffer_external *buffer_external;
    ...
    for_each_sg(buffer_external->sg->sgl, sgl, buffer_external->sg->nents, i)
    {
        ...
        if (j < npages)
        {
            MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_INFO, "[MVE] scatterlist size (%u pages) is bigger than the expected size (%u pages)", npages, num_pages);
        }
    }
    ...
    if (count < num_pages)
    {
        MVE_LOG_PRINT(&mve_blk1_rsrc_log, MVE_LOG_WARNING,"[MVE] scatterlist doesn't contain enough pages (found: %d expected: %d pages)\n", count, num_pages);
        goto error;
    }
...
bool mve_blk1_buffer_dmabuf_unmap_physical_pages(struct mve_buffer_external *buffer)
{
    struct dmabuf_buffer_external *buffer_external;
...
void mve_blk1_buffer_dmabuf_set_owner(struct mve_buffer_external *buffer,
                                 ...
                                 enum mve_port_index port)
{
    ...
    sync_func(&mve_blk1_rsrc_data.pdev->dev, buffer_external->sg->sgl, buffer_external->sg->nents, direction);
}
