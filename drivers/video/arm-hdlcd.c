...
static int hdlcd_set_par(struct fb_info *info)
{
...
#endif
		HDLCD_POLARITY_DATA;
	polarities |= (hdlcd->fb.var.sync & FB_SYNC_HOR_HIGH_ACT) ?
						HDLCD_POLARITY_HSYNC : 0;
	polarities |= (hdlcd->fb.var.sync & FB_SYNC_VERT_HIGH_ACT) ?
						HDLCD_POLARITY_VSYNC : 0;

	hdlcd_disable(hdlcd);

	WRITE_HDLCD_REG(HDLCD_REG_FB_LINE_LENGTH,
				hdlcd->fb.var.xres * bytes_per_pixel);
	WRITE_HDLCD_REG(HDLCD_REG_FB_LINE_PITCH,
				hdlcd->fb.var.xres * bytes_per_pixel);
	...
	WRITE_HDLCD_REG(HDLCD_REG_V_FRONT_PORCH,
				hdlcd->fb.var.lower_margin - 1);
	...
	WRITE_HDLCD_REG(HDLCD_REG_H_FRONT_PORCH,
				hdlcd->fb.var.right_margin - 1);
	WRITE_HDLCD_REG(HDLCD_REG_POLARITIES, polarities);
	WRITE_HDLCD_REG(HDLCD_REG_PIXEL_FORMAT, (bytes_per_pixel - 1) << 3);
	WRITE_HDLCD_REG(HDLCD_REG_RED_SELECT,
	  ((hdlcd->fb.var.red.length & 0xf) << 8) | hdlcd->fb.var.red.offset);
	WRITE_HDLCD_REG(HDLCD_REG_GREEN_SELECT,
				((hdlcd->fb.var.green.length & 0xf) << 8) |
				 hdlcd->fb.var.green.offset);
	WRITE_HDLCD_REG(HDLCD_REG_BLUE_SELECT,
				((hdlcd->fb.var.blue.length & 0xf) << 8) |
				 hdlcd->fb.var.blue.offset);
...
static irqreturn_t hdlcd_irq(int irq, void *data)
{
	...
	if (irq_status & HDLCD_INTERRUPT_VSYNC) {
		...
		WRITE_HDLCD_REG(HDLCD_REG_INT_MASK,
...
static int hdlcd_wait_for_vsync(struct fb_info *info)
{
	...
	err = wait_for_completion_interruptible_timeout(
			&hdlcd->vsync_completion, msecs_to_jiffies(100));
...
static int hdlcd_setup(struct hdlcd_device *hdlcd)
{
	...
	hdlcd->clk = clk_get(NULL, "2b000000.hdlcd");
	if (IS_ERR(hdlcd->clk)) {
		dev_err(hdlcd->dev, "HDLCD: unable to find clock data\n");
		return PTR_ERR(hdlcd->clk);
	}

	hdlcd->base = ioremap_nocache(hdlcd->fb.fix.mmio_start,
				      hdlcd->fb.fix.mmio_len);
	if (!hdlcd->base) {
	...
	/* Allow max number of outstanding requests with the
	 * largest beat burst */
	WRITE_HDLCD_REG(HDLCD_REG_BUS_OPTIONS,
...
/* EDID data is passed from devicetree in a literal string that can
 * contain spaces and the hexadecimal dump of the data */
...
static int __devinit hdlcd_probe(struct platform_device *pdev)
{
	...
	if (of_node) {
		...
		if (prop) {
			hdlcd->fb.fix.smem_start =
				of_read_ulong(prop, of_n_addr_cells(of_node));
			prop += of_n_addr_cells(of_node);
			framebuffer_size =
				of_read_ulong(prop, of_n_size_cells(of_node));
	...
	hdlcd->fb.screen_base =
			ioremap_wc(hdlcd->fb.fix.smem_start, framebuffer_size);
