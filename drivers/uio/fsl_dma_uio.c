...
static int __devinit fsl_dma_uio_probe(struct of_device *dev,
	const struct of_device_id *match)
{
	...
	cell = (u32 *)of_get_property(dev->node, "cell-index", NULL);
	if (!cell) {
		dev_err(&dev->dev, "Can't get property 'cell-index'\n");
		return -ENODEV;
	}
...
static int fsl_dma_uio_remove(struct of_device *dev)
{
	struct fsldma_device *fdev;
...
static struct of_platform_driver fsl_dma_uio_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = "fsl-of-dma",
	},
	.match_table = fsl_of_dma_match,
...
static __init int fsl_dma_uio_init(void)
{
	...
	err = of_register_platform_driver(&fsl_dma_uio_driver);
	if (err < 0)
		pr_warning(
...
{
	of_unregister_platform_driver(&fsl_dma_uio_driver);

	pr_warning("fsl dma uio driver removed\n");
}
