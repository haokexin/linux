...
static int __devinit fsl_srio_uio_probe(struct of_device *dev,
...
static int fsl_srio_uio_remove(struct of_device *dev)
{
...
static struct of_platform_driver fsl_of_srio_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = "fsl-of-srio",
	},
	.match_table = fsl_srio_uio_match,
...
static __init int fsl_srio_uio_init(void)
{
	...
	err = of_register_platform_driver(&fsl_of_srio_driver);
	if (err < 0)
		pr_warning(
...
{
	of_unregister_platform_driver(&fsl_of_srio_driver);
