...
void disp_req_callback(const uint32_t rep,
		       ...
		       void *ext, const ext_info_t * info)
{
	...
	BSTN_TRACE_PRINTK("exchange node nid:%d received", exchange->nid);
...
static struct bstn_rsp_msg *_rsp_recv(struct bstn_device *pbstn)
{
	...
	do {
		...
		//suppress simple debugging messages
			 ...
			 (pmsg >=
			  (phys_to_bus(pbstn->mem_manager.rmem_base) +
...
static int _req_send(struct bstn_device *pbstn, struct bstn_req_msg *req)
{
	...
	if (bstn_msg_interface == BSTN_MSG_INTERFACE_IPC) {
		...
		ret =
		    ...
		    net_client.disp_req_async(pmsg, disp_req_callback, pbstn,
					      NULL);
	}
...
bool bstn_msg_is_bootdone(struct bstn_device *pbstn)
{
	...
	int time;
	time =
	    wait_for_completion_timeout(&pbstn->msg_manager.ipc_boot_complete,
...
int bstn_msg_exchange(struct bstn_device *pbstn, struct bsnn_msg_exchange *msg)
{
	...
	uint32_t net_debug_dctl_fch_0 = 0;
	...
	ret =
	    wait_for_completion_timeout(&exchange->complete,
					msecs_to_jiffies(BSTN_RSP_TIMEOUT_MS));
	if (ret > 0) {
		//copy the response message
		...
		BSTN_DEV_ERR(&pbstn->pdev->dev, "exchange node nid:%d failed",
			     exchange->nid);
		/*
		 * "net_debug_dctl_fch_0[31:24], current section ID in DAG control logic,
		 * this ID increments when all DMA_IN and OUT of current section is done"
		 */
		reg_base = ioremap(0x5000A000, 0x4);	// NET TOP
		net_debug_dctl_fch_0 = readl_relaxed(reg_base + 0x4);
		BSTN_DEV_ERR(&pbstn->pdev->dev, "net top current model section id: %d", net_debug_dctl_fch_0 >> 24);	// bit[31:24]
		iounmap(reg_base);
	}
...
int bstn_msg_manager_init(struct bstn_device *pbstn)
{
	...
	if (bstn_msg_interface == BSTN_MSG_INTERFACE_IPC) {
		pbstn->msg_manager.ipc_session_id =
		    ipc_init(IPC_CORE_NET, IPC_CORE_ARM2, &pbstn->pdev->dev);
		...
		ret = pbstn->msg_manager.msgbx_client->
		    net_client.register_avail_changed(on_dst_changed, pbstn);
		BSTN_STAGE_PRINTK
	...
	pbstn->msg_manager.req_bufs = pbstn->mem_manager.ops->alloc(
		pbstn,
		BSTN_EXCHANGE_NODE_NUM * sizeof(struct bstn_req_msg),
		0,
		0
	);
	...
	for (i = 0; i < BSTN_EXCHANGE_NODE_NUM; i++) {
		...
		pbstn->msg_manager.exchange_nodes[i].req_buf =
		    (struct bstn_req_msg *)pbstn->msg_manager.
		    req_bufs->kern_addr + i;
		pbstn->msg_manager.exchange_nodes[i].req_buf->nid = i;
	}

	mutex_init(&pbstn->msg_manager.flist_lock);

	pbstn->msg_manager.msg_info = pbstn->mem_manager.ops->alloc(
		pbstn,
		sizeof(struct bstn_fw_msginfo),
		0,
		0
	);
	if (pbstn->msg_manager.msg_info == NULL) {
		BSTN_DEV_ERR(&pbstn->pdev->dev,
			     "bstn_msg_manager_init: cannot allocate bstn_fw_msginfo buffers");
	}
	// start the msgbx receiver thread
	if (bstn_msg_interface == BSTN_MSG_INTERFACE_IPC) {
		pbstn->msg_manager.msg_receiver_task =
		    kthread_run(bstn_msg_receiver, pbstn, "msg_receiver_task");
...
int bstn_msg_start_sw_bist_thread(struct bstn_device *pbstn)
{
	...
	if (pbstn->msg_manager.msg_sw_bister_task == NULL) {
		pbstn->msg_manager.msg_sw_bister_task =
		    kthread_run(bstn_sw_bister, pbstn, "msg_sw_bister_task");
...
void bstn_msg_manager_exit(struct bstn_device *pbstn)
{
	...
	if (pbstn->msg_manager.msg_info) {
		pbstn->mem_manager.ops->free(pbstn->msg_manager.msg_info);
