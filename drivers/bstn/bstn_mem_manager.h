...
#define bus_to_dma(baddr) ((bstn_mem_usingsmmu == 0)                           \
   ? ((dma_addr_t)bus_to_phys(baddr))                                          \
   : ((dma_addr_t)baddr))

#define dma_to_bus(daddr) ((bstn_mem_usingsmmu == 0)                           \
...
struct bstn_mem_ops {
	bstn_memblock_t *(*alloc)       (struct bstn_device *pbstn,
											uint32_t     size,
											uint32_t     align,
									 unsigned long       attr);

	void             (*iommu_bypass)(struct bstn_device *pbstn,
											uint32_t     size,
											uint32_t     align,
											phys_addr_t  addr,
									 unsigned long       attr);

	void             (*iommu_map)   (struct bstn_device *pbstn,
											uint32_t     size,
											uint32_t     align,
											phys_addr_t  addr,
											dsp_ptr     *iova,
	...
	void             (*iommu_free)  (struct bstn_device *pbstn,
											uint32_t     size,
											uint32_t     align,
											dsp_ptr      iova);
