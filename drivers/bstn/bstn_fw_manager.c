...
static void _firmware_dump(char *fw_buf, int size)
{
	...
	for (i = 0; i < size / BSTN_FIRMWARE_DUMP_LINE_SIZE; i++) {
		BSTN_TRACE_PRINTK("%02x %02x %02x %02x %02x %02x %02x %02x",
...
int bstn_firmware_load(struct bstn_device *pbstn)
{
	...
	if (bstn_msg_interface == BSTN_MSG_INTERFACE_IPC) {
		ret =
		    request_firmware(&fw, pbstn->fw_manager.name,
				     &pbstn->pdev->dev);
	} else {
		ret =
		    request_firmware(&fw, pbstn->fw_manager.name_msgbox,
				     &pbstn->pdev->dev);
	}
	if (ret < 0) {
...
void bstn_firmware_boot(struct bstn_device *pbstn)
{
	...
	writel_relaxed(
		...
		pbstn->fw_manager.net_sreg_base + BSTN_DSP_ALT_RESET_VEC_OFFSET
	);
	BSTN_STAGE_PRINTK(
	...
	// start dsp
	sys_ctrl.b.safety_apb_parity_chk_dec_en = 0;	// bit12
...
int bstn_fw_manager_init(struct bstn_device *pbstn)
{
	...
	// map registers and firmware memory
	...
	BSTN_TRACE_PRINTK("pbstn net regs start: 0x%llx, end: 0x%llx",
			  res->start, res->end);
	...
	// partition_vaddr = memremap(0x821000000, 0x1000000, MEMREMAP_WC);
	// if (pbstn->mem_manager.enable_smmu) {
	// 	pbstn->mem_manager.ops->iommu_bypass(
	// 		pbstn,
	// 		(uint32_t)(res->end - res->start + 1),
	// 		PAGE_SIZE,
	// 		(phys_addr_t)res->start,
	// 		IOMMU_READ | IOMMU_WRITE
	// 	);
	// 	BSTN_TRACE_PRINTK(
	// 		"pbstn net regs iommu_map: 0x%llx, end: 0x%llx",
	// 		res->start, res->end
	// 	);
	// }
	...
	BSTN_TRACE_PRINTK("bstn firmware mem start: 0x%llx, end: 0x%llx",
			  res->start, res->end);
	...
	pbstn->fw_manager.fwmem_size = res->end - res->start + 1;
	pbstn->fw_manager.fwmem_phys_addr = res->start;
	if (pbstn->mem_manager.enable_smmu) {
		pbstn->mem_manager.ops->iommu_bypass(
			pbstn,
			(uint32_t)(res->end - res->start + 1),
		...
		BSTN_TRACE_PRINTK(
			"pbstn net fw iommu_map: 0x%llx, end: 0x%llx",
			res->start, res->end
		);
	}

	// get firmware name(ipc)init
	ret = device_property_read_string(&pbstn->pdev->dev, "firmware",
					  (const char
					   **)(&(pbstn->fw_manager.name)));
	...
	ret = device_property_read_string(&pbstn->pdev->dev, "firmware1",
					  (const char**)(&(pbstn->fw_manager.name_msgbox)));
	if (ret == -EINVAL || ret == -ENODATA) {
		dev_dbg(&pbstn->pdev->dev, "no bstn-firmware1 property found!");
		return ret;
	} else if (ret < 0) {
		BSTN_DEV_ERR(&pbstn->pdev->dev, "invalid firmware1 name, ret %d",
			     ret);
		return ret;
	}
	BSTN_STAGE_PRINTK("firmware1(msgbox) name: %s", pbstn->fw_manager.name_msgbox);

	// assign memory for firmware
	ret =
	    device_property_read_u32_array(&pbstn->pdev->dev,
					   "assigned-mem-size",
					   &assigned_memsize, 1);
	if (ret == -EINVAL || ret == -ENODATA) {
		BSTN_DEV_ERR(&pbstn->pdev->dev,
...
void bstn_fw_rt_exit(struct bstn_device *pbstn)
{
	...
	reg &=
	    ~((1 << BSTN_DSP_SOFT_RESET_BIT) | (1 << BSTN_NET_SOFT_RESET_BIT));
	writel_relaxed(reg, pbstn->fw_manager.net_sreg_base + BSTN_SYS_CTRL);
