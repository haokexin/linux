...
/******************************************************************************
* bstn mem ops: alloc, iommu_bypass, free...
*******************************************************************************/
static bstn_memblock_t *bstn_alloc       (struct bstn_device *pbstn,
												 uint32_t     size,
												 uint32_t     align,
										  unsigned long       attr);

static void             bstn_iommu_map   (struct bstn_device *pbstn,
												 uint32_t     size,
												 uint32_t     align,
												 phys_addr_t  addr,
												 dsp_ptr     *iova,
...
static void             bstn_iommu_bypass(struct bstn_device *pbstn,
												 uint32_t     size,
												 uint32_t     align,
												 phys_addr_t  addr,
										  unsigned long       attr);

static void             bstn_iommu_free  (struct bstn_device *pbstn,
												 uint32_t     size,
												 uint32_t     align,
												 dsp_ptr      iova);
...
static struct bstn_mem_ops bstn_cma_memops = {
	.alloc 			= bstn_alloc,
	.iommu_bypass	= bstn_iommu_bypass,
								   ...
								   unsigned long       attr)
{
	...
	block = pmman->dma_ops->dma_alloc(pmman->pdev,
								  attr,
								  size,
								  DMA_BIDIRECTIONAL,
								  GFP_KERNEL);
							  ...
							  unsigned long       attr)
{
	...
	iovad->start_pfn = 0x80000000 >> shift; // iova must be bigger than 0x80000000
	BSTN_STAGE_PRINTK(
		"bstn_iommu_bypass ok. iova: 0x%08x size: 0x%08x",
		(dsp_ptr)iova,
		size
	);
}
								   ...
								   dsp_ptr      iova)
{
	...
	BSTN_STAGE_PRINTK(
		"bstn_iommu_free ok. iova: 0x%08x size: 0x%08x",
		(dsp_ptr)iova,
		size
	);
}
 ...
 *          error code - failure
 */
int bsnn_buffer_alloc(struct file        *filp,
					  struct bstn_device *pbstn,
					  struct bsnn_buffer *ubuffer)
{
	int    ret;
	struct bstn_buffer      *buffer;
	struct bstn_mem_ctx     *ctx;
	...
	//allocate the buffer
	buffer->block = pmman->ops->alloc(pbstn, ubuffer->size, ubuffer->align, 0);
	...
	BSTN_TRACE_PRINTK("user_addr: 0x%llx", (unsigned long long)buffer->user_addr);

	ubuffer->baddr = buffer->bus_addr;
	ubuffer->uaddr = (void *)buffer->user_addr;
	ubuffer->align = ubuffer->align > PAGE_SIZE ? ubuffer->align : PAGE_SIZE;
	BSTN_TRACE_PRINTK("addr: %x, size: %x", ubuffer->baddr, ubuffer->size);
	BSTN_TRACE_PRINTK("buffer: %px, dbuf %px, dbuf_size: %lx",
					   buffer, buffer->dbuf, buffer->dbuf->size);
...
int bsnn_buffer_free(struct file *filp,
					 struct bstn_device *pbstn,
					 struct bsnn_buffer *ubuffer)
{
	struct bstn_buffer      *buffer;
	...
	if (ctx == NULL) {
		mutex_unlock(&pmman->mm_mutex);
		BSTN_DEV_ERR(pdev, "memory context with filp=%px not found", filp);
		return -ENOENT;
	}

	hash_for_each_possible_safe(ctx->ht, buffer, tmp, node, ubuffer->baddr) {
		if (ubuffer->baddr == buffer->bus_addr) {
			hash_del(&buffer->node);
			mutex_unlock(&pmman->mm_mutex);
			BSTN_TRACE_PRINTK("buffer: %px", buffer);
	...
	mutex_unlock(&pmman->mm_mutex);
	BSTN_DEV_ERR(pdev, "no buffer @ baddr=0x%x", ubuffer->baddr);
	return -ENOENT;
}
...
int bstn_dma_buf_import(struct bstn_device *pbstn, struct bstn_dma_buf *buf)
{
	...
	buffer->block = block;
	if (IS_ERR(block)) {
		BSTN_DEV_ERR(pmman->pdev, "failed to attach dma_buf(fd=%d)", buf->fd);
		ret = PTR_ERR(block);
	...
	BSTN_TRACE_PRINTK("iommu_iova_to_phys, phys_addr 0x%llx", phys_addr);
	...
	BSTN_TRACE_PRINTK("bus_addr 0x%x, buf->size  0x%lx", buffer->bus_addr, buffer->block->size);
...
int bstn_dma_buf_return(struct bstn_device *pbstn, struct bstn_dma_buf *buf)
{
	...
	BSTN_TRACE_PRINTK("returned dmabuf bus addr %px", dmabuf->priv);
...
int bstn_cma_buf_import(struct bstn_device *pbstn, struct bstn_cma_buf *buf) {
	struct bstn_buffer      *buffer;
	struct bstn_mem_manager *pmman;
	struct device           *pdev;
	phys_addr_t pa;
...
int bstn_cma_buf_return(struct bstn_device *pbstn, struct bstn_cma_buf *buf) {
	struct bstn_buffer      *buffer;
	struct hlist_node       *tmp;
	struct bstn_mem_manager *pmman;
	struct device           *pdev;
 ...
 *              Error code - failure
 */
int bstn_dma_buf_export(struct file *filp,
						struct bstn_device *pbstn, struct bstn_dma_buf *buf) {
	struct bstn_buffer      *buffer;
	struct bstn_mem_ctx     *ctx;
	struct hlist_node       *tmp;
	struct bstn_mem_manager *pmman;
	...
	mutex_unlock(&pmman->mm_mutex);
	BSTN_DEV_ERR(pdev, "buffer fd for bus_addr 0x%08x not found", buf->bus_addr);
	return -ENOENT;
}
...
int bstn_mem_manager_init(struct bstn_device *pbstn)
{
	...
	if (pmman->enable_smmu) {
		...
		if (ret) {
			BSTN_DEV_ERR(pmman->pdev, "dma_set_coherent_mask fail, ret %d", ret);
			return -ENODEV;
		}
		BSTN_TRACE_PRINTK("dma_set_coherent_mask OK.");
	} else {
		ret = device_property_read_u64_array(pdev,
				"rmem-base", &pmman->rmem_base, 1);
		if (ret < 0) {
			BSTN_DEV_ERR(pdev, "no rmem-base property, ret %d", ret);
			return -ENOENT;
		}
		ret = device_property_read_u64_array(pdev,
				"rmem-size", &pmman->rmem_size, 1);
		if (ret < 0) {
			BSTN_DEV_ERR(pdev, "no rmem-size property, ret %d", ret);
			return -ENOENT;
		}
		...
		if (ret < 0) {
			BSTN_DEV_ERR(pdev, "of_reserved_mem_device_init fail, ret: %d", ret);
			return -ENODEV;
		}
