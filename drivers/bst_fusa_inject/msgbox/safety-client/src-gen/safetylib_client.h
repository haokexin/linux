...
#ifndef SAFETYLIB_CLIENT_H
#define SAFETYLIB_CLIENT_H
 ...
 */
typedef void (*safetylib_fusacoreip_method_callback_t)(
 ...
 * The data MAY CHANGED after leaving the callback function.
 */
typedef void (*safetylib_fusaenable_method_callback_t)(
...
/**
 * Callback function for broadcast fusalsm_method.
 *
 * @param block_id The output argument returned by broadcast fusalsm_method.
 * @param lsm_id The output argument returned by broadcast fusalsm_method.
 * @param ext The user-defined data passed to the method.
 * @param info The extended information, containing uuid and timestamp.
 * @note all the data are stored in ext_buf passed to async call.
 * If ext_buf is NULL, internal buffer will be used.
 ...
 * The data MAY CHANGED after leaving the callback function.
 */
typedef void (*safetylib_fusalsm_method_callback_t)(
...
struct _safetylib_client_t {
	...
	/**
	 * Subscribe to the fusalsm_method broadcast.
	 *
	...
	/**
	 * Unsubscribe from the fusalsm_method broadcast.
	 *
	 ...
	 */
	int32_t (*fusalsm_method_unsub)(broadcast_sub_unsub_callback_t cb, void *ext);
...
#define safetylib_client_t struct _safetylib_client_t

/**
 *  The extend data used by the client.
 */
struct _safetylib_client_ext_t {
	uint8_t cid;
	uint8_t ccid;
	uint8_t status;
	uint8_t res[5];
	...
	callback_registration_t fusalsm_method_registry;
};
#define safetylib_client_ext_t struct _safetylib_client_ext_t

/**
 * Initializes the client.
 *
 * @param data The data for com_client_data_t
 * @param client The data for safetylib_client_t
 * @param ext The data for safetylib_client_ext_t
 * @return 0 if success, negative if fail.
 */
int32_t safetylib_client_init(com_client_data_t *data,
			safetylib_client_t *client,
