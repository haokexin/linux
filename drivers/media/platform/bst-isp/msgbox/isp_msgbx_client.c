...
#include "isp_msgbx_client.h"

#define PID CPU_6
#define FID DEF
#define SID 1U

static isp_msgbx_client_data_t *s_ins;
...
static int32_t receive_message(void)
{
	...
	video_client_ext_t *video_ext = &s_ins->video_ext;
	...
	if (ret == QUERY_INFO_DST_STS_OFFLINE) {
		if (video_ext->avail_changed_cb)
			video_ext->avail_changed_cb(false, video_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_ONLINE) {
		if (video_ext->avail_changed_cb)
			video_ext->avail_changed_cb(true, video_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_CHANGED) {
		ret = ipc_trans_layer_get_endmap(data->pid, &map);
		if (ret == 0) {
			chip_map = video_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & video_ext->cid_mask) != 0 ? true : false;
			if (status != video_ext->status) {
				video_ext->status = status;
				if (video_ext->avail_changed_cb)
					video_ext->avail_changed_cb(status, video_ext->avail_ext);
			}
...
static int32_t dispatch_message(void)
{
	...
	while (packet_obj) {
		...
		// dispatch message
		ret = -1;
		if (des->header.pid == s_ins->video_ext.cid) {
			if (des->header.typ == MSGBX_MSG_TYPE_BROADCAST)
				ret = s_ins->client.video_client.dispatch_broadcast(des);
			else if (des->header.typ == MSGBX_MSG_TYPE_REPLY)
				ret = s_ins->client.video_client.dispatch_reply(des);
		}
		if (ret < 0)
...
static int32_t start(void)
{
	...
	if (ret != 0) {
#elif defined IPC_RTE_RTOS
	data->route_task = TaskCreate((task_func_t)router_func, "isp_msgbx_client_thread", 0, NULL,
		0, (uint8_t *)stack_buffer, &tcb_buffer, true);
	if (data->route_task == 0) {
#elif defined IPC_RTE_KERNEL
	data->route_task = kthread_run(router_func, NULL, "isp_msgbx_client_thread");
...
#endif
// isp_msgbx_client_init
isp_msgbx_client_t *isp_msgbx_client_init(isp_msgbx_client_data_t *ins)
{
	int32_t ret = 0;
	...
	// init clients
	ret = video_client_init(data, &ins->client.video_client, &ins->video_ext);
	if (ret < 0)
		return NULL;

	// create client handle.
	ret = ipc_trans_layer_proxy_create_handle(data->pid, data->fid, data->sid, ins->video_ext.cid, ins->video_ext.ccid, &data->handle);
	if (ret < 0)
	{
...
// isp_msgbx_client_destroy
int32_t isp_msgbx_client_destroy(void)
{
	int32_t ret = 0;
	com_client_data_t *data = (com_client_data_t *)s_ins;
