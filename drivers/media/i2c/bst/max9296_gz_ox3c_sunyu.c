...
#define MODULE_NAME "bst,max9296-gz"
#define max9296_N_LINKS 4

//sunyu ox3c ser
static unsigned short max9296_0x3c_senyun_regs[][2] = {
	{0x0001, 0x01}, //3Gbps
	{0x0330, 0x04}, //?
	{0x0051, 0x02}, //RX0 stream_id : 2
	{0x0052, 0x01}, //RX0 stream_id : 1
	{0x0333, 0x4e}, //default
	{0x0316, 0x6d}, //RAW14
	{0x0002, 0xf3}, //desr
	{0x044a, 0xd0}, //Set TX1 MIPI 4 data lane
	{0x048a, 0xd0}, //Set TX2 MIPI 4 data lane
	{0x0320, 0x2f}, // 0:4 0x0f CSI PHY1 out put frequency 1500Mhz
	{0x0330, 0x04}, //?
	{0x0316, 0x2d}, //RAW14
	{0x0325, 0x80}, //wait for a new frame before generating a mipi packet
};
...
static void config_max9296_regs(struct i2c_client *client)
{
	...
	for (i = 0; i < ARRAY_SIZE(max9296_0x3c_senyun_regs); i++) {
		if (write_reg(client, max9296_0x3c_senyun_regs[i][0], max9296_0x3c_senyun_regs[i][1])) {
		...
		pr_info("%s() line %d, write max9296 reg:%#x, val:%#x",
		       (char *)__func__, (int)__LINE__,
		       max9296_0x3c_senyun_regs[i][0],
		       max9296_0x3c_senyun_regs[i][1]);
	}
}

static int max_channel_map(struct max9296_priv *priv)
{
	...
	for (i = 0; i < priv->n_links; i++) {
		write_register(adap, 0x40, 0x0000, (priv->ser_addr[i] << 1));
		// modify sensor i2c address
		write_register(adap, priv->ser_addr[i], 0x0042,
			       (priv->sensor_addr[i] << 1));
		write_register(adap, priv->ser_addr[i], 0x0043, 0x6c);
	}
	mdelay(10);
	return 0;
}
...
{
	int err, i;
	u32 addrs[4], sensor_addrs[4], naddrs, sensor_naddrs;
	...
	sensor_naddrs =
		of_property_count_elems_of_size(np, "sensor-regs", sizeof(u32));
	err = of_property_read_u32_array(client->dev.of_node, "sensor-regs",
					 sensor_addrs, sensor_naddrs);

	memcpy(priv->sensor_addr, sensor_addrs, sensor_naddrs * sizeof(u32));
...
MODULE_DESCRIPTION("Max9296 deserializer driver for OX3C YUV camera");
