...
static void _dump_firmware(char *fw_buf, int size)
{
	...
	for (i = 0; i < size / BST_LWNN_FIRMWARE_DUMP_LINE_SIZE; i++) {
		BST_LWNN_TRACE_PRINTK("%02x %02x %02x %02x %02x %02x %02x %02x",
		    *(fw_buf +
			i * BST_LWNN_FIRMWARE_DUMP_LINE_SIZE),
		    *(fw_buf +
			i * BST_LWNN_FIRMWARE_DUMP_LINE_SIZE +
			1),
		    *(fw_buf +
...
static void _pre_load_firmware(struct bst_lwnn *pbst_lwnn, int dsp)
{
	...
	//disable cv parity
	reg = readl_relaxed(pbst_lwnn->fw_manager.lb_cv_reg_base + LB_CV_REG_R_CV_DSP_CV_PARITY_CTRL_REG0);
	if (cv_parity_en) {
		reg |= 0x3;	// [1:0] - cv_internal_pty_en, cv_sram_ecc_en
		writel_relaxed(reg,
		    pbst_lwnn->fw_manager.lb_cv_reg_base + LB_CV_REG_R_CV_DSP_CV_PARITY_CTRL_REG0);
	} else {
		reg &= ~0x3;
		writel_relaxed(reg,
			pbst_lwnn->fw_manager.lb_cv_reg_base + LB_CV_REG_R_CV_DSP_CV_PARITY_CTRL_REG0);
	...
	reg = readl_relaxed(pbst_lwnn->fw_manager.lb_cv_reg_base + LB_CV_REG_R_CV_DSP_CLUSTER_INTR_EN_REG);
	reg &= ~(0x7 << (16 + pbst_lwnn->dsp_indices[dsp]*3));
	writel_relaxed(reg,
				   pbst_lwnn->fw_manager.lb_cv_reg_base + LB_CV_REG_R_CV_DSP_CLUSTER_INTR_EN_REG);

	//setup reset vector
	writel_relaxed(phys_to_bus(pbst_lwnn->fw_manager.dsps[dsp].fwmem_phys_addr),
	    pbst_lwnn->fw_manager.lb_cv_reg_base +
	    LB_CV_REG_R_CV_DSP_ALT_RESET_VEC_OFFSET +
	    pbst_lwnn->dsp_indices[dsp] * BST_LWNN_REG_WIDTH);

	reg =
	    readl_relaxed(pbst_lwnn->fw_manager.lb_cv_reg_base +
	...
	reg |= (0x30); //cv shareRAM
	writel_relaxed(reg,
	    pbst_lwnn->fw_manager.lb_cv_reg_base +
	    LB_CV_REG_R_CV_SYS_CTRL_OFFSET);
	reg |= (1 << (BST_LWNN_RUNSTALL_BIT + pbst_lwnn->dsp_indices[dsp]));	//dsp stop
	reg |= (1 << (BST_LWNN_SOFT_RESET_BIT + pbst_lwnn->dsp_indices[dsp]));	//reset disable
	writel_relaxed(reg,
	    pbst_lwnn->fw_manager.lb_cv_reg_base +
	    LB_CV_REG_R_CV_SYS_CTRL_OFFSET);
}
...
static void _post_load_firmware_success(struct bst_lwnn *pbst_lwnn, int dsp)
{
	...
	reg =
	    ...
	    LB_CV_REG_R_CV_SYS_CTRL_OFFSET);
	reg |= (1 << (BST_LWNN_SOFT_RESET_BIT + pbst_lwnn->dsp_indices[dsp]));	//reset disable
	reg |= (1 << (BST_LWNN_DSP0_CLK_EN + pbst_lwnn->dsp_indices[dsp]));	//clk enable
	reg &= ~(1 << (BST_LWNN_RUNSTALL_BIT + pbst_lwnn->dsp_indices[dsp]));	//dsp run
	reg |= 0xf0; //cv shareRAM
	writel_relaxed(reg,
	    pbst_lwnn->fw_manager.lb_cv_reg_base +
	    LB_CV_REG_R_CV_SYS_CTRL_OFFSET);
...
static void _post_load_firmware_failed(struct bst_lwnn *pbst_lwnn, int dsp)
{
	...
	reg =
	    readl_relaxed(pbst_lwnn->fw_manager.lb_cv_reg_base +
	    LB_CV_REG_R_CV_SYS_CTRL_OFFSET);
	reg &= ~(1 << (BST_LWNN_SOFT_RESET_BIT + pbst_lwnn->dsp_indices[dsp]));	//reset enable
	reg &= ~(1 << (BST_LWNN_DSP0_CLK_EN + pbst_lwnn->dsp_indices[dsp]));	//clk disable
	reg |= (1 << (BST_LWNN_RUNSTALL_BIT + pbst_lwnn->dsp_indices[dsp]));	//dsp stop
	writel_relaxed(reg,
	    pbst_lwnn->fw_manager.lb_cv_reg_base +
	    LB_CV_REG_R_CV_SYS_CTRL_OFFSET);
...
int bst_lwnn_fw_manager_init(struct bst_lwnn *pbst_lwnn)
{
	...
	ret =
	    device_property_read_u32(&pbst_lwnn->pdev->dev, "ipc-register-addr",
	...
	if (pbst_lwnn->mem_manager.enable_smmu) {
		// todo: make next iommu map soft!
		pbst_lwnn->mem_manager.ops->iommu_bypass(
			pbst_lwnn,
			0x2000000,
			PAGE_SIZE,
			(phys_addr_t)0x81a000000,
	...
	for_each_child_of_node(bst_lwnn_node, dsp_node) {
		...
		BST_LWNN_TRACE_PRINTK("rt-init-addr of DSP %d: 0x%x", i,
		    pbst_lwnn->fw_manager.dsps[i].rt_init_addr);
		...
		ret = of_property_read_string(dsp_node, "firmware1",
		    (const char **)
		    &pbst_lwnn->fw_manager.dsps[i].name_msgbox);
		if (ret == -EINVAL || ret == -ENODATA) {
			BST_LWNN_DEV_ERR(&pbst_lwnn->pdev->dev,
		...
		BST_LWNN_STAGE_PRINTK("firmware1(msgbox) name %d: %s", i,
		    pbst_lwnn->fw_manager.dsps[i].name_msgbox);
		...
		//map firmware memory
		res =
		    platform_get_resource(pbst_lwnn->pdev, IORESOURCE_MEM,
		    1 + i);
		if (!res) {
			BST_LWNN_DEV_ERR(&pbst_lwnn->pdev->dev,
			    "could not get firmware resource for DSP %d", i);
			goto cur_iter_failure;
		}
		...
		pbst_lwnn->fw_manager.dsps[i].fwmem_base =
		    devm_ioremap_resource(&pbst_lwnn->pdev->dev, res);
		if (IS_ERR(pbst_lwnn->fw_manager.dsps[i].fwmem_base)) {
			BST_LWNN_DEV_ERR(&pbst_lwnn->pdev->dev,
			    "failed to remap firmware mem: %ld",
			    PTR_ERR(pbst_lwnn->fw_manager.dsps[i].fwmem_base));
			goto cur_iter_failure;
		}
		...
		if (pbst_lwnn->mem_manager.enable_smmu) {
			...
			BST_LWNN_TRACE_PRINTK(
				"pbst_lwnn fw iommu_map: 0x%llx, end: 0x%llx",
				res->start, res->end
			);
		}
...
skip_iommu_map:
		...
		pbst_lwnn->fw_manager.dsps[i].assigned_mem =
		    ...
		    assigned_mem_size, 0, 0);
		if (pbst_lwnn->fw_manager.dsps[i].assigned_mem == NULL) {
			BST_LWNN_DEV_ERR(&pbst_lwnn->pdev->dev,
			    "could not allocate assigned-mem for DSP %d", i);
			goto cur_iter_failure;
		}
		BST_LWNN_TRACE_PRINTK
...
int bst_lwnn_fw_rt_setup(struct bst_lwnn *pbst_lwnn)
{
	...
	for (i = 0; i < pbst_lwnn->dsp_num; i++) {
		if (pbst_lwnn->dsp_online[i]) {
			...
			init =
			    (void *)(pbst_lwnn->fw_manager.dsps[i].fwmem_base +
			    (pbst_lwnn->fw_manager.dsps[i].rt_init_addr -
				phys_to_bus(
				    pbst_lwnn->fw_manager.dsps
				    [i].fwmem_phys_addr)));
			init->assigned_mem =
			    kern_to_bus(pbst_lwnn,
			...
			if (init->init_status != BST_LWNN_INIT_END) {
				BST_LWNN_DEV_ERR(&pbst_lwnn->pdev->dev,
				    "initialization for target %d failed", i);
				pbst_lwnn->dsp_online[i] = 0;
				continue;
			} else {
				if (pbst_lwnn->fw_manager.
				    ver_info.release_year == 0) {
					...
					pbst_lwnn->fw_manager.
					    ...
					    init->ver_info.patch;
					pbst_lwnn->fw_manager.
					    ver_info.release_month =
					    init->ver_info.month;
					pbst_lwnn->fw_manager.
					    ver_info.release_date =
					    init->ver_info.date;
					pbst_lwnn->fw_manager.
					    ver_info.release_year =
					    init->ver_info.year;
					BST_LWNN_STAGE_PRINTK
			...
			if (bst_lwnn_msg_xchg(pbst_lwnn, &msg_xchg) < 0) {
				BST_LWNN_DEV_ERR(&pbst_lwnn->pdev->dev,
				    ...
				    i);
				pbst_lwnn->dsp_online[i] = 0;
			} else if (msg_xchg.rsp.status != RT_CMD_STATUS_SUCCESS) {
				BST_LWNN_DEV_ERR(&pbst_lwnn->pdev->dev,
...
void bst_lwnn_fw_manager_cleanup(struct bst_lwnn *pbst_lwnn)
{
	...
	for (i = 0; i < pbst_lwnn->dsp_num; i++) {
		if (!pbst_lwnn->dsp_online[i]
			...
			pbst_lwnn->mem_manager.ops->free(pbst_lwnn->fw_manager.
			    dsps[i].assigned_mem);
		}
	}
	return;
...
static inline void _release_rt_fw(struct bst_lwnn *pbst_lwnn, int dsp)
{
	...
	writel_relaxed(reg, pbst_lwnn->fw_manager.lb_cv_reg_base + LB_CV_REG_R_CV_SYS_CTRL_OFFSET);
	//then reset the DSP
...
void bst_lwnn_fw_manager_exit(struct bst_lwnn *pbst_lwnn)
{
	...
	for (i = 0; i < pbst_lwnn->dsp_num; i++) {
		if (pbst_lwnn->mem_manager.enable_smmu) {
			pbst_lwnn->mem_manager.ops->iommu_free(
				...
				pbst_lwnn->fw_manager.dsps[i].fwmem_size,
				0,
				addr_truncate(phys_to_bus(pbst_lwnn->fw_manager.dsps[i].fwmem_phys_addr))
			);
			if (i == 2) {
	...
	if (pbst_lwnn->mem_manager.enable_smmu) {
		pbst_lwnn->mem_manager.ops->iommu_free(
			...
			0x2000000,
			PAGE_SIZE,
			addr_truncate(phys_to_bus(0x81a000000))
		); // free cv reserved
	}
	return;
}
	...
	for (i = 0; i < pbst_lwnn->dsp_num; i++) {
		if (pbst_lwnn->dsp_online[i]) {
			if (pbst_lwnn->dsp_rtinit[i]) {
				...
				init =
				    (void *)(pbst_lwnn->fw_manager.
				    dsps[i].fwmem_base +
				    (pbst_lwnn->fw_manager.
					dsps[i].rt_init_addr -
					phys_to_bus(
					    pbst_lwnn->fw_manager.
					    dsps[i].fwmem_phys_addr)));
				for (j = 0; j < 100; j++) {
					if (init->init_status ==
					    BST_LWNN_INIT_EXIT) {
						break;
