...
static inline int32_t call_disp_req_callback(des_buf_t *des)
{
	...
	des_buf_t *buf = NULL;
	com_client_data_t *data = s_data;
	cvdsp2_disp_req_callback_t cb = NULL;
	uint32_t rep = 0;
	...
	cb = (cvdsp2_disp_req_callback_t)(reg->cb);
	if (cb)
...
static inline int32_t call_disp_run_callback(des_buf_t *des)
{
	...
	des_buf_t *buf = NULL;
	com_client_data_t *data = s_data;
	cvdsp2_disp_run_callback_t cb = NULL;
	uint32_t status = 0;
	uint32_t perf_us = 0;
	cvdsp2_ErrorEnum_t err = 0;
	...
	// call callback function
	cb = (cvdsp2_disp_run_callback_t)(reg->cb);
	if (cb)
