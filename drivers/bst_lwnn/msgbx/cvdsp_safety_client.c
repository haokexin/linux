...
static cvdsp_safety_client_ext_t *s_ext;
				...
				)
{
	...
	*out->psm_id = (cvdsp_safety_UInt32Array4_t *)psm_id;
	*out->err = err;
...
static inline int32_t call_fusaenable_method_callback(des_buf_t *des)
{
	...
	des_buf_t *buf = NULL;
	com_client_data_t *data = s_data;
	cvdsp_safety_fusaenable_method_callback_t cb = NULL;
	...
	if (ret >= 0)
		ret = deserialize_cvdsp_safety_ErrorEnum(buf, &err);

	if (ret < 0)
		return -ERR_APP_SERDES;
	if (err == CVDSP_SAFETY_NO_ERROR) {
		if (ret >= 0)
			ret = deserialize_8(buf, (uint8_t *)&block_id_out);
		if (ret >= 0)
			ret = deserialize_cvdsp_safety_UInt32Array4(buf, &psm_id);
		if (ret < 0)
			return -ERR_APP_SERDES;
	}

	// call callback function
	cb = (cvdsp_safety_fusaenable_method_callback_t)(reg->cb);
	if (cb)
...
// initialize client
int32_t cvdsp_safety_client_init(com_client_data_t *data, cvdsp_safety_client_t *client,
