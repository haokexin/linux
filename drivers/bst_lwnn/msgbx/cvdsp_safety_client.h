...
#ifndef CVDSP_SAFETY_CLIENT_H
#define CVDSP_SAFETY_CLIENT_H
...
#endif
#include "cvdsp_safety_datatype.h"
 ...
 */
typedef void (*cvdsp_safety_fusaenable_method_callback_t)(
				const uint8_t block_id_out,
				const cvdsp_safety_UInt32Array4_t *psm_id,
				const cvdsp_safety_ErrorEnum_t err,
				void *ext,
...
struct _cvdsp_safety_client_t {
	...
	int32_t (*fusaenable_method_sync)(
					...
					cvdsp_safety_UInt32Array4_t **psm_id,
					cvdsp_safety_ErrorEnum_t *err,
					int64_t timeout_ms,
					des_buf_t *ext_buf
					);
	#endif
	...
	int32_t (*fusaenable_method_async)(
					const uint8_t block_id,
					cvdsp_safety_fusaenable_method_callback_t cb,
					void *ext,
					des_buf_t *ext_buf
					);
...
#define cvdsp_safety_client_t struct _cvdsp_safety_client_t

/**
 *  The extend data used by the client.
 */
struct _cvdsp_safety_client_ext_t {
	uint8_t cid;
	uint8_t ccid;
	uint8_t status;
	uint8_t res[5];
...
#define cvdsp_safety_client_ext_t struct _cvdsp_safety_client_ext_t

/**
 * Initializes the client.
 *
 * @param data The data for com_client_data_t
 * @param client The data for cvdsp_safety_client_t
 * @param ext The data for cvdsp_safety_client_ext_t
 * @return 0 if success, negative if fail.
 */
int32_t cvdsp_safety_client_init(com_client_data_t *data,
			cvdsp_safety_client_t *client,
			cvdsp_safety_client_ext_t *ext);

/**
 * Destroys the client.
 */
void cvdsp_safety_client_destroy(void);

#ifdef __cplusplus
}
#endif

#endif // CVDSP_SAFETY_CLIENT_H
