...
static cvdsp1_client_ext_t *s_ext;
...
static inline int32_t call_disp_req_callback(des_buf_t *des)
{
	...
	des_buf_t *buf = NULL;
	com_client_data_t *data = s_data;
	cvdsp1_disp_req_callback_t cb = NULL;
	uint32_t rep = 0;
	...
	if (ret >= 0)
		ret = deserialize_cvdsp1_ErrorEnum(buf, &err);

	if (ret < 0)
		return -ERR_APP_SERDES;
	if (err == CVDSP1_NO_ERROR) {
		if (ret >= 0)
	...
	cb = (cvdsp1_disp_req_callback_t)(reg->cb);
	if (cb)
...
static inline int32_t call_disp_run_callback(des_buf_t *des)
{
	...
	des_buf_t *buf = NULL;
	com_client_data_t *data = s_data;
	cvdsp1_disp_run_callback_t cb = NULL;
	uint32_t status = 0;
	uint32_t perf_us = 0;
	cvdsp1_ErrorEnum_t err = 0;
	...
	// call callback function
	cb = (cvdsp1_disp_run_callback_t)(reg->cb);
	if (cb)
...
// initialize client
int32_t cvdsp1_client_init(com_client_data_t *data, cvdsp1_client_t *client,
