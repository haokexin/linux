...
static int32_t receive_message(void)
{
	...
	cvdsp0_client_ext_t *cvdsp0_ext = &s_ins->cvdsp0_ext;
	cvdsp1_client_ext_t *cvdsp1_ext = &s_ins->cvdsp1_ext;
	cvdsp2_client_ext_t *cvdsp2_ext = &s_ins->cvdsp2_ext;
	cvdsp3_client_ext_t *cvdsp3_ext = &s_ins->cvdsp3_ext;
	cvdsp_safety_client_ext_t *cvdsp_safety_ext = &s_ins->cvdsp_safety_ext;
	...
	if (ret == QUERY_INFO_DST_STS_OFFLINE) {
		if (cvdsp0_ext->avail_changed_cb)
			cvdsp0_ext->avail_changed_cb(false, cvdsp0_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_ONLINE) {
		if (cvdsp0_ext->avail_changed_cb)
			cvdsp0_ext->avail_changed_cb(true, cvdsp0_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_CHANGED) {
		ret = ipc_trans_layer_get_endmap(data->pid, &map);
		if (ret == 0) {
			chip_map = cvdsp0_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & cvdsp0_ext->cid_mask) != 0 ? true : false;
			if (status != cvdsp0_ext->status) {
				cvdsp0_ext->status = status;
				if (cvdsp0_ext->avail_changed_cb)
					cvdsp0_ext->avail_changed_cb(status, cvdsp0_ext->avail_ext);
			}
			chip_map = cvdsp1_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & cvdsp1_ext->cid_mask) != 0 ? true : false;
			if (status != cvdsp1_ext->status) {
				cvdsp1_ext->status = status;
				if (cvdsp1_ext->avail_changed_cb)
					cvdsp1_ext->avail_changed_cb(status, cvdsp1_ext->avail_ext);
			}
			chip_map = cvdsp2_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & cvdsp2_ext->cid_mask) != 0 ? true : false;
			if (status != cvdsp2_ext->status) {
				cvdsp2_ext->status = status;
				if (cvdsp2_ext->avail_changed_cb)
					cvdsp2_ext->avail_changed_cb(status, cvdsp2_ext->avail_ext);
			}
			chip_map = cvdsp3_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & cvdsp3_ext->cid_mask) != 0 ? true : false;
			if (status != cvdsp3_ext->status) {
				cvdsp3_ext->status = status;
				if (cvdsp3_ext->avail_changed_cb)
					cvdsp3_ext->avail_changed_cb(status, cvdsp3_ext->avail_ext);
			}
			chip_map = cvdsp_safety_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & cvdsp_safety_ext->cid_mask) != 0 ? true : false;
			if (status != cvdsp_safety_ext->status) {
				cvdsp_safety_ext->status = status;
				if (cvdsp_safety_ext->avail_changed_cb)
					cvdsp_safety_ext->avail_changed_cb(status, cvdsp_safety_ext->avail_ext);
			}
...
static int32_t start(void)
{
	...
	if (ret != 0) {
#elif defined IPC_RTE_RTOS
	data->route_task = TaskCreate((task_func_t)router_func, "lwnn_client_thread", 0, NULL,
...
lwnn_client_t *lwnn_client_init(lwnn_client_data_t *ins)
{
	...
	if (ret < 0)
		return NULL;
	ret = cvdsp_safety_client_init(data, &ins->client.cvdsp_safety_client, &ins->cvdsp_safety_ext);
	if (ret < 0)
		return NULL;
