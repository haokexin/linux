...
static touchclient_data_t *s_ins;
...
static int32_t receive_message(void)
{
	...
	bst_touch_client_ext_t *bst_touch_ext = &s_ins->bst_touch_ext;
	...
	if (ret == QUERY_INFO_DST_STS_OFFLINE) {
		if (bst_touch_ext->avail_changed_cb)
			bst_touch_ext->avail_changed_cb(false, bst_touch_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_ONLINE) {
		if (bst_touch_ext->avail_changed_cb)
			bst_touch_ext->avail_changed_cb(true, bst_touch_ext->avail_ext);
	} else if (ret == QUERY_INFO_DST_STS_CHANGED) {
		ret = ipc_trans_layer_get_endmap(data->pid, &map);
		if (ret == 0) {
			chip_map = bst_touch_ext->ccid != 0 ? map.hi_map : map.lo_map;
			status = (chip_map & bst_touch_ext->cid_mask) != 0 ? true : false;
			if (status != bst_touch_ext->status) {
				bst_touch_ext->status = status;
				if (bst_touch_ext->avail_changed_cb)
					bst_touch_ext->avail_changed_cb(status, bst_touch_ext->avail_ext);
			}
...
static int32_t dispatch_message(void)
{
	...
	while (packet_obj) {
		...
		// dispatch message
		ret = -1;
		if (des->header.pid == s_ins->bst_touch_ext.cid) {
			if (des->header.typ == MSGBX_MSG_TYPE_BROADCAST)
				ret = s_ins->client.bst_touch_client.dispatch_broadcast(des);
			else if (des->header.typ == MSGBX_MSG_TYPE_REPLY)
				ret = s_ins->client.bst_touch_client.dispatch_reply(des);
		}
		if (ret < 0)
...
static int32_t start(void)
{
	...
	if (ret != 0) {
#elif defined IPC_RTE_RTOS
	data->route_task = TaskCreate((task_func_t)router_func, "touchclient_thread", 0, NULL,
		0, (uint8_t *)stack_buffer, &tcb_buffer, true);
	if (data->route_task == 0) {
#elif defined IPC_RTE_KERNEL
	data->route_task = kthread_run(router_func, NULL, "touchclient_thread");
...
touchclient_t *touchclient_init(touchclient_data_t *ins)
{
	...
	// init clients
	ret = bst_touch_client_init(data, &ins->client.bst_touch_client, &ins->bst_touch_ext);
	if (ret < 0)
		return NULL;

	// create client handle.
	ret = ipc_trans_layer_proxy_create_handle(data->pid, data->fid, data->sid, ins->bst_touch_ext.cid, ins->bst_touch_ext.ccid, &data->handle);
	if (ret < 0)
	{
