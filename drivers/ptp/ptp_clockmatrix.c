...
static int idtcm_state_machine_reset(struct idtcm *idtcm)
{
	...
	if (!err) {
		...
		if (!status)
			dev_dbg(idtcm->dev, "Timed out waiting for CM_RESET to complete\n");
	}
...
{
	if (index >= MAX_TOD) {
		dev_err(idtcm->dev, "ToD%d not supported\n", index);
		return -EINVAL;
	}

	if (pll >= MAX_PLL) {
		dev_err(idtcm->dev, "Pll%d not supported\n", pll);
		return -EINVAL;
	}
...
static void display_pll_and_masks(struct idtcm *idtcm)
{
	...
	dev_dbg(idtcm->dev, "tod_mask = 0x%02x\n", idtcm->tod_mask);
			       ...
			       struct device *dev)
{
	...
	dev_info(idtcm->dev, "firmware '%s'", fname);
	...
	dev_dbg(idtcm->dev, "firmware size %zu bytes\n", fw->size);
