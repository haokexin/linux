		...
		gfp_t mem_flags)
{
	...
	if (!hrtimer_active(&dum_hcd->timer) && !list_empty(&dum_hcd->urbp_list))
...
static int dummy_urb_dequeue(struct usb_hcd *hcd, struct urb *urb, int status)
{
	...
	if (!rc && !list_empty(&dum_hcd->urbp_list) &&
...
treat_control_like_bulk:
			ep->last_io = jiffies;
			send_len = transfer(dum_hcd, urb, ep, limit, &status);
			if (send_len)
				dummy_last_io = jiffies;
...
return_urb:
	...
	if (list_empty(&dum_hcd->urbp_list)) {
		...
		if (!dummy_last_io || ((jiffies - dummy_last_io) < 2*HZ))
			hrtimer_start(&dum_hcd->timer, ns_to_ktime(DUMMY_TIMER_INT_NSECS), HRTIMER_MODE_REL);
		else
			pr_debug("%s %d  %ld last %ld\n", __func__, __LINE__, jiffies, dummy_last_io);
	}
...
static int dummy_bus_resume(struct usb_hcd *hcd)
{
	...
	if (!HCD_HW_ACCESSIBLE(hcd)) {
		...
		set_link_state(dum_hcd);
		if (!list_empty(&dum_hcd->urbp_list) && (!hrtimer_active(&dum_hcd->timer)))
			hrtimer_start(&dum_hcd->timer, ns_to_ktime(0), HRTIMER_MODE_REL);
