...
/* set AHB-prot/AXI-cache/OCP-ReqInfo size for read and write configuration registers */
static void dwc3_set_reqinfo_len(struct dwc3 *dwc)
{
	struct device *dev = dwc->dev;
	u32 val = 0;
	u32 cfg;
	...
	cfg = dwc3_readl(dwc->regs, DWC3_GSBUSCFG0);
	ret = device_property_read_u32(dev, "snps,reqinfo-for-data-read", &val);
	if (ret == 0) {
		cfg &= ~DWC3_GSBUSCFG0_REQINFO_DATA_READ_MASK;
		cfg |= DWC3_GSBUSCFG0_REQINFO_DATA_READ(val);
	}
	ret = device_property_read_u32(dev, "snps,reqinfo-for-descriptor-read", &val);
	if (ret == 0) {
		cfg &= ~DWC3_GSBUSCFG0_REQINFO_DES_READ_MASK;
		cfg |= DWC3_GSBUSCFG0_REQINFO_DES_READ(val);
	}
	ret = device_property_read_u32(dev, "snps,reqinfo-for-data-write", &val);
	if (ret == 0) {
		cfg &= ~DWC3_GSBUSCFG0_REQINFO_DATA_WRITE_MASK;
		cfg |= DWC3_GSBUSCFG0_REQINFO_DATA_WRITE(val);
	}
	ret = device_property_read_u32(dev, "snps,reqinfo-for-descriptor-write", &val);
	if (ret == 0) {
		cfg &= ~DWC3_GSBUSCFG0_REQINFO_DES_WRITE_MASK;
		cfg |= DWC3_GSBUSCFG0_REQINFO_DES_WRITE(val);
	}
	dwc3_writel(dwc->regs, DWC3_GSBUSCFG0, cfg);
}

static void dwc3_set_deemph(struct dwc3 *dwc)
{
	struct device *dev = dwc->dev;
	u32 val = 0;
	u32 cfg;
	...
	ret = device_property_read_u32(dev, "snps,ss_gen1_deemph", &val);
	if (ret == 0) {
		cfg = dwc3_readl(dwc->regs, DWC3_GUSB3PIPECTL(0));
		cfg &= ~DWC3_SS_TX_DE_EMPHASIS_MASK;
		cfg |= DWC3_SS_TX_DE_EMPHASIS(val);
		dwc3_writel(dwc->regs, DWC3_GUSB3PIPECTL(0), cfg);
	}
	...
	if (ret == 0) {
		for (i = val - 1; i >= 0; i--) {
			if (tx_deemph[i] == 0)
				continue;
			cfg = dwc3_readl(dwc->regs, DWC3_LCSR_TX_DEEMPH(0) + (i * 4));
			cfg &= ~0x3ffff;
			cfg |= tx_deemph[i];
			dwc3_writel(dwc->regs, DWC3_LCSR_TX_DEEMPH(0) + (i * 4), cfg);
		}
	}
}
