...
static int cma_heap_get_phy_addrs(struct dma_heap *heap,
			struct dma_buf *dmabuf, union dma_heap_phy_addrs_info *info)
{
	struct cma_heap_buffer *buffer = dmabuf->priv;

	info->pages = buffer->cma_pages;
	info->pagecount = buffer->pagecount;
	return CMA_HEAP;
}

static const struct dma_heap_ops cma_heap_ops = {
	.allocate = cma_heap_allocate,
	.get_phy_addrs = cma_heap_get_phy_addrs
};
...
static int __add_cma_heap(struct cma *cma, void *data)
{
	...
	/*
	 * If a "linux,cma-heap" dts property is present, then Linux
	 * will export the CMA region via the DMA-BUF Heaps interface,
	...
	printk("__add_cma_heap: exp_info.name = %s\n", exp_info.name);
