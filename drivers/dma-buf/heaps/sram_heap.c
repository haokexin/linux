 ...
 *
 * Copyright (C) 2025 Black Sesame Technologies, Inc.
 *
...
/* Contains all sram dma heaps */
static LIST_HEAD(sram_dma_heap_list);
...
static int sram_dma_heap_export(const char *name,
			 struct gen_pool *sram_gp, bool is_global)
{
	struct sram_dma_heap *sram_dma_heap;
	struct dma_heap_export_info exp_info;

	pr_info("Exporting SRAM pool '%s'%s\n", name, is_global ? " as global" : "");

	sram_dma_heap = kzalloc(sizeof(*sram_dma_heap), GFP_KERNEL);
	if (!sram_dma_heap)
		return -ENOMEM;
	sram_dma_heap->pool = sram_gp;
	strscpy(sram_dma_heap->name, name, sizeof(sram_dma_heap->name));

	exp_info.name = sram_dma_heap->name;
	...
	list_add(&sram_dma_heap->node, &sram_dma_heap_list);

	return 0;
}

static struct dma_buf *__sram_heap_alloc_dma_buf(struct sram_dma_heap *sram_dma_heap,
						 size_t size)
{
	struct sram_dma_heap_buffer *buffer;
...
{
	if (!sram_dma_heap_global)
		return ERR_PTR(-ENODEV);
	return __sram_heap_alloc_dma_buf(sram_dma_heap_global, size);
}
EXPORT_SYMBOL_GPL(sram_heap_alloc_dma_buf);

struct dma_buf *sram_heap_alloc_dma_buf_by_name(const char *name, size_t size)
{
	struct sram_dma_heap *sram_dma_heap;

	list_for_each_entry(sram_dma_heap, &sram_dma_heap_list, node) {
		if (strcmp(sram_dma_heap->name, name) == 0)
			return __sram_heap_alloc_dma_buf(sram_dma_heap, size);
	}

	pr_err("SRAM heap '%s' not found\n", name);
	return ERR_PTR(-ENODEV);
}
EXPORT_SYMBOL_GPL(sram_heap_alloc_dma_buf_by_name);

static struct page *__sram_heap_alloc_pages(struct sram_dma_heap *sram_dma_heap, size_t size)
{
	void *vaddr;
	phys_addr_t paddr;
	struct page *p;
...
struct page *sram_heap_alloc_pages(size_t size)
{
	if (!sram_dma_heap_global)
		return ERR_PTR(-ENODEV);
	return __sram_heap_alloc_pages(sram_dma_heap_global, size);
}
EXPORT_SYMBOL_GPL(sram_heap_alloc_pages);

struct page *sram_heap_alloc_pages_by_name(const char *name, size_t size)
{
	struct sram_dma_heap *sram_dma_heap;

	list_for_each_entry(sram_dma_heap, &sram_dma_heap_list, node) {
		if (strcmp(sram_dma_heap->name, name) == 0)
			return __sram_heap_alloc_pages(sram_dma_heap, size);
	}

	pr_err("SRAM heap '%s' not found\n", name);
	return ERR_PTR(-ENODEV);
}
EXPORT_SYMBOL_GPL(sram_heap_alloc_pages_by_name);
...
static void __sram_heap_free_pages(struct sram_dma_heap *sram_dma_heap, struct page *p)
{
	void *vaddr;
...
{
	return __sram_heap_free_pages(sram_dma_heap_global, p);
}
EXPORT_SYMBOL_GPL(sram_heap_free_pages);

void sram_heap_free_pages_by_name(const char *name, struct page *p)
{
	struct sram_dma_heap *sram_dma_heap;

	list_for_each_entry(sram_dma_heap, &sram_dma_heap_list, node) {
		if (strcmp(sram_dma_heap->name, name) == 0)
			return __sram_heap_free_pages(sram_dma_heap, p);
	}

	pr_err("SRAM heap '%s' not found\n", name);
}
EXPORT_SYMBOL_GPL(sram_heap_free_pages_by_name);
...
static int add_sram_heap(void)
{
	struct device_node *np = NULL, *np_pool = NULL, *default_np = NULL;
	struct gen_pool *sram_gp = NULL;
	int i = 0, ret = 0, count = 0;

	np = of_find_compatible_node(NULL, NULL, "bst,sram-heap");
	if (!np) {
	...
	count = of_count_phandle_with_args(np, "bst,sram", NULL);

	for (i = 0; i < count; i++) {
		sram_gp = of_gen_pool_get(np, "bst,sram", i);
		if (sram_gp == NULL) {
			pr_err("failed to get sram pool\n");
			return -ENOMEM;
		}
		np_pool = of_parse_phandle(np, "bst,sram", i);
		default_np = of_parse_phandle(np, "bst,default-sram", 0);
		ret = sram_dma_heap_export(np_pool->name, sram_gp,
					   default_np == NULL ? false :
					   np_pool->phandle == default_np->phandle);
		if (ret) {
			pr_err("failed to export sram heap\n");
			goto out;
		}
	}
out:
...
module_init(add_sram_heap);
MODULE_DESCRIPTION("BST DMA-BUF SRAM Heap");
MODULE_LICENSE("GPL");
