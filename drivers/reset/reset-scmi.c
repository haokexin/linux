...
#if 0
static int __init scmi_reset_init_driver(void)
{
	return scmi_driver_register(&scmi_reset_driver,THIS_MODULE,KBUILD_MODNAME);
}
 subsys_initcall(scmi_reset_init_driver);

static void __exit scmi_reset_exit_driver(void)
{
	scmi_driver_unregister(&scmi_reset_driver);
}
module_exit(scmi_reset_exit_driver);
#endif
