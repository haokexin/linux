...
/*ufs intr for multisystem*/
#if defined(CONFIG_BST_UFS_UTRD_SLOT_NUMBER) && defined(CONFIG_BST_UFS_UTRD_SLOT_START)
#define UFSCD_UTRLCNR_MASK	((u32)(((0x1ul<<CONFIG_BST_UFS_UTRD_SLOT_NUMBER) - 1)<<CONFIG_BST_UFS_UTRD_SLOT_START))
#else
#define UFSCD_UTRLCNR_MASK   (0ul)
#endif
#define UFSCD_INTR_STATUS_MASK ((u32)(~UFSCD_UTRLCNR_MASK))

//utrdl and utmrdl base addr
#define UFSHCD_UTRDL_BASEADDR	(0x800000000)
#define UFSHCD_UTMRDL_BASEADDR	(0x800001000)
...
static inline int ufshcd_hba_capabilities(struct ufs_hba *hba)
{
	...
	default_slot = CONFIG_BST_UFS_UTRD_SLOT_START;
#endif	
	...
	hba->reserved_slot = default_slot;
...
static int ufshcd_memory_alloc(struct ufs_hba *hba)
{
	 ...
	 */
	hba->utrdl_dma_addr = UFSHCD_UTRDL_BASEADDR;
	hba->utrdl_base_addr = ioremap_wc(UFSHCD_UTRDL_BASEADDR, 0x1000);
	 ...
	 */
	hba->utmrdl_dma_addr = UFSHCD_UTMRDL_BASEADDR;
	hba->utmrdl_base_addr = ioremap_wc(UFSHCD_UTMRDL_BASEADDR, 0x1000);
...
static void ufshcd_host_memory_configure(struct ufs_hba *hba)
{
	...
	for (i = slot_start; i < slot_start + slot_num; i++) {
...
static int ufshcd_poll(struct Scsi_Host *shost, unsigned int queue_num)
{
	...
	if (1) {
		utrlcnr = ufshcd_readl(hba, REG_UTP_TRANSFER_REQ_LIST_COMPL) & UFSCD_UTRLCNR_MASK;
		if (utrlcnr) {
			ufshcd_writel(hba, utrlcnr, REG_UTP_TRANSFER_REQ_LIST_COMPL);
...
static irqreturn_t ufshcd_intr(int irq, void *__hba)
{
	...
	while (intr_status && retries--) {
		...
		if (intr_status && (0 == (utrlcnr & UFSCD_INTR_STATUS_MASK)))
		...
		intr_status = ufshcd_readl(hba, REG_INTERRUPT_STATUS);
		utrlcnr = ufshcd_readl(hba, REG_UTP_TRANSFER_REQ_LIST_COMPL);
		if((utrlcnr & UFSCD_UTRLCNR_MASK) == 0 && (intr_status & UTP_TRANSFER_REQ_COMPL) == UTP_TRANSFER_REQ_COMPL)
			break;
	}
...
#if 0
	char *ori_byte;
	void __iomem *ori_addr = ioremap(0x800000000, 0x20000);
	backup_ptr = (char *)kzalloc(0x20000, GFP_KERNEL);
	if (!backup_ptr) {
		dev_err(dev, "Memory allocation failed\n");
		goto out;
	}
	ori_byte = (char *)ori_addr;
	for(int i=0; i<0x20000; i++) {
		backup_ptr[i] = ori_byte[i];
	}
	iounmap(ori_addr);
	return 0;
#endif
...
int ufshcd_init(struct ufs_hba *hba, void __iomem *mmio_base, unsigned int irq)
{
	...
	host->can_queue = slot_start + slot_num;
	...
	hba->tmf_tag_set = (struct blk_mq_tag_set) {
		.nr_hw_queues	= 1,
		.queue_depth	= utmrd_slot_start + utmrd_slot_num,
		.reserved_tags  = utmrd_slot_start,
		.ops		= &ufshcd_tmf_ops,
