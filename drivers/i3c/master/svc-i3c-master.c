...
struct svc_i3c_master* p_master[2];

static struct kobject *svc_kobj;

ssize_t svc_daa_show(struct kobject *object,
			     struct kobj_attribute *attr, char *buf)
{
	int ret;

	ret = sprintf(buf,"svc master%d svc_daa_show called !\n", call_num);
	ret = strlen(buf);

	return ret > PAGE_SIZE ? PAGE_SIZE : ret;
}

ssize_t svc_daa_store(struct kobject *object,
			      ...
			      size_t count)
{
	...
	i3c_master_do_daa(&p_master[value]->base);

	return count;
}

static struct kobj_attribute s_svc_daa_attribute = __ATTR(
	svc_daa, 0664, svc_daa_show, svc_daa_store);

static struct attribute *attrs[] = {
	&s_svc_daa_attribute.attr, 
...
int svc_i3c_master_register_sysfs(struct svc_i3c_master *master)
{
	int ret = -1;

	p_master[call_num] = master;
	if(call_num == 0){
		svc_kobj = kobject_create_and_add("svc", NULL);
		if (!svc_kobj) {
		...
		ret = sysfs_create_group(svc_kobj, &attr_group);
		if (ret) {
...
static int svc_i3c_master_probe(struct platform_device *pdev)
{
	...
	svc_i3c_master_register_sysfs(master);
