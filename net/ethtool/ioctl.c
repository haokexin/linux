...
static int ethtool_set_fpe(struct net_device *dev, void __user *useraddr)
{
	struct ethtool_fpe eth_fpe;

	if (!dev->ethtool_ops->set_fpe)
		return -EOPNOTSUPP;

	if (copy_from_user(&eth_fpe, useraddr, sizeof(eth_fpe)))
		return -EFAULT;

	return dev->ethtool_ops->set_fpe(dev, &eth_fpe);
}

static int ethtool_get_fpe(struct net_device *dev, void __user *useraddr)
{
	struct ethtool_fpe eth_fpe;
	int rc;

	if (!dev->ethtool_ops->get_fpe)
		return -EOPNOTSUPP;

	rc = dev->ethtool_ops->get_fpe(dev, &eth_fpe);
	if (rc)
		return rc;

	if (copy_to_user(useraddr, &eth_fpe, sizeof(eth_fpe)))
		return -EFAULT;
	      ...
	      u32 ethcmd, struct ethtool_devlink_compat *devlink_state)
{
	...
	case ETHTOOL_SFECPARAM:
		...
		break;
		case ETHTOOL_GFPE:
		rc = ethtool_get_fpe(dev, useraddr);
		break;
	case ETHTOOL_SFPE:
		rc = ethtool_set_fpe(dev, useraddr);
		break;
	default:
		rc = -EOPNOTSUPP;
	}
